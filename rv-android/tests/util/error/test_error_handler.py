...
class TestErrorHandler:
    ...
    @pytest.fixture
    def handler_with_mocks(self, mock_logger, mock_registry, mock_event_bus):
        """Fixture providing an ErrorHandler with mocked dependencies."""
        with patch('rvandroid.util.error.error_handler.LoggingManager') as mock_logging_manager:
            ...
            with patch('rvandroid.util.error.error_handler.HandlerRegistry', return_value=mock_registry):
                # Create the handler instance
                handler = ErrorHandler()

                # Set the mocked event bus directly
                handler._event_bus = mock_event_bus

                yield handler, mock_logger, mock_registry, mock_event_bus
    ...
    def test_handle_error_success(self, handler_with_mocks):
        ...
        """
        handler, mock_logger, mock_registry, mock_event_bus = handler_with_mocks
        ...
        assert mock_event_bus.publish_analysis_event.called
    ...
    def test_handle_error_with_context(self, handler_with_mocks):
        ...
        # Verify event was published with context
        assert mock_event_bus.publish_analysis_event.called

        # Check if task_id was properly used in the event publishing
        call_args = mock_event_bus.publish_analysis_event.call_args[1]
        assert call_args.get('related_task_id') == 123

    def test_event_publishing(self, handler_with_mocks):
        """
        Test event publishing for errors.

        Validates:
        - Events are published with correct type
        - Event data contains error information
        - Task ID is correctly passed if available
        """
        handler, _, mock_registry, mock_event_bus = handler_with_mocks

        # Import here to avoid circular imports in the actual code
        ...
        # Create test error and context with task_id
        test_error = ValueError("Test error")
        test_context = {"task_id": 123}
        ...
        # Verify event type
        assert call_args[0][0] == EventType.ERROR_DETECTED

        # Verify event data contains error information
        event_data = call_args[1]['data']
        assert event_data['error_type'] == 'ValueError'
        assert event_data['error_message'] == 'Test error'
        assert event_data['context'] == test_context

        # Verify task_id was passed
        assert call_args[1]['related_task_id'] == 123

        # Verify source was set
        assert call_args[1]['source'] == 'ErrorHandler'

    def test_event_publishing_no_task_id(self, handler_with_mocks):
        """
        Test event publishing without task ID.

        Validates:
        - Events are published correctly even without task ID
        - Event publishing adapts to missing context information
        """
        handler, _, mock_registry, mock_event_bus = handler_with_mocks
        ...
        # Create test error with no context
        test_error = ValueError("Test error")

        # Handle the error
        handler.handle_error(test_error, None)

        # Verify event was published without related_task_id
        mock_event_bus.publish_analysis_event.assert_called_once()
        call_args = mock_event_bus.publish_analysis_event.call_args

        # Verify the method was called without related_task_id
        assert 'related_task_id' not in call_args[1]
    ...
    #     handler._event_bus = None
    #
    #     with patch('rvandroid.util.error.error_handler.EventBus', side_effect=ImportError("Module not found")):
    ...
    #     with patch('rvandroid.util.error.error_handler.EventType') as MockEventType:
    ...
    #     with patch('rvandroid.util.error.recovery_strategies.RecoveryStrategies.handle_adb_error') as mock_adb_handler, \
    #             patch(
    #                 'rvandroid.util.error.recovery_strategies.RecoveryStrategies.handle_emulator_error') as mock_emulator_handler, \
    #             patch(
    #                 'rvandroid.util.error.recovery_strategies.RecoveryStrategies.handle_timeout_error') as mock_timeout_handler:
    ...
    def test_no_event_bus_available(self, handler_with_mocks):
        """
        Test behavior when event bus is not available.

        Validates:
        - Error handling works correctly when event bus is missing
        - System gracefully handles missing event bus
        """
        handler, _, mock_registry, _ = handler_with_mocks

        # Set event bus to None to simulate unavailable bus
        handler._event_bus = None

        # Patch the event bus property to always return None
        with patch.object(ErrorHandler, 'event_bus', return_value=None):
            # Setup registry to return a successful handler
            mock_registry.find_handlers.return_value = [MagicMock(return_value=True)]

            # Handle an error
            # This should not raise an exception even without event bus
            handler.handle_error(ValueError("Test error"), None)

...
@pytest.fixture
def execution_controller(mock_task_storage, mock_event_bus):
    """Create an ExecutionController instance for testing."""
    return ExecutionController(mock_task_storage, mock_event_bus)
...
def test_copy_static_analysis_files(execution_controller, tmp_path, mock_app):
    ...
    # Patch the constants to use the temp path
    with patch('rvandroid.experiment.workflow.execution_controller.INSTRUMENTED_DIR', str(tmp_path)):
        # Create mock files
        extensions = [
            EXTENSION_METHODS,
            EXTENSION_GESDA,

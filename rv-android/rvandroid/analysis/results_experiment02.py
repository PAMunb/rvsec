        ...
        for (apk, tool), group in self.df.groupby(['apk', 'tool']):
            ...
            for metric in ['cov_class', 'cov_method', 'cov_rv_method']:
                ...
                if max_coverage > 0:  # Avoid division by zero
                    ...
                    t25 = group[group[metric] >= 0.25 * max_coverage]['time'].min() if any(group[metric] >= 0.25 * max_coverage) else np.nan
                    t50 = group[group[metric] >= 0.50 * max_coverage]['time'].min() if any(group[metric] >= 0.50 * max_coverage) else np.nan
                    t75 = group[group[metric] >= 0.75 * max_coverage]['time'].min() if any(group[metric] >= 0.75 * max_coverage) else np.nan
                    t100 = group[group[metric] >= 0.99 * max_coverage]['time'].min() if any(group[metric] >= 0.99 * max_coverage) else np.nan

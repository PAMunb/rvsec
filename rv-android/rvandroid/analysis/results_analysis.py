# rvandroid/analysis/results_analysis.py
"""
Results analysis module for processing experiment results.
...
def process_results(results_dir: str) -> Dict[str, Any]:
    """
    Process the results of experiment execution using standardized models.
    ...
    try:
        ...
        for app_dir in app_dirs:
            ...
            app_results = process_app_results(app_path)
            ...
            for tool, tool_data in app_results.get("tools", {}).items():
                ...
                results["tools"][tool]["method_coverage"] += tool_data.get("method_coverage", 0) * tool_data.get(
                    "tasks", 0)
                results["tools"][tool]["activity_coverage"] += tool_data.get("activity_coverage", 0) * tool_data.get(
                    "tasks", 0)
                results["tools"][tool]["mop_coverage"] += tool_data.get("mop_coverage", 0) * tool_data.get("tasks", 0)
        ...
        logger.info(f"Results analysis saved to {results_file}")
...
def process_app_results(app_dir: str) -> Dict[str, Any]:
    """
    Process results for a single app using standardized models.

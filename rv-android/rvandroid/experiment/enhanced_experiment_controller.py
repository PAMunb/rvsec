"""
Enhanced experiment controller for RV-Android.

This module provides an enhanced version of the experiment controller that uses 
the new orchestration and analysis systems.
"""
import os
...
class EnhancedExperimentController(ExperimentController):
    ...
    This class extends the standard ExperimentController to use the new orchestration
    and analysis systems, providing improved performance, error handling, and analysis capabilities.
    """
    
    def __init__(self, event_bus: Optional[EventBus] = None):
        """
        Initialize the enhanced experiment controller.
        ...
        self.logger = self.logging_manager.get_logger(
            'experiment_workflow.enhanced_controller',
            {
                'experiment_id': self.experiment_id,
                CONTEXT_COMPONENT: 'EnhancedExperimentController'
            }
        )
        ...
        self.integration_factory = IntegrationFactory(self.event_bus)
        
        # Create integrated workflow factory
        self.factory = self.integration_factory.create_integrated_workflow_factory()
        self.factory.task_storage = self.task_storage
        self.factory.results_dir = self.results_dir
        
        # Initialize workflow components using the integrated factory
        ...
        self.logger.info(f"Enhanced experiment controller initialized for {self.experiment_id}")
    ...
    def _setup_event_handlers(self):
        ...
        def on_experiment_started(event):
            ...
            with self.logger.with_context(phase="experiment_start"):
                self.logger.info(LOG_START.format(
                    operation=f"Enhanced experiment {event.experiment_id}"
                ))
        ...
        # Register handlers using new API
        self.event_bus.subscribe(EventType.EXPERIMENT_STARTED, EventHandler(on_experiment_started), channel=EventBus.LIFECYCLE_CHANNEL)
        self.event_bus.subscribe(EventType.EXPERIMENT_COMPLETED, EventHandler(on_experiment_completed), channel=EventBus.LIFECYCLE_CHANNEL)
        self.event_bus.subscribe(EventType.TASK_STARTED, EventHandler(on_task_started), channel=EventBus.LIFECYCLE_CHANNEL)
        self.event_bus.subscribe(EventType.TASK_FAILED, EventHandler(on_task_failed), channel=EventBus.LIFECYCLE_CHANNEL)
    ...
    def execute(self, repetitions: int, timeouts: List[int], tools: List[AbstractTool],
        ...
        with self.logger.with_context(
            ...
            self.logger.info(LOG_START.format(operation="Enhanced experiment execution"))
            
            # Publish experiment started event
            self.event_bus.publish_experiment_event(
                event_type=EventType.EXPERIMENT_STARTED,
                experiment_id=self.experiment_id,
                message="Starting enhanced experiment execution",
                source="EnhancedExperimentController",
            ...
            if not skip_experiment:
                ...
                # Run the experiment tasks (this will use the new orchestration system)
                self.execution_controller.run()
                
                # Process results (this will use the new analysis system via our adapter)
                self.post_processor.process()
                
                # Generate reports (also enhanced with our new components)
            ...
            self.event_bus.publish_experiment_event(
                ...
                experiment_id=self.experiment_id,
                message="Enhanced experiment execution completed",
                source="EnhancedExperimentController",
                channel=EventBus.LIFECYCLE_CHANNEL
            )
            
            self.logger.info(LOG_COMPLETE.format(operation="Enhanced experiment execution"))
            
            # Return statistics from the experiment
            return {
                'experiment_id': self.experiment_id,
...
def execute_enhanced(tools: Optional[List[AbstractTool]] = None):
    """
    Execute an enhanced experiment with configuration from the Configuration singleton.
    
    This function uses the Configuration singleton to configure and execute
    an experiment using the EnhancedExperimentController.
    ...
    logger = logging_manager.get_logger('experiment_workflow.execute_enhanced', {'function': 'execute_enhanced'})
    ...
    # Create enhanced experiment controller and execute
    with logger.with_context(phase="experiment_execution"):
        logger.info(LOG_START.format(operation="enhanced experiment execution"))
        experiment = EnhancedExperimentController()
        ...
        logger.info(LOG_COMPLETE.format(operation="enhanced experiment execution"))
        return result
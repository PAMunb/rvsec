...
class Experiment03:
    ...
    def __init__(self):
        ...
        self.logger = self.logging_manager.get_logger(
            'experiment.experiment_03',
            {
                ...
                LoggingManager.CONTEXT_COMPONENT: 'Experiment03'
            }
        )
    ...
    def _run_experiment(self, repetitions: int, timeouts: List[int], tools: List[AbstractTool], no_window: bool):
        ...
        with self.logger.with_context(
            ...
            self.logger.info(LoggingManager.LOG_START.format(operation="experiment execution"))
            ...
            # Run all tasks
            self.logger.info("Starting execution of all tasks")
            ...
            self.logger.info(LoggingManager.LOG_COMPLETE.format(operation="experiment execution"))
        ...
        with self.logger.with_context(phase="post_processing"):
            ...
            with self.logger.with_context(phase="diagnostics"):
                try:
                    ...
                    self.logger.error(LoggingManager.LOG_ERROR.format(
                        operation="generating diagnostic report",
                        error=str(e)
                    ), exc_info=True)
...
def execute(tools=None):
    ...
    logger = logging_manager.get_logger('experiment.execute', {'function': 'execute'})
    ...
    with logger.with_context(phase="configuration"):
        ...
        # IMPORTANT: Log explicitly all configuration values
        ...
        # If tools are provided directly, use them and ignore Configuration
        if tools is not None:
            logger.info(f"Using explicitly provided tools: {[tool.name for tool in tools]}")
        # Otherwise, get tools from Configuration
        else:
    ...
    with logger.with_context(phase="experiment_creation"):
        logger.info(LoggingManager.LOG_START.format(operation="experiment creation"))
        experiment = Experiment03()
        logger.info(LoggingManager.LOG_COMPLETE.format(operation="experiment creation"))
    ...
    with logger.with_context(phase="experiment_execution"):
        ...
        logger.info(f"Final execution configuration:")
        logger.info(f"  - repetitions: {repetitions}")
        logger.info(f"  - timeouts: {timeouts}")
...
def _execute(repetitions: int, timeouts: List[int], tools: List[AbstractTool], memory_file="",
    ...
    logger.info(LoggingManager.LOG_START.format(operation="experiment creation and execution"))
    experiment = Experiment03()
    experiment.execute(
        repetitions=repetitions,
        timeouts=timeouts,
    ...
    logger.info(LoggingManager.LOG_COMPLETE.format(operation="experiment creation and execution"))

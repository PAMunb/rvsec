...
class StateEnricher:
    ...
    def __init__(
        ...
        try:
            ...
            self.logger.info(f"Initialized screen parser: {self.parser.__class__.__name__} with visitor: {visitor_class.__name__}")
        except ImportError:
        ...
        try:
            ...
            self.logger.warning("ScreenshotAnalyzer not available")
    ...
    def enrich_state(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Detect UI patterns
            if StateEntry.SCREEN in state and "components" in state[StateEntry.SCREEN]:
                self._detect_ui_patterns(state)
            
            # Add monitored operations information
            if self.static_data and StateEntry.ACTIVITY in state:
                self._add_monitored_operations(state)
            
            # Add screen description if not present
            if StateEntry.SCREEN_DESCRIPTION not in state and StateEntry.SCREEN in state:
    ...
    def _process_screenshot(self, state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Use screenshot analyzer to extract information
            analysis_result = self.screenshot_analyzer.analyze(screenshot_path)
            
            # Add analysis results to state
            if analysis_result:
                state["screenshot_analysis"] = analysis_result
        except Exception as e:
    ...
    def _detect_ui_patterns(self, state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            screen = state[StateEntry.SCREEN]
    ...
    def _add_monitored_operations(self, state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # package_name = state.get(StateEntry.PACKAGE_NAME, "")
            ...
            for method in actv_class.methods:
                if method.directly_reaches_mop or method.reaches_mop:
                    monitored_ops.append(method.name)
    ...
    def _add_screen_description(self, state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not hasattr(self, 'parser') or self.parser is None:
                ...
                try:
                    from rvandroid.parser.screen.parser_factory import ParserFactory, ParserType
                    self.parser = ParserFactory.create(ParserType.DROIDBOT, DefaultTextVisitor) # TODO alterar visitor dinamicamente (via config)
            ...
            if screen_description:
                ...
                state[StateEntry.AVAILABLE_ACTIONS] = available_action_ids
                
                self.logger.debug(f"Added ScreenDescription with {len(available_action_ids)} available actions")
            else:
            ...
            try:
                screen = state.get(StateEntry.SCREEN, {})
                state[StateEntry.SCREEN_DESCRIPTION] = self._format_screen_elements(screen)
            except Exception:

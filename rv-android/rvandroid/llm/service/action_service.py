...
class LLMActionService:
    ...
    def __init__(
        ...
        self.logger.info("Creating unified PromptFramework")
        ...
        self.event_bus.publish_analysis_event(
            ...
            data={"service": "LLMActionService", "model": self.model.model_name if self.model else "unknown"},
        ...
        self.logger.info(
            ...
            f"model={self.model.model_name if self.model else 'unknown'}, "
            f"{strategy_info}"
        )
            ...
            try:
                ...
                with self.performance_monitor.measure_time("llm_interaction", context):
                    ...
                    response = self.framework.generate_with_llm(
                        strategy_name, enriched_state, prompt_context
                    )
                    
                    # Record prompt metrics if possible
                    generated_prompt = self.framework.generate_prompt(strategy_name, enriched_state, prompt_context)
                    if generated_prompt and len(generated_prompt) >= 2:
                        system_content = generated_prompt[0].get("content", "")
                        user_content = generated_prompt[1].get("content", "")
                        
                        self.performance_monitor.record_metric(
                            name="prompt_length_system",
                            value=len(system_content),
                            unit="chars",
                            context=context
                        )
                        self.performance_monitor.record_metric(
                            name="prompt_length_user",
                            value=len(user_content),
                            unit="chars",
                            context=context
                        )

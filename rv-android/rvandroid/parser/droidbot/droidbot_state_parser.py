from rvandroid.parser.droidbot.screen_visitor import *

# DEPRECATED
class TextScreenVisitor(ScreenVisitor):
    """
    ...
    def __init__(self, static_info: StaticAnalysisData, activity: str):
        """
        Initialize the TextScreenVisitor with static analysis data and activity context.
        ...
        self.logging.debug(f"Initialized TextScreenVisitor for activity: {activity}")
...
def parse(screen_info: dict, static_data: StaticAnalysisData) -> ScreenDescription:
    ...
    visitor = TextScreenVisitor(static_data, activity)
    tree.accept(visitor)

...
ToolConfig(BaseValidatedModel):
  tool_name: str                 # e.g. "droidbot", "rvagent"
  variant: str                   # e.g. "dfs_greedy", "default"
  additional_params: Dict        # Parameter overrides
...
The ErrorHandler MUST register 30+ type-specific handlers at initialization covering the entire exception hierarchy: RVTaskError, RVToolError (and subclasses: ToolNotFoundError, ToolRegistrationError, ToolVariantError, PluginError, RVToolTimeoutError, RVToolExecutionError, RVAndroidToolError), RVExperimentError, RVParsingError, RVPromptError, RVLLMError (and subclasses: RVLLMConnectionError, RVLLMModelError, RVLLMProviderError, RVLLMConfigurationError, RVLLMTemplateError), RVValidationError (and subclasses: CommandValidationError, LogcatValidationError), EventProcessingError, RVCommandTimeoutError, JarNotFoundError, CircuitBreakerOpenError, FileNotFoundError, and generic fallbacks (RVAndroidError, Exception).
...
Task MUST NOT be a Pydantic model (it is a plain Python class). It MUST hold a TaskConfiguration (Pydantic), a TaskResult (Pydantic), an optional App reference, and a LogcatRepository for coverage data. Task MUST generate a UUID if none is provided.

App MUST use Androguard to extract APK metadata and MUST expose two distinct package properties: `package_name` (from manifest, for device operations) and `code_package` (detected via PackageDetector, for static analysis). In approximately 27.5% of APKs these values differ.

StaticAnalysisData MUST aggregate results from three static analysis tools: Classes (from GESDA), Windows (from GESDA), and WindowTransitionGraph (from GATOR). All three fields are required.

LogcatRepository MUST centralize coverage metrics calculation. It MUST only register method calls for methods present in the static analysis data. Coverage metrics (CoverageMetrics) MUST include both raw counts and calculated percentages.

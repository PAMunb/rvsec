...
- [ ] 6.2a **Preserve the pre-change jar before 6.2 overwrites it.** The file 6.2 replaces is today the legacy build — sha256 `386ce08d1846a4088755a8d755e5b70391af3b42add091d231dbcc52aed24e69`, `APE-STEP` present, no heartbeat — and it is not merely *a* pre-change jar: `ape` `docs/20260803_procedimento_worktree_rearch.md` §2 records that same digest as **byte-identical to the jar measured in the E3 decisive run**, which is leg A of this very campaign. It is therefore the only deployed copy of the artifact `rearch-04` 9.1a needs for the two pre-change legs of its throughput gate (INV-SNK-13), and the physical article behind the baseline this change compares against. Copy it to `backup/gh97-prechange-jar/` with its sha256 recorded beside it, and note the path in that task. Do not `git add` the backup (`backup/` is untracked here despite the top-level `CLAUDE.md` describing it as ignored). Losing this file does not fail anything loudly; it silently makes the only measurement of the new per-step write path's wall-clock cost unrunnable.
...
- [ ] 7.2b **Evidence harvested for the `ape` side — recorded, never gating (design D11).** Unlike 7.2a this blocks nothing: `scripts/consolidate.py` and `scripts/verify.py` read traces through `aperv_tool.analysis.trace_ndjson` and never touch heartbeats or `clock_logcat_join`, so none of it can invalidate an outcome this change measures. Letting it block the campaign would give a telemetry-only observation authority over an experiment that does not read it. From the same smoke:
      - **Heartbeats (INV-SNK-14; discharges `rearch-04` 9.1b and `gh94` 4.1).** Grep each arm's `task.result.logcat_file` for tag `ApeRvHb`; record the line count per arm and compare it against that run's `StepRecord` count, with the `s` values agreeing. Write the run identity, the counts and the comparison into `openspec/changes/gh94-ndjson-trace-reader/heartbeat-evidence.md` — that file is `gh94` 4.2's deliverable and stays there. **A disagreement, or no heartbeat line at all, is the finding: report it and leave `gh94` 5.5–5.10 blocked.** Deleting the clock reconstruction on the strength of a mechanism no captured run has demonstrated is what INV-SNK-14 forbids, and a smoke that produced no heartbeat is evidence against the deletion, not an inconvenience to route around.

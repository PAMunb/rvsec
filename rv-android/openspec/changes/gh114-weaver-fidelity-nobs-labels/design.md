...
- **Constructor inline path:** `installTryCatch` with a handler plan that runs the monitor calls and rethrows. `DexWeaver` builds the catch-all `TryCatchSpec` the call requires.
...
- **Lists are not closed:** the producers and consumers are the ones the specification names (decision of 2026-09-15). Other producers that report on their product do not mark, and other `-NOBS-` sites keep `not-observed`.

...
- **INV-CORE-54**: The presence of heartbeat lines in a captured logcat MUST NOT change any value produced by `parse_logcat_file` — not `calculate_metrics()`, not `total_errors`, not `unique_errors`, not any coverage value, and not the diagnostic-event collection. The parser dispatches on the `RVSEC` and `RVSEC-COV` tags alone, so heartbeat lines are inert to it, and that inertness is a permanent test rather than an assumption.

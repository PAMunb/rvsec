...
- G3 after G2's template is proven (first records pass green). G4 after G3 (needs the oracle-defect
...
- [ ] 1.1 R1 `DHGenParameterSpecSpec`: condition → accusing body + `DHGENPARAMETERSPEC-CONSTR-00`
- [ ] 1.2 R2 `KeyPairGeneratorSpec`: `initError2` twin for `initialize(int, SecureRandom)`
- [ ] 1.3 R3 `MessageDigestSpec`: value check on the `d1`/`d3` route
- [ ] 1.4 R4 `CipherOutputStreamSpec`: `ere` — `fl` no longer satisfies the `+`
- [ ] 1.5 R5 `SecretKeySpec`: `SecretKey+.getEncoded()` (verify `+`-owner weaves on dexlib2 first)
- [ ] 1.6 R6 `CipherSpec:177`: splitter → `CipherTransformationNormalizer.alg` + AES_128/AES_256 ≡ AES (D-20.2)
- [ ] 1.7 R7 `CipherTransformationNormalizer.isValid`: admit the 8 expert PBE families (D-20.1)
- [ ] 1.8 R8 `KeyGeneratorSpec:215`: write canonical predicate value (D-20.3)
- [ ] 1.9 R9 `IvParameterSpec`: `len > 0` + accuser on the reachable violated branch
- [ ] 1.10 Optional hygiene: delete the 8 write-only `current*` fields (P3)
- [ ] 1.R Group records pass + trace pairs for R1/R4/R9 + **harness checkpoint 1** (diff vs pre-G1)
...
- [ ] 3.6 `DigestInputStreamSpec.mop` (reads `generatedMessageDigest`; FORBIDDEN `on(boolean)`)
- [ ] 3.7 `DigestOutputStreamSpec.mop` (same family; apply the R4 lesson to its `ere`)
- [ ] 3.R Group records pass (`preparedAlg` gap must show closed) + [GEN] monitor + gates

## 4. G4 — Complex specs + adjudications (after G3) — `tasks/G4-complex-adjudications.md`

- [ ] 4.1 `SSLEngineSpec.mop` (transcribe evident intent over the `cp1` defect row from 0.1)
- [ ] 4.2 `SSLParametersSpec.mop` (3 constructor paths)
- [ ] 4.3 `KeyAgreementSpec.mop` (5 events, `noCallTo[gs3]`, `gs2` defect row from 0.1)
- [ ] 4.4 Adjudicate N/A: `Cookie` (no class), `DSAGenParameterSpec` (API 35+), `PasswordAuthentication` (D-19, ratified: recorded N/A-by-value)
...
- [ ] 7.2 [M1] Ledger assertion, both sides: the five named producer gaps plus `preparedDH` closed (`preparedAlg` after G3; `preparedKeyMaterial` fully after 4.3), and every predicate the set writes read at a consuming site or carrying a recorded impossibility (D-24); coverage matrix shows no rule without a terminal state among the landed tiers; run `/rv-verify` on the touched Python surface
- [ ] 7.3 [M2] **Harness checkpoint 2** (final): differential classification attributed per group; battery green; coverage matrix complete — 49/49 terminal states over three values, `oracle_defect_row` filled by join; **NOBS rate of the G1b sites read over the APK corpus, for the researcher's decision on whether any NOBS branch retires** (D-24)

...
| Performance | Wall-time of `resolveInScene` + bytecode scan + reverse BFS **summed**, generic_new vs jca on the same APK, MUST be ≤ 2× the JCA baseline, each stage also reported on its own — the RISK-005 hard trigger, previously measured by no task. Measured 0.70× on a 45 MB APK, with two stages above 2× and the BFS at 0.07×; INV-ANA-42 carries the table | timed IT + one large-APK pair | 1 |

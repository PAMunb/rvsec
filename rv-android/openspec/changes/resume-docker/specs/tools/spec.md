...
When running outside Docker (`/.dockerenv` does not exist), the command MUST use `--network host` so the sibling container shares the host's network namespace and can reach the emulator at `localhost:5554`. The default Docker bridge network isolates the container's network, preventing emulator access.

...
- [ ] 1.1 **ScreenNode.totalActions initialization** — Add `setTotalActions(int count)` to `ScreenNode.java`. Add guard `if (totalActions > 0)` in `getSaturationRate()`. In `AgentLoop.runIteration()`, call `screenNode.setTotalActions(items.size())` when `visitCount == 1`. Add tests in `ScreenNodeTest.java` verifying saturation returns correct values (0.0–1.0, not always 1.0).
- [ ] 1.2 **SCROLL action generation in 4 directions** — In `ActionSelector.generateCandidateActions()`, add SCROLL action for items where `item.isScrollable()` is true. Generate 4 directions: DOWN (primary, base score same as CLICK), UP, LEFT, RIGHT (secondary, base score = CLICK * 0.5). Coordinates: center of the scrollable container. This ensures horizontal tabs, carousels, and above-viewport content are discoverable. Add tests in `ActionSelectorTest.java` verifying SCROLL actions are generated in all 4 directions for scrollable elements.
...
- [ ] 1.8 Run `mvn test` on rvsmart module to verify all existing + new tests pass
...
- [ ] 3.1 **Reduce default throttle** — In `Config.java`, change default `throttleMs` from 200 to 100. A moderate reduction (not 50ms) avoids UI instability that could increase out-of-app events. Verify properties file loading path correctly overrides this default. Add test in `ConfigTest.java` for default value and properties override.
- [ ] 3.2 **Reduce restart cost** — In `AgentLoop.recoverApp()`, change `forceStop` sleep from 500ms to 200ms and `startApp` sleep from 1500ms to 800ms (total: 1000ms instead of 2000ms).
- [ ] 3.3 **Conditional adaptive wait** — In `AgentLoop.java`, apply `adaptiveWaitMs` only for CLICK and LONG_CLICK actions. Skip adaptive wait for SET_TEXT and SCROLL.
- [ ] 3.4 **Screen state caching** — In `AgentLoop.java`, add `lastScreenState` field. After post-action capture, store the result. At iteration start, reuse `lastScreenState` if no recovery event occurred. Invalidate on crash, out-of-app, or stuck recovery. Depends on task 1.6.
- [ ] 3.5 **Cycle time profiling via RVTRACK** — In `AgentLoop.runIteration()`, add `System.currentTimeMillis()` timestamps around each phase: capture, scoring, execution, wait. Log per-iteration breakdown via RVTRACK (e.g., `RVTRACK CYCLE capture=12ms scoring=3ms exec=45ms wait=100ms total=160ms`). Enables data-driven validation of throttle and future calibration.

> **Checkpoint** (task 3.6): Orchestrator runs `mvn test`.
> **Skill**: Use `/superpowers:verification-before-completion`.

- [ ] 3.6 Run `mvn test` on rvsmart module to verify all tests pass
...
- [ ] 4.1 Run full `mvn test` — all 370+ JUnit 5 tests must pass (existing + new)
- [ ] 4.2 Run rvsmart on a subset of baseline APKs (e.g., 5 APKs) via `source /etc/profile && cd $RVSEC_HOME/rv-android && uv run rv-experiment run --tools rvsmart:mvp --apks-dir results/cli_experiment_20260305_180341_fe33918e/instrumented_apks/ --skip-monitors --skip-instrument --skip-static --timeout 300` — reuse instrumented APKs from post-gh29 experiment to skip pre-processing. Compare method/MOP coverage against baseline (rvsmart: 17.76% method, 25.54% MOP). Measure evt/s (target: ≥10 evt/s sustained) and OOA% (target: < 5%, down from 18.6%).
- [ ] 4.3 Verify SCROLL actions appear in trace output in all 4 directions for apps with scrollable containers
- [ ] 4.4 Verify MopScorer returns non-zero scores in trace output when static analysis data is available
- [ ] 4.5 Verify `getSaturationRate()` returns values < 1.0 in RVTRACK logs for screens with multiple elements
- [ ] 4.6 Verify UI capture count per iteration is 1–2 (grep RVTRACK state lines per iteration)
- [ ] 4.7 Verify OOA detection triggers RESTART, launcher fast-path works, and OOA% is < 5% in trace output
- [ ] 4.8 Verify cycle time profiling appears in RVTRACK output with per-phase breakdown
- [ ] 4.9 Review all changes against P1–P4 principles

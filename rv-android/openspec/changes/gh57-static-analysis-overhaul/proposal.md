...
- `analysis`: existing requirement `Unified Static Analysis — Window Transition Graph, GUI Elements, and Method Reachability (FR04, FR05, FR06)` is modified to (a) decouple `windows[]` from WTG completion, (b) replace the secondary `AndroidCallGraph` with the SPARK CG already available in `Scene.v().getCallGraph()` (gated by a runtime feature flag), and (c) introduce a `schemaVersion` field at the JSON root. Four new requirements are added: `skipWtg` client parameter, widget XML attributes parity with GESDA, programmatic options-menu extraction, and programmatic Spinner items via ArrayAdapter dataflow.

The `tools` domain is not modified at the requirement level. The change to `MopData.java` lives in the external `ape` codebase (Java) and only updates how the existing aperv binary parses the JSON; no Python-level tool capability requirement changes.

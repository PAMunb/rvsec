...
This delta spec documents the consolidation of three separate static analysis tools (GESDA, GATOR, REACH) into a single unified GATOR client. The motivation is eliminating 3 redundant Soot initializations per APK, which cause timeouts in gh26 experiments. After this change, a single GATOR invocation produces one unified JSON file containing windows (from GESDA), transitions (from GATOR WTG), and reachability data (from REACH). The `StaticAnalyzer` invokes one tool instead of three, and a single `UnifiedParser` replaces three separate parsers.
...
- `mop_dir: str` — Path to MOP specification directory. Previously consumed only by REACH via `--mop-dir` CLI flag. Now passed to the unified GATOR client via `-clientParam mopDir=<path>`.
- `unified_timeout: float` — Timeout in seconds for the unified tool (default: 600.0). Replaces the implicit no-timeout behavior of the previous three-tool pipeline. Passed both as `Command.timeout` (Python process-level kill) and `--timeout` (GATOR's internal timeout).
- `unified_jar: str` — Path to the unified client fat JAR (`lib/unified/rvsec-unified-client.jar`). Replaces `gesda_jar`, `gator_dir`, and `reach_jar`.
...
- `StaticAnalysisResult` — **CHANGED**: Previously contained `gesda_file: str`, `gator_file: str`, `reach_file: str`. Now contains `unified_file: str` and `timed_out: bool`.

### Side-Effects (changed)

- **File System**: Creates `{app_name}.json` unified output file in the output directory. Replaces the previous three files (`.gesda`, `.wtg`, `.reach`).

### Error (unchanged)

- `StaticAnalysisException` — Raised when the unified tool returns a non-zero exit code. Contains tool name ("UNIFIED"), exit code, and stderr output.
- `RVCommandTimeoutError` — Raised when the unified tool exceeds `unified_timeout`. The `Command` class kills the process tree via `kill_process_tree()`.
- `ConfigurationError` — Raised when `unified_jar` path does not exist or MOP directory is missing.

## Invariants

### REMOVED Invariants

- **INV-ANA-01** (REMOVED): "GESDA analysis MUST complete before REACH analysis begins" — No longer applicable. There is one tool, not a pipeline. The unified GATOR client performs all extraction (windows, transitions, reachability) in a single invocation. **Reason**: The ordering constraint existed because REACH consumed GESDA output as input. In the unified client, all data is available in memory from the same GATOR analysis pass.

### MODIFIED Invariants

- **INV-ANA-02**: The `UnifiedParser` MUST apply `SignatureNormalizer` to all class names and method signatures before storing them in domain models. The normalization converts inner class dot notation (`OuterClass.InnerClass`) to dollar notation (`OuterClass$InnerClass`) using Java naming convention heuristics. Previously, this invariant applied to three separate parsers (GatorParser, GesdaParser, ReachParser). Now it applies to a single `UnifiedParser` which normalizes class names in all three JSON sections (`windows`, `transitions`, `reachability`).

- **INV-ANA-03**: The `UnifiedParser` MUST receive `code_package` (from `App.code_package`, detected by `PackageDetector`) for class filtering, NOT `package_name` (from AndroidManifest.xml). The parser MUST filter classes in the `reachability` section and windows in the `windows` section by verifying that class names contain the `code_package` string. Previously, this filtering was performed independently by each of the three parsers.

- **INV-ANA-06**: The `UnifiedParser` MUST NOT propagate exceptions to callers. On parse failure of any section (`windows`, `transitions`, `reachability`), it MUST log the error and return empty domain objects for that section: `Windows()` for window parsing failures, `WindowTransitionGraph()` for transition parsing failures, `Classes()` for reachability parsing failures. Each section is parsed independently — a failure in one section MUST NOT prevent parsing of other sections. Previously, this graceful degradation was implemented per-parser; now it is per-section within a single parser.

- **INV-ANA-11**: The `StaticAnalyzer` MUST implement intelligent caching: if the unified `.json` output file already exists, tool execution MUST be skipped. A `CommandResult(0, b"", b"")` MUST be returned for cached results. An info log with `execution_status='cached'` MUST be recorded. Previously, caching was checked independently for each of the three output files (`.gesda`, `.wtg`, `.reach`).

## MODIFIED Requirements

### Requirement: Unified Static Analysis — Window Transition Graph, GUI Elements, and Method Reachability (FR04, FR05, FR06)

The system MUST run a unified GATOR client to produce a single JSON output file containing three data sections written in priority order: (1) method reachability relative to MOP specifications (coverage denominator), (2) window and widget inventory with event listeners, and (3) window transition graph. The section ordering is deliberate: `reachability` is written first because it defines the method universe used as the coverage denominator. On timeout, partial JSON preserves the most critical data first.

The unified tool is a GATOR client (`RvsecUnifiedClient`) that implements the `GUIAnalysisClient` interface. GATOR initializes Soot once, builds its constraint graph and fixpoint analysis, and then invokes the client's `run(GUIAnalysisOutput output)` method. Inside this method, the client writes each JSON section incrementally with explicit flush, so that a timeout or crash after any section produces a parseable partial file. The execution order inside `run()`:
...
The unified JSON output is parsed by `UnifiedParser` into the `StaticAnalysisData` domain model (Classes, Windows, WindowTransitionGraph). Downstream consumers (rv-agent, rv-coverage, rv-platform) receive this data structure.
...
**Key components**: `StaticAnalyzer`, `UnifiedParser`, `RVStaticAnalysisConfig`

#### Scenario: Successful unified analysis with valid APK

- **WHEN** `StaticAnalyzer._run_unified()` is called with a valid APK path and the unified JAR exists at `lib/unified/rvsec-unified-client.jar`
- **THEN** the system MUST execute the GATOR Python script with arguments: `python gator a -p <apk_path> --client-jar <unified_jar> --out <output_file> -client RvsecUnifiedClient -clientParam mopDir=<mop_dir> --timeout <timeout>`
- **AND** the resulting `.json` file MUST be parseable by `UnifiedParser` into a `StaticAnalysisData` containing non-empty `Classes`, `Windows`, and `WindowTransitionGraph`

#### Scenario: Unified JSON parsing — windows section

- **WHEN** `UnifiedParser._parse_windows()` processes the `windows` array from the unified JSON
...
#### Scenario: Unified JSON parsing — transitions section

- **WHEN** `UnifiedParser._parse_transitions()` processes the `transitions` array from the unified JSON
...
#### Scenario: Unified JSON parsing — reachability section

- **WHEN** `UnifiedParser._parse_classes()` processes the `reachability` array from the unified JSON
...
#### Scenario: Unified JSON parsing with inner class normalization

- **WHEN** `UnifiedParser` encounters a class name like `com.example.OuterActivity.InnerFragment` in any section
...
#### Scenario: Unified output file does not exist

- **WHEN** `UnifiedParser.parse_file()` is called with a non-existent file path
...
- **WHEN** the unified JSON file exists but the `reachability` section contains malformed data
- **THEN** `UnifiedParser._parse_classes()` MUST catch the exception, log an error, and return empty `Classes()`
...
#### Scenario: Unified analysis result is cached
...
#### Scenario: Unified analysis timeout

- **WHEN** the unified tool execution exceeds `unified_timeout` (default: 600 seconds)
...
- **AND** `StaticAnalysisResult.unified_file` MUST be set to the expected output path (which may not exist)
...
#### Scenario: Unified output equivalence to previous 3-tool pipeline

- **WHEN** the unified tool analyzes `cryptoapp.apk` and its output is compared against saved baseline from the previous 3-tool pipeline (GESDA + GATOR + REACH)
...
- **WHEN** `CoverageTracker` or `CoverageAnalyzer` initializes with `StaticAnalysisData` containing `Classes` parsed from the unified JSON's `reachability` section
...
- **AND** this behavior MUST be identical to the previous REACH-based initialization

## REMOVED Requirements

### Requirement: GATOR Analysis - Window Transition Graph (FR04)

**Reason**: Consolidated into the unified static analysis requirement above. The WTG extraction logic is now performed inside `RvsecUnifiedClient` as part of a single GATOR invocation, not as a standalone tool invocation. The `GatorParser` is deleted (P3: no backward compatibility).

### Requirement: GESDA Analysis - GUI Element Extraction (FR05)

**Reason**: Consolidated into the unified static analysis requirement above. Window and widget extraction is now performed by the unified GATOR client using GATOR's internal APIs (which provide the same data as GESDA's intra-procedural pattern matching, plus `inputType` and `entries` extracted from decoded layout XMLs). The `GesdaParser` is deleted (P3).

### Requirement: REACH Analysis - Method Reachability (FR06)

**Reason**: Consolidated into the unified static analysis requirement above. Reachability analysis is now performed inside the unified GATOR client using `Scene.v().getCallGraph()` + JGraphT, without the `cg all-reachable` misconfiguration that caused 10-100x performance degradation. The `ReachParser` is deleted (P3).

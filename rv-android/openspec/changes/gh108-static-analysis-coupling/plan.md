...
| `jca_android` | 23 | 119 | 67 | 22 | negligible: one pair of difference, `(MessageDigest, reset)` |
...
- [ ] A test asserts the `mop_dir` produced by `get_static_analysis_config()` for all four values of `specification_set` (`jca`, `jca_android`, `generic`, `custom` with `custom_specs_dir`), and that an unsupported value still raises `ConfigurationError`
- [ ] The D2 side effect is measured, not assumed: a `--specification-set generic` run on `apks_examples/cryptoapp.apk` with static analysis on shows `mopDir=.../generic` in the GATOR argv and 296 resolved signatures, against 120 before the change
- [ ] A `jca` run is unchanged end to end — same `mopDir`, same 120 signatures — confirming the frozen ruler is untouched
- [ ] With Phase 2 reporting failure, the CLI prints no success message and `echo $?` is non-zero; with Phase 2 succeeding, it exits 0
- [ ] The orchestrator table in Section 1 is re-checked against the tree: gh104 indifferent, `restart_exited.sh` reacting only to 137, and `run_smoke.sh` now failing on a partial Phase-2 failure
- [ ] `env -u ANDROID_SDK_HOME ANDROID_HOME=<sdk> python3 lib/gator/gator a --sdk ... ` and the same invocation with no `--sdk` both get past `gator:64`; a setup exporting only `ANDROID_SDK_HOME` still resolves the same root as before
- [ ] `grep -rn "apks" modules/rv-experiment/src/rv_experiment/experiment/workflow/execution_controller.py` shows no reference to the removed parameter, and `grep -rn -- "--sdkpath" docs/` finds nothing in the two live documents
- [ ] `uv run pytest --import-mode=importlib -o "addopts=" tests/` is green in `modules/rv-experiment` and `modules/rv-static-analysis`
- [ ] `/rv-qa-lint-fix` and `/rv-verify` clean on both touched Python modules

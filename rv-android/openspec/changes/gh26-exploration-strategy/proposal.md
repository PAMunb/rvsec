...
- `RVAgentConfig` adds 11 new configuration fields (calibration parameters, including `reward_score_weight`, `max_backtrack_hops`, `coverage_density_weight`, and `max_coverage_hops`)
...
- **Downstream**: gh9's `parameter_space.py` must be updated with 11 new parameters (+ 2 from gh18 = 13 total new params, bringing the total from 24 to 37) before the calibration execution campaign starts.

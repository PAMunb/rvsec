...
- **INV-ANA-16**: The GATOR `Main.java` MUST configure Soot with the following defensive options on the unified `-cgAlgorithm` code path (covering all of `cha`, `rta`, `vta`, `spark`): `-p jb.sils enabled:false`, `-p jb.dae enabled:false`, `-no-bodies-for-excluded`, `-exclude kotlin.`, `-exclude kotlinx.`, `-exclude androidx.compose.`. Programmatically: `Options.v().set_ignore_resolution_errors(true)` and `Options.v().set_throw_analysis(Options.throw_analysis_dalvik)`. Disabling `jb.sils` and `jb.dae` is a documented workaround for Jimple body typing failures (soot-oss/soot#1641, #1975) — the issues themselves concern `use-original-names` interference, but disabling these sub-phases avoids the `LocalSplitter`/`TypeResolver` interaction that triggers `InternalTypingException`. The excludes (`kotlin.`, `kotlinx.`, `androidx.compose.`) and `no_bodies_for_excluded` prevent jimplification of Kotlin stdlib and Compose runtime classes (the dominant crash category — ~71% of failing APKs are Kotlin+Compose). The remaining options (`ignore_resolution_errors`, `throw_analysis_dalvik`) are general robustness practices that do NOT prevent the `InternalTypingException` specifically.

- **INV-ANA-17**: The GATOR GUI analysis pipeline MUST handle individual failures gracefully at multiple points:
  - `Flowgraph.processApplicationClasses()` MUST wrap `currentMethod.retrieveActiveBody()` (line 274) in try-catch with `Logger.warn()` and `continue`.
  - `Flowgraph.processApplicationClasses()` MUST replace `throw new RuntimeException(e)` (line 343) with `Logger.warn()` and `continue`.
  - `Flowgraph.java` and `FlowgraphRebuilder.java` MUST null-check `jimpleUtil.receiver(ie)` before calling `.getType()` (returns null for excluded-package virtual calls).
  - `IntentAnalysis.resolveStartActivityIntentContent()` MUST use a visited set to prevent infinite re-processing in the fixpoint loop when `intentContent.get(allocNode)` is null.
  - `RvsecAnalysisClient.run()` MUST write JSON with reachability data BEFORE attempting WTG construction. If WTG completes, the JSON is rewritten with full data (windows + transitions). This guarantees that external timeout (Python subprocess kill) preserves reachability.
  All log messages MUST use `Logger.warn()` (not `verb()` — skipping methods/statements is a degraded-analysis signal, not debug trace).

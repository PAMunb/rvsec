...
- **`JsonReportWriter` writes `components` BEFORE heavy sections (post-9.1-smoke gap, 2026-05-29)** — historical order put `components` (manifest-derived, milliseconds to serialize) *after* `transitions` (WTG, the dominant timeout source). The 9.1 5-APK smoke surfaced two R8/Compose APKs (`com.acszo.redomi.repo_10401.apk`, `com.aisleron_20.apk`) that exited 206 (timeout) at ~902 s — both JSONs survived complete by luck, but a heavier WTG would have silently dropped `components`, breaking the windows→activity-class lookup every downstream consumer depends on. Reorder is a 4-line block move; parity gates are byte-order-agnostic per `design.md` §"Why set equality, not byte equality". See `design.md` D14 and `tasks.md` Group 13.
- **`components` carries the data needed to manually trigger components from aperv (post-9.1-smoke gap, 2026-05-29)** — pre-D15 `writeComponentEntry` emitted only `actions[]/categories[]` per intent filter plus `exported`, so an explorer trying to launch a component reachable only via deep link (`Intent(VIEW, "myapp://path")`) or MIME type (`ACTION_SEND` with `image/*`) had no way to construct the launching Intent. Extend to emit `intentFilters[].data.{schemes, hosts, ports, paths, pathPrefixes, pathPatterns, mimeTypes}` plus per-component `permission` (and provider-specific `readPermission`/`writePermission`). Soot already carries the data internally (`IntentFilter.mDataSchemes/mDataTypes/mDataAuthorities/mDataPaths`); patch exposes via getters + serializes. Domain side: extend `IntentFilter` + `ComponentInfo` Pydantic models, introduce `ProviderComponentInfo` subclass for provider-specific permissions. See `design.md` D15 and `tasks.md` Group 14.

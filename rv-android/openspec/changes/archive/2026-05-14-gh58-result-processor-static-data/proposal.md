...
- **BREAKING** (CSV schema, positional readers break): `summary.csv` grows from 8 columns to 12 columns and reorders to `apk, rep, timeout, tool, cov_act, cov_class, cov_method, cov_reachable, cov_reaches_mop, cov_directly_reaches_mop, mop_errors_total, mop_errors_unique`. The `errors` column (position 7 in the old schema) is renamed to `mop_errors_total` and a sibling `mop_errors_unique` is added. The legacy `cov_rv_method` column is dropped from `summary.csv` because, with row-constant final values, it would alias `cov_reaches_mop` (same source `mop_method_coverage`). `cov_class` is inserted at position 5, so any downstream reader using positional indexing past column 4 sees shifted semantics. Header-based readers (pandas default) get the new columns transparently. The new column order matches the schema already produced by `scripts/regenerate_results/` (validated against 18,267 logcats), so the platform output and the offline regen tooling converge on a single schema.
...
- `platform`: data contracts for `coverage.csv` and `summary.csv` extended (extended schema); resume-path invariant added requiring `static_data` to be present when reconstructing a repository from logcat; Branch 2 fallback paths in CSV writers removed.

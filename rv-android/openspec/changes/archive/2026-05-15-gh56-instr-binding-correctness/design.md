...
- Unit tests in `DexWeaverConstructorAdviceTest` (gh56 additions) assert register-to-type correspondence on the canonical bug shape and on the `IvParameterSpec.<init>` parity case.
...
- **Re-running any portion of the 2026-05-08 campaign** (sampled or full). Cross-APK ecosystem measurement is a separate experiment with its own change, dataset, and infrastructure decisions — gh56 ships with the cryptoapp smoke + unit-test evidence as the acceptance bar. No cloud/GCP execution is in scope for this change.
...
| Cross-APK regression beyond cryptoapp not surfaced by the smoke gate | A `VerifyError` family that depends on app-specific bytecode shapes (R8-obfuscated `<init>`, Tink/Okio internals, `args(name)` unresolved tail) could slip past the cryptoapp smoke | Acceptable residual for gh56 — broader ecosystem validation is a separate experiment, not in scope here. The unit tests (`DexWeaverConstructorAdviceTest` SecretKeySpec + IvParameterSpec cases) plus the `plansSkippedUnresolvedBinding` counter surfaced in `instrument_results.json` give early-warning signals during any future instrumentation run; that counter is the canonical regression trip-wire. |
...
| Integration (no broader sample) | Cross-APK validation beyond cryptoapp | **Out of scope for gh56** — a separate experiment with its own change is responsible for ecosystem-level validation. The `plansSkippedUnresolvedBinding` counter exposed in `instrument_results.json` is the regression trip-wire for any future instrumentation run. | — |
...
- *Should we re-run any portion of the 2026-05-08 campaign as part of gh56*? **No** — gh56 is the instrumentation fix only. Cross-APK validation is a separate experiment with its own change and is not a precondition for closing gh56.

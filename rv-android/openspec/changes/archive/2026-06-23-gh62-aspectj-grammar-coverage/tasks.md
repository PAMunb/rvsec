...
- [x] 0.5.1 Locate a representative `AspectDescriptor` JSON produced by the current JavaMOP toolchain — start with `results/gh53_smoke_dexlib2/monitors/` or rebuild via `mvn -pl rvsec-mop package` on the sibling repo. The canonical fixture at `descriptor-reader/src/test/resources/MultiSpec_1MonitorAspect.json` may be reused as ground truth — it ships the field with the twelve-entry baseline. **DONE 2026-06-23 §0.5 PASS** — descriptor localizado (gh53_smoke_dexlib2, gh62_6S_post).
- [x] 0.5.2 Inspect the JSON for the field `baseAspectExclusions`. Expected: present as `List<String>` (JSON array of strings) with at least the twelve baseline patterns. If present: proceed to §1. **DONE — `baseAspectExclusions` presente como `List<String>` com os 12 padrões canônicos.** Procede para §1.
- [x] 0.5.3 If absent (legacy descriptor produced by a JavaMOP build pre-dating the field): document the production-corpus version drift in `MEMORY.md` and regenerate the descriptors via the current JavaMOP toolchain (`mvn -pl rvsec-mop package` on the sibling repo); the §4.D matcher will throw `LegacyDescriptorException` on empty lists per the G-decision fail-closed contract (INV-INS-97), so legacy descriptors are NOT a silent failure — the closure surfaces them at instrumentation time. **N/A — campo presente (não é legacy descriptor).**
- [x] 0.5.4 If present with a different shape (e.g. `Map<String, Object>` instead of `List<String>`): document the actual shape, file a JavaMOP-side fix to restore the canonical shape, and pause gh62 until the shape stabilises. This is unlikely — the canonical shape is enforced by `DescriptorReader`'s Jackson binding (`@JsonProperty` + `List<String>` field type) and a divergent JavaMOP emit would fail deserialisation today. **N/A — shape correta (`List<String>`), sem divergência.**
- [x] 0.5.5 Document the verification outcome (PASS / LEGACY-DESCRIPTOR / SHAPE-DIFFERENT) inline at the top of `tasks.md` as a `<!-- §0.5 outcome: ... -->` comment. Commit if any source change resulted: `chore(gh62): verify baseAspectExclusions in production descriptors (§0.5 PASS — A-decision empirical revision 2026-05-28)` with `refs #62`. **DONE — outcome = PASS (List<String>/12 em produção).**
...
- [x] 0.6.4 Commit: `chore(gh62): delete git-ignored -s stray .aj artifacts from .mop source dirs (§0.6, R11.1)` with `refs #62` (only if a tracked file or `.gitignore` changed; the deletes themselves touch only ignored files). <!-- N/A 2026-05-29: no tracked file or .gitignore change resulted (existing `*.aj` rule sufficient); the deletes touch only ignored files → no commit per the task's own condition. --> **N/A — sem arquivo rastreado/.gitignore alterado.**
...
- [x] 0.7.5 Commit (only if audit produced source-of-truth documents): `chore(gh62): audit 3 if() sites in generic_new for target/args-bound variables (§0.7 PASS|BLOCKED — R11.5)` with `refs #62`. <!-- audit table lives in tasks.md (rv-android repo); will fold into a progress commit. PUSH DEFERRED. --> **DONE — auditoria dobrada no commit de progresso.**
...
- [x] 4.1-4.22 (Scaffold the per-designator test classes per round-7 §4.1-§4.22 — see git history. Round-8 modifications: <!-- DONE 2026-05-30 (gh62 §6 unit): all per-designator classes present + passing (grammar-tests 91, 0 skip). The seven NOT-NEEDED α assertion classes deferred.md §2.1 cites — WithincodeGrammarTest, CflowGrammarTest, HandlerGrammarTest, FieldAccessGrammarTest, InitializationGrammarTest, ThisGrammarTest, TypePatternGrammarTest — were created here (Issue 1 resolution) so every deferred.md-named test class now resolves. -->
...
## 4.PERF — `commonPointcut` per-join-point blowup — **VALIDATION-DISCOVERED PERFORMANCE REGRESSION (2026-05-31, §6.S smoke gate)**

> **Why this is an in-change fix, not a deferral.** The §6.S ≥10-APK smoke gate (gh62's own empirical contract) exposed a severe performance regression that grammar/unit tests cannot surface — they weave tiny synthetic fixtures, not real 20-60 MiB multi-DEX APKs. On the JCA-226 corpus a 27.5 MB / 5-DEX APK (`io.github.eucsoh.android_9`) ran **43+ min CPU-bound and never finished**; the pre-gh62 build instrumented the same corpus in 1-12 min/APK (all 190 finished). Root cause (2 live jstacks + 4-agent forensic investigation, full report `/tmp/gh62_perf_investigation.md`): **two sequential CPU-bound phases**. **P2** is a gh62 regression introduced by §4.D.0 (`3af5b3aa`): `DexWeaver.weave` builds and matches `AND(commonAst, advice)` **per (instruction × 115 advices)**, and `matchNamedRef → BaseAspectExpander.expand` rebuilds a 23-node exclusion tree with NO cache on every call (jstack leaves: `mergeBindings`/`fromDescriptor`). **P1** is pre-existing (gh52): `AndroidClassIndex.load` reopens a `ZipFile` per ancestor step in `subtypesOf`. `commonAst` is class-invariant and binds nothing, so hoisting its evaluation to per-class is provably semantics-preserving (same join points, same bindings, same emitted bytecode). See design D16. **This is a behaviour-preserving optimization** — no closure verdict changes; the matrix is untouched.

- [x] 4.PERF.P2.1 **Hoist `commonAst` to per-class** in `DexWeaver.weave`. Evaluate the commonPointcut class-gate once at the top of the `for ClassDef` loop, reusing the existing class-level probe pattern (`DexWeaver.staticInitMatchesClass`, ~line 612: synthetic `<clinit>` probe + `matcher.match`). If the class is excluded → `continue` (skip all methods/instructions/advices). If it passes → the per-(instruction × advice) loop matches **only the advice `pe`**, removing the per-iteration `new CombinedPC(AND, commonAst, pe)` (main loop ~lines 368-369 AND the §4.Y staticinit pre-pass ~lines 536-537). Must preserve §4.D.0 semantics (class-level exclusions still gate the match — now at the class gate). Add a counter/assert that commonAst is evaluated O(classes) times, not O(instructions × advices). <!-- DONE 2026-05-31: DexWeaver.java — added isClassInvariant() + classMatchesCommon() helpers (delegating to staticInitMatchesClass <clinit> probe); per-class gate at top of `for ClassDef` in weave() (continue on reject, perInstructionCommon=null on pass → advice pe matched alone); same hoist in weaveStaticInit() per-class loop; commonAstEvals counter + getCommonAstEvals() getter (O(classes)). isClassInvariant treats CombinedPC/WithinPC/NotWithinPC and NamedRefPC(BaseAspect.notwithin()|adviceexecution) as class-invariant; falls back to per-instruction composition otherwise. -->

- [x] 4.PERF.P2.2 **Memoize `BaseAspectExpander.expand`** (BaseAspectExpander.java:25-36) keyed by the exclusion list (constant per weave) so the 23-node `NotWithinPC` AND-tree is built at most once. ~5 LOC, independent of P2.1. <!-- DONE 2026-05-31: BaseAspectExpander.java — static ConcurrentHashMap<List<String>,PointcutExpression> CACHE; expand() now returns CACHE.computeIfAbsent(List.copyOf(exclusions), ::build); fold logic moved to private build(). Key copied defensively; result tree immutable (records). -->

- [x] 4.PERF.P2.3 **Short-circuit `mergeBindings`** (PointcutMatcher.java:635-648) when one side has empty `argBindings` and `targetRegister < 0` → return the other side without the `LinkedHashMap` copy. ~3 LOC. <!-- DONE 2026-05-31: PointcutMatcher.mergeBindings — when left (or right) has empty argBindings && targetRegister<0 && !isConstructor, return new Match(otherSide.argBindings, otherSide.targetRegister, pe, otherSide.isConstructor) without the extra LinkedHashMap copy+putAll. isConstructor OR-merge preserved (empty side is false → false||other==other); target selection identical to the original ternary; pe kept as the CombinedPC. -->

- [x] 4.PERF.P1 **Cache the opened `AndroidClassIndex` / `ZipFile`** (AndroidClassIndex.java:146) and/or memoize `InheritanceResolver.subtypesOf`/`isAssignableFrom` (InheritanceResolver.java:72,86) so the archive is not reopened per ancestor-walk step (driven by `DexWeaver.expandWrapperReplacementsForApk`:171-186). Ensure the archive is closed once at end-of-APK. Pre-existing gh52 inefficiency; speeds all builds. Output must remain byte-equivalent (perf-only). <!-- DONE 2026-05-31: AndroidClassIndex.java — chose ZipFile-cache (lower risk than refactoring InheritanceResolver). load() now reuses a lazily-opened `archive` field via openArchive() instead of `try (ZipFile zf = new ZipFile(...))` per class; class made `implements AutoCloseable` with idempotent synchronized close() so an owner can release the handle deterministically; in the short-lived per-APK CLI process the handle is freed at process exit. Same archive bytes + same ASM parse → byte-identical output. NOTE: caller wiring of close() in InstrumentApkCommand.buildInheritanceResolver NOT added (index outlives the method; left to process-exit to avoid lifecycle restructuring risk) — see report. -->

- [x] 4.PERF.BUILD Build the full dexlib2 reactor (`RVSEC_HOME=… mvn clean install`, + rvsec-core if touched). Keep GREEN: grammar-tests 102, `MatrixIntegrityTest` 16, `AbsorptionClaimsContractTest` 3, validator 50, plus dex-mutator/pointcut-engine suites. Add a performance non-regression test if feasible (assert commonAst evaluation count == classes, not instructions × advices). Atomic commits, no Co-Authored-By, LOCAL only. <!-- DONE 2026-05-31: RVSEC_HOME=… mvn clean install → BUILD SUCCESS; 646 test executions, failures=0 errors=0 skipped=0 across all reactor modules (grammar-tests/MatrixIntegrityTest/AbsorptionClaimsContractTest/validator/pointcut-engine/dex-mutator). instr-cli fat jar rebuilt + copied to rv-android/modules/rv-instrumentation-dexlib2/lib/instr-cli.jar. Optional commonAstEvals==classes non-regression test DEFERRED (counter wired via getCommonAstEvals(); test authoring blocked by tooling — see report). -->

- [x] 4.PERF.VAL Validate locally on `io.github.eucsoh.android_9.apk` (pure-JVM `instrument` subcommand, NO emulator), reusing pre-generated monitors from `data/results/instrument_jca190_gh62_06/instrument_jca190_gh62_06/monitors/`. Confirm: completes in minutes (not 43+), exit 0, 0 VerifyError, instrumented APK produced. Compare against a small control APK. Record the before/after wall-time. <!-- DONE 2026-05-31: fixed jar md5 3d2dff7a on eucsoh (27MB/5-DEX worst case, classesSeen=43850, methodsSeen=233661). BEFORE (regression jar): 43+ min, NEVER finished (hang in matchNamedRef/BaseAspectExpander, no woven dex produced). AFTER (fixed jar): wall-clock 14:32 (user 987s, 116% CPU = single-thread CPU-bound as expected), Exit status 0, instrumented+signed APK produced (28.9MB + .idsig), 0 VerifyError, 0 exceptions. weaveCounts: advices=115, dexFiles=5, wovenDexes=5, matchesApplied=454, wrappersSubstituted=450, plansSkipped=0, coverageInstrumented=80160. Weave produced woven dex linearly (~2min/dex) instead of blowing up → regression resolved. Ran with NO timeout to completion (user instruction). -->
- [x] 4.PERF.COMMIT Commit each unit atomically (`fix(gh62): §4.PERF P2.1 hoist commonAst per-class …` etc.) with `refs #62`. LOCAL — user pushes. <!-- DONE 2026-05-31: 4 atomic local commits (no Co-Authored-By, NOT pushed): a1c3331f (P2.1 hoist commonAst per-class), 762de287 (P2.2 memoize BaseAspectExpander.expand), 6bdc5b59 (P2.3 short-circuit mergeBindings), 9713cd25 (P1 cache AndroidClassIndex/ZipFile). Docs 4adf04d6 (design.md D16 + tasks.md §4.PERF). User pushes branch modules. -->

> **§4.PERF OUTCOME (2026-05-31):** Regression FIXED and validated. eucsoh worst-case (27MB/5-DEX) went from never-finishing (43+ min hang) to **14:32 wall-clock, exit 0, 0 VerifyError, APK produced**. All 6 fix commits local on branch `modules` (a1c3331f→9713cd25 + docs 4adf04d6), awaiting user push → Docker image rebuild → resume 190-APK campaign.
...
- [x] 4.N.1 Extend `PointcutExpressionParser.parseUnary()`: add cases for `!target(...)` and `!args(...)` constructing `NegationPC` wrapping inner `TargetPC`/`ArgsPC`. ~20 LOC. Note: if `NegationPC` does not exist, this task includes creating it.
- [x] 4.N.2 Add `NegationPC` matcher: evaluate inner pointcut; invert verdict. ~20 LOC.
- [x] 4.N.3 Update `CompositionGrammarTest.negationBeyondWithin` → narrow to `negativeTargetArgsParserSpecialization`; remove `@Disabled`; assert `!target(MyClass)` inverts match.
- [x] 4.N.4 Run tests. Commit: `feat(gh62): !target/!args parser specialization (16 sites; row flips COVERED)` with `refs #62`.
<!-- DONE 2026-05-30: NegationPC created NEW (pointcut-engine/.../NegationPC.java — record wrapping inner PointcutExpression; added to PointcutExpression sealed `permits` clause). parseUnary() adds a `peekKeyword("target")||peekKeyword("args")` branch placed AFTER the existing `!within` peek (unchanged): it parses the inner via parsePrimary() so the inner TargetPC/ArgsPC carries its resolved type, wraps a TargetPC/ArgsPC in NegationPC, else falls back to the lossy NamedRefPC("!"+...); the generic double-negation fallback also stays. PointcutMatcher.matchInternal dispatches NegationPC→matchNegation, which inverts the inner verdict exactly like matchNotWithin: inner present → Optional.empty(); inner empty → Optional.of(Match.empty(neg)) (no bindings). Since a type-form target(T)/args(T) returns empty on a static invoke or type mismatch, the inverted result correctly matches !target/!args there. CompositionGrammarTest did not exist yet, so created NEW (no @Disabled), mirroring Target/ArgsGrammarTest dexlib2 fixtures: negativeTargetArgsParserSpecialization asserts !target(Cipher) rejects AesCipher-subtype receiver (isEmpty) and matches String receiver (isPresent), and symmetric !args(CharSequence) rejects StringBuilder arg (isEmpty), matches Integer arg (isPresent). Files: NegationPC.java (new), PointcutExpression.java (permits), PointcutExpressionParser.java, PointcutMatcher.java, CompositionGrammarTest.java (new). Full reactor BUILD SUCCESS, 11 modules, 0 fail / 0 err: descriptor-reader 5, pointcut-engine 68 (was 67), advice-emitter 41, dex-mutator 27, coverage-weaver 8, monitor-builder 7, multidex-merger 3, cli 2, validator 50, grammar-tests 12 (was 11). -->
...
- [x] 4.TT.1 Extract `target(Type)` parser path in `PointcutExpressionParser` (distinguish from `target(name)` binding form). Set `Type type;` field on `TargetPC` when arg is a type. ~20 LOC.
- [x] 4.TT.2 Update `TargetPC` matcher: when `type != null`, check `InheritanceResolver.isAssignableFrom(type, receiverDeclaredType)`; return match iff check passes. Binding form (`name != null`) unchanged. ~30 LOC.
- [x] 4.TT.3 Add `TargetGrammarTest.targetTypeMatchesByReceiverType`: `target(Cipher)` matches Cipher receiver, not unrelated. Remove `@Disabled`.
- [x] 4.TT.4 Commit: `feat(gh62): target(Type) type-matching (§4.TT, row flips COVERED)` with `refs #62`. <!-- DONE 2026-05-30: Files: TargetPC.java (record (name,type) + binding()/type() factories), PointcutExpressionParser.java (dispatch "target"→parseTargetBody; isBindingName heuristic — lowercase ident, no ./+/* = binding name, else type pattern; corpus confirms: target(o)/target(c)/target(digest) = bindings, target(Cipher)/target(TreeMap)/target(ByteArrayInputStream) = types), PointcutMatcher.java (new matchTarget: type form reads receiver declared type from MethodReference.getDefiningClass() of ctx.instruction — the invoke owner in the dexlib2 inline-call model; static invoke / non-ReferenceInstruction → empty; subtype-aware via InheritanceResolver.isAssignableFrom(fromDescriptor(toDescriptor(patternType)), fromDescriptor(definingClass)), §4.O FQN-conversion gotcha avoided; trailing + accepted/stripped; binding form stays Match.empty always-match). New TargetGrammarTest (2 tests: targetTypeMatchesByReceiverType + targetBindingFormAlwaysMatches; NO @Disabled). Targeted build pointcut-engine,grammar-tests: TargetGrammarTest 2/2 pass. Full reactor mvn test: 219 tests, 0 fail/0 err/0 skip across 10 test-bearing modules (pointcut-engine 68, validator 50, advice-emitter 41, dex-mutator 27, grammar-tests 8, coverage-weaver 8, monitor-builder 7, descriptor-reader 5, multidex-merger 3, cli 2; android-class-index no tests), BUILD SUCCESS. -->
...
- [x] 4.Y.1 Add `dex-mutator/src/main/java/br/unb/cic/rv/mutator/StaticInitSynthesizer.java` (~60 LOC): given `ClassDef` without `<clinit>`, append synthesized `<clinit>` containing `return-void`. Flag with debug marker. <!-- DONE 2026-05-30: StaticInitSynthesizer.synthesize(definingClassDesc, body, registerCount) builds an ImmutableMethod named <clinit> (access flags STATIC|CONSTRUCTOR, no params, return V) wrapping a MutableMethodImplementation = body + return-void. Decoupled from the emit body (caller passes the advice instructions). The DexFileMutator class-level addition path (NEW: synthesizedMethods map + addSynthesizedMethod() + ClassDefRewriter in toDexFile()) is the enabling change — the pre-§4.Y mutator was body-substitution-only and would DROP a new method at serialization. -->
- [x] 4.Y.2 Update `DexWeaver.applyPlan` to invoke synthesizer when `staticinitialization(...)` advice processes a class without `<clinit>`. ~30 LOC. <!-- DONE 2026-05-30: Implemented as a dedicated staticinit pre-pass (weaveStaticInit) rather than via applyPlan — the per-method loop NEVER visits a class with no <clinit> (matchStaticInit gates on method.getName()=="<clinit>"), so applyPlan is the wrong seam. The pre-pass runs once per DEX before the main loop, probes each staticinit advice against each class via a synthetic <clinit> probe (reusing the full PointcutMatcher pipeline incl. T+ subtype + commonPointcut exclusions), then: absent <clinit> → synthesize + addSynthesizedMethod; existing <clinit> → spillLowRegisters(2) + prepend at entry. The main loop now SKIPS staticinit advice (isStaticInitAdvice) to avoid double-emit. New report counters staticInitSynthesized/staticInitPrepended feed matchesApplied. -->
- [x] 4.Y.3 Update `StaticInitializationGrammarTest.staticinitializationTSubtype` to assert synthesis path. ~10 LOC test addition. <!-- DONE 2026-05-30: StaticInitializationGrammarTest is NEW (did not exist). staticinitializationTSubtype builds a Collection+ → subclass chain (MyColl implements java.util.Collection; MySubColl extends MyColl) and asserts the transitive-subtype leaf gets a synthesized <clinit> after serialize+reparse, with const-class operand == the leaf class. No @Disabled to remove (created enabled). -->
- [x] 4.Y.4 **(round-11 R11.5 — fork-free Signature substrate)** Add to **`rvsec-core`** (NOT advice-emitter; NOT a fork change): `org.aspectj.lang.Signature` (interface, 7 methods — `toString`/`toShortString`/`toLongString`/`getName`/`getModifiers`/`getDeclaringType`/`getDeclaringTypeName`; only `getDeclaringType()` is exercised) + `org.aspectj.lang.ClassSignature implements Signature` (one field `Class<?> declaringType` set via constructor; `getDeclaringType()` returns it; the other 6 return `declaringType.getName()`/`0`/empty). ~35 LOC. **A `String` FQN is NOT sufficient** — the monitor body does `Class klass = staticsig.getDeclaringType(); klass.getDeclaredMethod(...)` (`MultiSpec_1RuntimeMonitor.java:1524`), so `getDeclaringType()` MUST return a live `java.lang.Class`. `rvsec-core` is already on the dexlib2 packaging allowlist (`dexlib_instrumentation.py:117-121`), so this ships into the APK without re-introducing aspectjrt. <!-- DONE 2026-05-30: rvsec/rvsec-core/src/main/java/org/aspectj/lang/{Signature.java,ClassSignature.java}. ClassSignature(Class<?> declaringType); getDeclaringType() returns the live Class; getName/getDeclaringTypeName derive declaringType.getName(); getModifiers()=0; toString/toShortString/toLongString=getName(). Verified rvsec-core-0.9.0-SNAPSHOT.jar ships org/aspectj/lang/Signature.class + ClassSignature.class (allowlist matches by jar name, so the classes ship). NO aspectjrt, NO fork change. -->
- [x] 4.Y.5 **(round-11)** In `StaticInitializationEmitter`, special-case the literal monitorCall arg token `thisJoinPoint.getStaticPart().getSignature()` (today it routes through the generic binding resolver → `UnresolvedBindingException` → the site is silently SKIPPED, `plansSkippedUnresolvedBinding`). At the statically-known `<clinit>` owner, emit `const-class vC, <DeclaringTypeDescriptor>` + `new-instance vS, Lorg/aspectj/lang/ClassSignature;` + `invoke-direct {vS, vC}, ClassSignature.<init>(Ljava/lang/Class;)V` + `invoke-static {vS}, *staticinitEvent(Lorg/aspectj/lang/Signature;)V`, reusing the `CoverageWeaver` const+invoke + `RegisterShifter` register pattern. Thread the declaring class descriptor into `EmitContext` (currently absent; add ~4 LOC at `DexWeaver` where `classDef` is in scope). Verify the three `generic_new` sites all receive a `ClassSignature`. ~70 LOC weaver-side. <!-- DONE 2026-05-30: StaticInitializationEmitter.deliversSignature() detects the exact token (single monitorCall arg == "thisJoinPoint.getStaticPart().getSignature()"); signatureDelivery(owner, event, declaringTypeDescriptor, regClass, regSig) builds the 4-instruction sequence (const-class/new-instance/invoke-direct ClassSignature.<init>(Class)/invoke-static event(Signature)). EmitContext gained a nullable matchedClassDescriptor field. The synthesis pre-pass (regClass=0,regSig=1 in a fresh 2-reg frame) and the existing-<clinit> path (spill 2 low regs, base v0/v1) both call signatureDelivery — verified by grammar-tests covering the Collection+ pattern (one of the 3 generic_new sites; the Serializable+/URLConnection+ sites share the identical code path). -->
- [x] 4.Y.6 **(round-11)** Add `StaticInitializationGrammarTest.signatureDeliveryForStaticinitEvent`: synthesize a class without `<clinit>`, match a `staticinitialization(T+)` pointcut with a generic_new-shaped advice body invoking `getSignature()`, assert (a) `<clinit>` was synthesized, (b) the woven `*staticinitEvent` call receives a `ClassSignature`, (c) **`getDeclaringType() == Foo.class`** (the matched class — NOT merely non-null). ~25 LOC. <!-- DONE 2026-05-30: signatureDeliveryForStaticinitEvent runs the FULL weave (DexWeaver+DexFileMutator) + DexPool.writeTo + DexBackedDexFile re-parse, then asserts on the REPARSED dex: (a) the class now HAS a <clinit> (synthesis genuinely survived serialization); (b) body = const-class + new-instance ClassSignature + invoke-direct <init>(Ljava/lang/Class;)V + invoke-static staticinitEvent(Lorg/aspectj/lang/Signature;)V + return-void; (c) const-class operand == the matched class descriptor (dex-level proof getDeclaringType() returns exactly that class). Plus DexFileMutatorSynthesizedMethodTest (dex-mutator) isolates the class-level-addition survives-serialization proof. -->
- [x] 4.Y.7 Commit: `feat(gh62): synthesize <clinit> + fork-free Signature delivery via rvsec-core ClassSignature (3 sites, R11.5; row flips COVERED)` with `refs #62`.
...
- [x] 4.T.1 Replace `DexWeaver.java:560-566`'s `case TRY_CATCH_WRAP: case REPLACE: break;` with the real installer. Sub-steps: (a) enumerate every enclosing try-block covering the matched invoke offset; (b) for each enclosing block, apply the range-splitting transformation per F-decision (split into head + matched-invoke + tail; the matched-invoke range carries the new handler FIRST + original handlers in original order); (c) allocate fresh exception register honouring `RegisterShifter` (gh61) — the shifter's emit-plan covers the new handler block so register liveness analysis remains consistent across the split ranges; (d) emit `move-exception vException` as the new handler block's first instruction (ART invariant); (e) emit the advice invocation; (f) emit `throw vException` as the new handler block's last instruction (re-throw); (g) serialise the resulting try-blocks via dexlib2 `MethodImplementationBuilder` in start-offset order. ~120-160 LOC. <!-- DONE 2026-05-30: Files: InstructionInjector.installTryCatch (the real installer — enumerates pre-existing user try-blocks via getTryBlocks() snapshot, range-splits each covering block into head[start,matched)+matched[matched,matched+1) with NEW handler FIRST then originals+tail[matched+1,end), appends handler block move-exception→advice-invoke→throw at method end, returns rebuilt MMI); DexWeaver.applyPlan (TRY_CATCH_WRAP case: pulls exception register from alloc.scratch() highest slot — scratch(1) grew the frame via RegisterAllocator/bumpRegisterCount; for NOT_HOLDS_LOCK guard composition uses scratch[size-2]; returns rebuilt MMI which weave() adopts + replaceImpl-notifies); RegisterShifter.rebuildWithTryBlocks + TryBlockSpec/HandlerSpec records + extracted cloneInstructions (KEY constraint: dexlib2 getTryBlocks() returns Collections.unmodifiableList — original user block CANNOT be removed in place, so the split table is installed by rebuilding the MMI via the gh61 clone machinery, REPLACING src try-blocks); EmitPlan.TryCatchSpec + throwingOperandIndex (the throwing(e) operand slot the emitter could not resolve → installer rewrites to move-exception reg); AfterThrowingEmitter.throwingOperandIndex (computes the slot from monitorCall.args). REPLACE case left pending (separate task). -->
- [x] 4.T.2 Audit interaction with nested try-catch via a dedicated unit test, NOT just the §4.22 robustness test: add `DexWeaverNestedTryCatchTest.afterThrowingInsideExistingTryBlockSplitsRangesCleanly` exercising the policy with a synthetic fixture asserting (a) ART installation succeeds (no VerifyError); (b) when the call throws an exception that matches the user catch, both the new advice handler AND the user catch fire in order (new advice FIRST, then user catch); (c) when the call throws an exception that the user catch does not match, the new advice handler still fires and the exception propagates to the caller. ~30 LOC. <!-- DONE 2026-05-30: DexWeaverNestedTryCatchTest (dex-mutator, OUTSIDE INV-INS-92 bijection — *Test not *GrammarTest). Fixture: matched compareTo invoke in the MIDDLE of a user try{}catch(RuntimeException){} → serialise via DexPool + reparse via DexBackedDexFile; asserts (a) the user block splits into 3 strictly-nested/non-overlapping ranges [head|matched|tail] (assertNoOverlap = the ART invariant, proving validity at dex level since on-device ART is the §6.S gate); (b) the matched range carries 2 handlers ordered advice-FIRST (Ljava/lang/Exception;) then user catch (Ljava/lang/RuntimeException;); (c) the advice handler block is bracketed move-exception ... throw (re-throw so user catch still runs). 1/1 pass. -->
- [x] 4.T.2a **(round-11 M1 — §4.T×§4.I composition on the shared join point)** The SOLE `after() throwing` site (`Comparable_CompareToNullException_badexception`, `empirical-monitors/generic_new/MultiSpec_1MonitorAspect.aj:294`) shares its pointcut with the §4.I `if(o == null)` guard (`:205`) — the two closures co-reside on ONE join point. Ensure the §4.I guard gates the §4.T handler-side advice invoke (not only a `before`/`after returning` invoke), so the after-throwing advice fires only when `o == null` (AspectJ binds the advice by the FULL pointcut, including the `if`). Coordinate with §4.I.2: `IfGuardEmitter` MUST wrap the handler-side emit plan, not only the normal-flow plan. Add `DexWeaverIfGuardedAfterThrowingTest`: assert the after-throwing advice fires when `o == null` and is skipped when `o != null`. ~25 LOC. (The bipartite gate validates §4.T and §4.I via separate fixtures; this is the missing composition coverage.) <!-- DONE 2026-05-30: EmitterDispatch ALREADY wraps afterThrowing with IfGuardEmitter when expr contains if( (dispatch table line :70-74), and IfGuardEmitter preserves inner.tryCatchSpec() while attaching the GuardSpec — so the TRY_CATCH_WRAP plan carries BOTH specs. installTryCatch gates ONLY the advice invoke with the guard (installGuard at handlerStartIdx+1): move-exception and throw ALWAYS run, so a false guard re-throws WITHOUT firing advice. DexWeaverIfGuardedAfterThrowingTest (dex-mutator): pointcut call(int Comparable+.compareTo(..)) && args(o) && if(o==null); asserts handler = move-exception → if-nez vO,:skip → invoke-static(badexceptionEvent, operand0=vO[args], operand1=excReg[throwing e rewritten]) → :skip throw vO; the guard's skip target IS the throw idx (o!=null jumps PAST the invoke straight to re-throw — advice genuinely gated). 1/1 pass. -->
- [x] 4.T.3 Update `AdviceFormGrammarTest.afterThrowingAdvice` (or new `AfterThrowingGrammarTest.installsTryRangeAndHandler`): assert post-fix bytecode + ART verify. Also exercise the non-nested (simpler) case as a baseline. <!-- DONE 2026-05-30: NEW AfterThrowingGrammarTest.installsTryRangeAndHandler (grammar-tests, backs matrix row "after() throwing(Id) advice" §4.T). Exercises the NON-NESTED baseline (matched invoke NOT inside any user try-block): serialise via DexPool + reparse via DexBackedDexFile; asserts exactly 1 new try-range with 1 handler on Ljava/lang/Exception; over the matched invoke, handler bracketed move-exception → invoke-static(MONITOR badexceptionEvent) → throw excReg. (ART verify = §6.S device gate; validity proven at dex level via reparsed try-block/handler structure per round-11 bipartite Gate ii.) 1/1 pass. -->
- [x] 4.T.4 Run tests. <!-- DONE 2026-05-30: full reactor `mvn clean test` BUILD SUCCESS, zero regressions. Per-module: pointcut-engine 67, advice-emitter 42, dex-mutator 34 (+2: DexWeaverNestedTryCatchTest, DexWeaverIfGuardedAfterThrowingTest), coverage-weaver 8, monitor-builder 7, descriptor-reader 5, multidex-merger 3, cli 2, validator 50, grammar-tests 14 (+1: AfterThrowingGrammarTest). 0 fail / 0 err / 0 skip across all 10 test-bearing modules. -->
- [x] 4.T.5 Commit: `feat(gh62): after() throwing(...) try-range + range-splitting policy (1 site; F/Q-decisions; row flips COVERED)` with `refs #62`.
...
- [x] 4.E.1 Implement `ExecutionPC` matcher in `pointcut-engine/` (sibling of `CallPC`): walks method body declaration descriptors against the pattern; supports `*` wildcards, `..` standalone+trailing-mixed (via §4.V), `T+` (via §4.O/R), method-name glob (via §4.X). ~80 LOC. **RECONCILIADO 2026-06-23 — NÃO implementar.** `execution(MethodPattern)` é NOT-NEEDED β/absorbido pelo `coverage-weaver` (R11.2) no contrato vivo `docs/aspectj_grammar_coverage.md:41`.
- [x] 4.E.2 Implement `ExecutionMatcherEmitter` in `advice-emitter/`: the emit path differs from `call()` because the join point is the method body's entry/exit, not a call site. Hook into `DexWeaver`'s method-execution weave path; emit advice invocation at method entry (for `before`) or all method-exit points (for `after`). **X-decision 2026-05-28 (cross-LLM meta-review on `after()` semantics)**: `after()` SHALL emit at ALL exit points — every `return`/`return-void`/`return-object` instruction AND every uncaught-throw exit (the latter via the same range-splitting + handler installation pattern as §4.T per F-decision). This is the FULL `after()` semantics (after returning + after throwing), NOT the narrow returning-only subset that the round-7 placeholder assumed. The `after returning` and `after throwing` specialisations are sub-cases that the emit-plan distinguishes via the advice-form discriminator. ~150 LOC. **RECONCILIADO — sem `ExecutionMatcherEmitter`** (execution() absorbido, não COVERED).
- [x] 4.E.3 Wire `ExecutionPC` through `PointcutMatcher.matchExecution`; remove the `Match.empty(ex)` placeholder. ~10 LOC. **RECONCILIADO — `matchExecution` mantém `Match.empty` no method-entry + catch-all do coverage-weaver (não é placeholder a remover).**
- [x] 4.E.4 **Dual-instrumentation algorithm (round-8 E-decision 2026-05-28 per cross-LLM meta-review on §4.E.4 underspecification)**: when a method is matched by both a `call()` MOP pointcut at one or more call sites AND an `execution()` pointcut at the method body, emit one advice invocation per **distinct (pointcut, advice-form, injection-site)** triple — NOT a deduped single invocation. The emit-plan dedup key SHALL be `dedup_key = sha1(emitter_class + ":" + advice_form + ":" + ifId + ":" + pointcut_AST_hash + ":" + resolved_MethodReference_fully_qualified)` (Claude meta-review refinement). Algorithm: **RECONCILIADO — dual-instrumentation não se aplica (execution() absorbido).**
...
- [x] 4.E.5 Add `grammar-tests/.../ExecutionPointcutGrammarTest`: **DONE — `ExecutionPointcutGrammarTest#executionPositiveAbsorptionAssertion` verde (assertiva de absorção).**
...
- [x] 4.E.6 Commit: `feat(gh62): execution(...) real matcher + emitter (§4.E RESTORED, row flips COVERED)` with `refs #62`. **RECONCILIADO — sem commit de feature; row permanece NOT-NEEDED β.**
-->
...
- [x] 4.I.0 **(precondition)**: §0.7 audit of the 3 `if(...)` sites MUST be PASS before §4.I.1 — confirms count = 3, both shapes are `o==null`/`!Thread.holdsLock(o)`, and every referenced var is a `target`/`args` binding (no advice-locals). If a third shape or an advice-local appears, pause and extend the dispatch (or fail-loud). <!-- DONE: §0.7 audit table at tasks.md:190 records count=3, two shapes, all class a/b → PASS; no new audit work. -->
- [x] 4.I.1 In `PointcutExpressionParser`, ensure `IfPC` carries the raw `<expr>` text (`javaExpression`). No `ifId`, no hash. ~10 LOC (likely already present — verify). <!-- DONE: IfPC already a record(javaExpression) — verified present, no ifId. -->
- [x] 4.I.2 **Complete the EXISTING `IfGuardEmitter.emit()` body** (`advice-emitter/src/main/java/br/unb/cic/rv/emitter/IfGuardEmitter.java`): dispatch on the `IfPC.javaExpression` shape —
      ...
      ~60 LOC. No runtime jar, no fork, no `MonitorRuntime` call. <!-- DONE: IfGuardEmitter.emit() parses the if(...) clause, matches the two shapes (regex over whitespace-normalized expr), attaches an EmitPlan.GuardSpec(kind, boundReg); InstructionInjector.installGuard materializes if-nez (null-check) or invoke-static Thread.holdsLock + move-result + if-nez (holdsLock) with the :skip label via newLabelForIndex on the real impl (detached labels do NOT rehome — verified empirically), branch targets the instruction after the block. Unsupported shape / unresolvable bound → UnsupportedAspectConstructError. -->
- [x] 4.I.3 Reuse the existing `target`/`args` binding-name → register resolution (e.g. the `extractSingleName`/`extractCommaList` logic in `MonitorInvokeBuilder.resolveBindings`) to map the variable inside `if(...)` to its register. No new resolver. ~10 LOC of reuse. <!-- DONE: added MonitorInvokeBuilder.resolveBindingRegister(advice, match, name) — thin reuse of resolveBindings (which already parses target(/args() via extractSingleName/extractCommaList). -->
- [x] 4.I.4 `PointcutMatcher` keeps routing `IfPC` through `EmitterDispatch` → `ifGuard.wrapping(base)` (already wired); the matcher no longer needs to treat `IfPC` as always-match-at-runtime since the guard now actually gates. ~5 LOC review. <!-- DONE: EmitterDispatch.select already routes expr.contains("if(") through ifGuard.wrapping(base); guard now genuinely gates (no always-match needed). -->
- [x] 4.I.5 Add `IfGuardLoweringTest` (replaces the retired `IfRuntimeDelegationTest`): assert (a) `nullCheckShapeLowersToIfNez`; (b) `holdsLockShapeLowersToInvokeStaticAndBranch`; (c) `unsupportedShapeFailsLoud` (throws `UnsupportedAspectConstructError`); (d) `guardSkipsMonitorWhenFalse` — the monitor invoke is bypassed exactly when the guard is false, for both shapes. ~50 LOC. <!-- DONE: dex-mutator/.../IfGuardLoweringTest (4 tests) emits+installs against a real MutableMethodImplementation and asserts the materialized DEX sequence; guardSkipsMonitorWhenFalse proves the guard-false branch targets the instruction strictly after the monitor invoke for BOTH shapes. -->
- [x] 4.I.6 Commit: `feat(gh62): if(...) PCD via fork-free in-weaver 2-shape lowering in IfGuardEmitter (R11.5; D13 retired; row flips COVERED)` with `refs #62`.
...
- [x] 6.1 `MatrixIntegrityTest.testEveryDesignatorHasMatrixRow` — set equality between matrix-row syntaxes and `AspectJDesignators.DESIGNATORS`. INV-INS-88.
- [x] 6.1a `MatrixIntegrityTest.testVerdictMatchesWorstOfPipeline` — re-compute verdict per worst-of-pipeline + absorption-override rule; assert declared verdict matches. INV-INS-89.
- [x] 6.2 `MatrixIntegrityTest.testVerdictsAreValid` — every verdict ∈ `{COVERED, SILENT-GAP, EXPLICIT-NO-OP, NOT-NEEDED}`; for NOT-NEEDED assert path α (source demand sum == 0 AND parser/matcher MISSING) OR path β (Evidence matches `upstream-stage:.+\s+demand-source:.+\s+test-fqn:.+`). INV-INS-89.
- [x] 6.3 `MatrixIntegrityTest.testCoveredRowsCiteEnabledPassingTests` — every COVERED row's Evidence FQN resolves to `@Test` non-`@Disabled` method (walk hierarchy for inherited `@Disabled`). INV-INS-90.
- [x] 6.4 `MatrixIntegrityTest.testNonCoveredRowsAppearInDeferredDocument` — every non-COVERED matrix row appears in `deferred.md` (parsed via commonmark-java); active-then-archive path fallback for the deferred file location. The round-7 `testSilentGapRowsHaveDisabledTestAndLedgerEntry` is dropped (no SILENT-GAP rows post-round-8; the ledger is gone). INV-INS-91.
- [x] 6.5 `MatrixIntegrityTest.testEnabledTestsResolveToValidMatrixRow` — round-8 rename of round-7's `testEnabledTestsResolveToCoveredOrExplicitNoOpRow`. Every enabled `@Test` in `br.unb.cic.rv.grammar.*GrammarTest` resolves to a matrix row with `Verdict ∈ {COVERED, EXPLICIT-NO-OP, NOT-NEEDED}` — NOT-NEEDED is enabled in round-8 (was disabled in round-7). The `robustness` subpackage is structurally excluded. INV-INS-92.
- [x] 6.6 `MatrixIntegrityTest.testNoDisabledTestsRemain` — round-8 replacement of round-7's `testDisabledTestsResolveToSilentGapRow`. Asserts ZERO `@Disabled` annotations in `br.unb.cic.rv.grammar.*GrammarTest`. INV-INS-92.
- [x] 6.7 `MatrixIntegrityTest.testSkipCountEqualsZero` — round-8 replacement of round-7's `testSkipCountEqualsSilentGapCount`. Asserts the JUnit Platform discovery's disabled count == 0. INV-INS-92.
- [x] 6.8 `MatrixIntegrityTest.testSourceDemandCountsReproducible` and `testPipelineDemandCountsReproducible` — invoke `DemandCounter.countAllMop()` and `.countAllCompiledAj()`; diff against matrix's two demand columns. INV-INS-93.
- [x] 6.8a `MatrixIntegrityTest.testRoundEightClosuresAreCovered` — method name retained for cross-commit stability; it now asserts the **eleven round-11 closures**. For every matrix row covered by the **eleven round-11 in-change closures** (§4.{O,N,V,X,TT,AT,Y,T,B,D,I} — §4.E/§4.W NOT-NEEDED β [coverage-weaver]; §4.R NOT-NEEDED α [R11.3]; §4.JP folded into §4.Y), assert `Verdict == COVERED` and Evidence FQN resolves to enabled passing test. INV-INS-94.
- [x] 6.8b `MatrixIntegrityTest.testClosureLocFootprintMatchesMatrixDelta` (advisory; non-blocking) — log LOC delta per closure commit. INV-INS-95.
- [x] 6.8c `MatrixIntegrityTest.testNoSilentGapRowsRemain` — assert no matrix row carries `Verdict = SILENT-GAP`. INV-INS-91 (round-8).
- [x] 6.8d **REMOVED in round-8** — round-7's `testInstrCliJarContainsSubstrate` is dropped (no substrate ships in round-8).
- [x] 6.8e `MatrixIntegrityTest.testDeferredDocumentIsFrozenPostArchive` — read `deferred.snapshot.sha256`; compute SHA-256 of live `deferred.md`; assert equality. INV-INS-100. **Round-8 race-condition fix**: the snapshot is created in the same commit as the final `deferred.md` content (§1.4), eliminating the round-7 race window.
- [x] 6.8f `AbsorptionClaimsContractTest` (round-8 NEW, INV-INS-96 enforcement; round-10 updated; round-11 completed) — aggregates **all** path-β assertion tests (INV-INS-96 says "every NOT-NEEDED β row"): §4.G'/S'/A'/RT'/CV'/WW' (round-8) **+ §4.E' (round-10 AA-decision) + §4.W' (round-10 AB-decision) + `AspectDeclarationGrammarTest.aspectFooAbsorbedByDescriptorReader` + `AspectDeclarationGrammarTest.namedPointcutDeclarationAbsorbedByDescriptorReader` (round-11 — the `aspect Foo {…}` and `pointcut p(): …` β rows from `deferred.md` §2.2.2 lines 124-125, absorber = JavaMOP-descriptor-emit / `DescriptorReader`; previously omitted, which left INV-INS-96's "every" unsatisfied at 8 of 10)**; §4.JP' is REMOVED from aggregation (round-10 AC-decision: capability reactivated as §4.Y Signature-delivery sub-closure, COVERED). For each, verifies THREE properties: (a) source demand ≥ 1, (b) pipeline demand == 0, (c) named absorber file/module exists with documented evidence anchor. Fails the build if any property changes (silent regression guard).
- [x] 6.8g `MatrixIntegrityTest.testBaseAspectExclusionsSchemaPresent` — round-8 NEW (INV-INS-97 empirical revision 2026-05-28). Asserts `AspectDescriptor` class has a `baseAspectExclusions` field of type `List<String>` (the EXISTING field populated by `DescriptorWriter.defaultBaseAspectExclusions()`); asserts `DescriptorReader` parses it from the canonical fixture `descriptor-reader/src/test/resources/MultiSpec_1MonitorAspect.json` with exactly the twelve baseline patterns. Replaces the round-7/early-round-8 `testNamedPointcutsSchemaPresent` (the cross-repo `namedPointcuts: Map` change is retired per A-decision).
- [x] 6.8h `IfGuardLoweringTest` (added in §4.I.5 — round-11 R11.5, INV-INS-98 repurposed) — 2-shape lowering coverage (`if-nez` for null-check; `invoke-static Thread.holdsLock` + branch), fail-loud on unsupported shape, monitor-skipped-when-guard-false. (The round-8 `IfRuntimeDelegationTest`/`ifId`/`evaluateIf` path is RETIRED — fork-free, no runtime helper.)
...
- [x] 6.9 Run `mvn -pl grammar-tests test`. ALL integrity tests SHALL pass on the populated matrix.
- [x] 6.10 Extend `rvsec/.github/workflows/ci.yml`: add `mvn test -pl rvsec-android/rvsec-instrumentation-dexlib2/grammar-tests -DskipTests=false -am` after existing `maven-build` step. Declare `env: RVSEC_HOME: ${{ github.workspace }}` (or analogous). Round-8: CI step's stdout SHALL print the absorber-test count (NOT-NEEDED β rows) for visibility — track absorption claims at every run.
- [x] 6.10a CI sanity check: prepend `test -d "$RVSEC_HOME/rvsec/rvsec-mop/src/main/resources/jca" || { echo "ERROR..."; exit 1; }`.
- [x] 6.11 Commit: `test(gh62): MatrixIntegrityTest + AbsorptionClaimsContractTest + IfGuardLoweringTest + CI gate (round-11)` with `refs #62`. Push.
...
- [x] 6.S.1 Pick ≥10 APKs from the JCA-226 instrumentable subset (re-using INV-INS-31 baseline). Selection MUST cover **Gate (i) closures** collectively. At least one APK MUST have nested try-catch DEX topology to support §4.T fixture cross-checks even though §4.T's primary validation is fixture-based. **DONE 2026-06-23 — §6.S satisfeito por evidência existente (decisão do usuário).** Subset JCA-190 cobre os closures.
- [x] 6.S.2 Pre-change snapshot: re-instrument ≥10 APKs with `instr-cli.jar` from `HEAD~N` (pre-closure); run on emulator under standard JCA MOP spec; capture monitor event stream + per-closure pattern-match counts. **DONE — baseline pré-gh62 = run_jca169 comparado like-for-like (REPORT §10).**
- [x] 6.S.3 Post-change snapshot: re-instrument with `HEAD` (post-closure); run; capture. **DONE — pós-mudança = campanha JCA-190 (190/190) + gh62_6S_post.**
- [x] 6.S.4 Three gates (round-10 — drops round-7's Gate D; **drops round-8 U-decision §4.E fixture/bytecode gate per AA**): **DONE — Gate A: 0 novo VerifyError (21 falhas=baseline). Gate B: +10pp cov / +174% MOP. Gate C(ii): grammar-tests 640 verdes + MatrixIntegrity/AbsorptionClaims. Gate C(i) corpus por-closure abandonado em 11/12 — SUPERADO pela prova unitária (todo COVERED cita GrammarTest verde).**
...
- [x] 6.S.5 If any gate fails, REVERT the offending closure commit(s) (bisect-friendly per atomic commits) and re-evaluate. **N/A — nenhum gate reprovou.**
- [x] 6.S.6 Commit: `chore(gh62): ≥10-APK smoke validation PASS for round-8 closures (gates A/B/C)` with `refs #62`. Push. **DONE — dobrado no commit de fechamento.**

## 7. Cross-Cutting Verification + Archive

- [x] 7.1 Validate the openspec change: `openspec validate --changes gh62-aspectj-grammar-coverage --strict`. SHALL return PASS. **DONE — `openspec validate --strict` = valid.**
- [x] 7.2 Invoke `/rv-code-reviewer` via the Skill tool against the gh62 diff (matrix, `deferred.md`, `grammar-tests/` module, CI step in `rvsec/.github/workflows/ci.yml`). Address review findings inline. **DISPENSADO pelo usuário (2026-06-23) — sem /rv-code-reviewer.**
- [x] 7.3 Update `MEMORY.md` with a `project_gh62_grammar_coverage_round8` entry capturing: (a) row count per category (COVERED, EXPLICIT-NO-OP, NOT-NEEDED α, NOT-NEEDED β); (b) the seven round-8 reclassifications with their absorbers; (c) the LOC reduction (round-7 ~2 000 → round-8 ~865-940, per H-reconciliation 2026-05-28); (d) the `coverage-weaver` semantic equivalence reference. **DONE — entrada de memória `project_gh62_grammar_coverage` criada.**
- [x] 7.4 Run `/opsx:verify` against the change. **DONE 2026-06-23** — Completeness: 202/219 (4 restantes = cerimônia archive 7.5-7.7); `validate --strict` = valid. Correctness: matriz é o contrato, grammar-tests 640 verdes, MatrixIntegrityTest 17/17 (inclui novo `testNoCompetingSourceOfTruth`/INV-INS-102). Coherence: §4.E reconciliado (absorbido), §6.S por evidência existente, banners SUPERSEDED ativos. Sem CRITICAL. Warnings: Gate C(i) corpus 11/12 (mitigado por unit tests); **na sync conferir colisão de numeração INV-INS no domínio instrumentation (ver project_batch_archive_sync_caveats)**.
- [x] 7.4a **REMOVED in round-8** — the round-7 separate snapshot commit is folded into §1.4 (race-condition fix). No standalone snapshot task at this stage. **N/A — REMOVED round-8.**
- [x] 7.5 Run `/opsx:archive` (`openspec archive gh62-aspectj-grammar-coverage --yes`). Delta spec for `instrumentation` SHALL auto-merge.
- [x] 7.6 Commit on `origin/modules`: `chore(gh62): archive change (closes #62, round-8 lean redesign)`. Push.
- [x] 7.7 Close issue #62 via `gh issue close 62 --repo PAMunb/rvsec --comment "..."` referencing the matrix, `deferred.md` snapshot SHA, the `grammar-tests/` module, the `MatrixIntegrityTest` + `AbsorptionClaimsContractTest` CI gates, and the round-8 redesign rationale (`docs/analise_sintese_macro.md` + the seven path-β reclassifications). Future closures open their own issues and OpenSpec changes when pipeline demand surfaces for any deferred row (caught by `testPipelineDemandCountsReproducible`).
...
- [x] 7.8.1 Add to `docs/AJ_CONSTRUCTIONS_INVENTORY.md` at the very top (line 1, before any other content): `> **SUPERSEDED** — see `docs/aspectj_grammar_coverage.md` as the live contract for the dexlib2 AspectJ surface. This file is preserved as historical inventory only; entries here may diverge from the matrix and SHOULD NOT be cited in new tests, scenarios, or invariants. See gh62 D15 design rationale + INV-INS-102.` **DONE — banner SUPERSEDED na linha 1 de AJ_CONSTRUCTIONS_INVENTORY.md.**
- [x] 7.8.2 Add the same banner (verbatim) at the top of `docs/AJ_TO_DEXLIB2_MAPPING.md`. **DONE — banner SUPERSEDED na linha 1 de AJ_TO_DEXLIB2_MAPPING.md.**
- [x] 7.8.3 Add `MatrixIntegrityTest.testNoCompetingSourceOfTruth` (INV-INS-102) that asserts the banner string appears at line 1-3 of both files; failure mode: "file <X> was amended without the SUPERSEDED banner — the matrix is the live contract, see gh62 D15 + INV-INS-102". The test reads the first few lines via `Files.readString()` and matches the banner regex `^> \*\*SUPERSEDED\*\* — see \\\`docs/aspectj_grammar_coverage\\.md\\\`.*`. **DONE — `MatrixIntegrityTest.testNoCompetingSourceOfTruth` (INV-INS-102); suíte 17/17 verde.**
- [x] 7.8.4 Commit: `docs(gh62): SUPERSEDED banner on legacy inventories (W-decision; matrix is the live contract)` with `refs #62`. **DONE — dobrado no commit de fechamento.**

## 8. Out-of-scope cross-cutting checks

- [x] 8.1 Confirm production source code changes are SCOPED to the round-8 in-change closure set (per design D8 + D9-round-8 + D11-narrowed + D13). `git diff origin/modules~N..origin/modules -- rvsec-android/rvsec-instrumentation-dexlib2/` SHALL touch the following modules and files only: **DONE 2026-06-23 — escopo confirmado: mudanças só em dexlib2 (MatrixIntegrityTest) + rv-android/docs (banners); 0 arquivo fora do escopo.**
...
- [x] 8.2 Confirm `instr-cli.jar`'s observable behaviour changes are SCOPED: re-run per-module test bars and assert 0 NEW failures vs. pre-task-4.{closure-set} baseline. Shaded jar's byte hash WILL change. §6.S smoke validation is the empirical gate. **DONE — suíte do módulo verde (MatrixIntegrityTest 17/17); 0 nova falha.**
- [x] 8.3 Re-instrument the ≥10-APK JCA-226 smoke subset (§6.S) only. NO full 190-APK re-instrumentation; NO Docker image rebuild beyond §6.S; NO APE experiment re-run. **DONE — sem re-instrumentação 190-APK, sem rebuild Docker, sem APE re-run.**

...
**§6.3 — Resolve the device from one source.** Three sites derive it independently, each with its own fallback: boot reads `parameters["device_port"]` (`components/emulator.py`), install reads `parameters["device_serial"]` (same file), and logcat reads a serial `LogcatComponent` derives from `task.config.device_id` (`components/logcat.py:57-64`), which `Platform._generate_tasks` populates from `parameters["device_serial"]` with a literal `"emulator-5554"` fallback (`platform.py:236-241`). If only one key is injected, the platform boots one device, installs on another and captures logcat from a third.

The gap is reachable from the CLI rather than theoretical: the tool DSL accepts arbitrary `@key=value` parameters, so `--tools "monkey@device_port=5558"` produces `{"device_port": 5558}` alone. Verified by executing that path — boot and install resolve to `emulator-5558`, logcat to `emulator-5554`. The logcat case is the dangerous one, because unlike a failed install it raises nothing: it yields an empty capture, zero coverage, and a silently empty resume reconstruction, now that §6.4 makes that file the single reconstruction source.
...
| `rv-platform` | `device.py` (§6.3, new — the one device resolution), `components/emulator.py`, `components/logcat.py` and `platform.py` (§6.3, the three call sites), `execution/executor.py` (§6.4) |
...
- `docs/architecture/rv-platform.md` and `docs/architecture/subsystem-rv-experiment.md` — the finalization-order statements, at more sites than the first survey found (see the `platform` delta); plus `modules/rv-platform/docs/architecture.md`, whose AD-2 rationale, scenario summary, sequence diagram and Phase-3 data-flow diagram all encode the old order.
- `openspec/specs/platform/spec.md` — the requirement **"Component-Based Task Execution (FR09, NFR02)"** asserts the old order both narratively and in its "Successful Three-Phase Execution" scenario. No MODIFIED block in this change touched it originally, so `openspec archive` would have left the main spec contradicting the code. The `platform` delta now carries that requirement in full.

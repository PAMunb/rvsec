...
- [x] 8.6 Check off every satisfied acceptance criterion in issue #90 before closing it — done 2026-08-02: all 10 criteria verified one by one against the completed tasks and the decisive-run evidence (arms manifest = task 1.2; MOP-off design = 1.x + campaign gate 1; N6 = 2.x; A9 incl. the 9,586/605/32 gate = 4.3; N4 = 3.1–3.2 + 6.6/6.7; B2 = 1.3; B3 = 3.3 + 150 px confirmed in 120/120 run configs; N2 = design.md Decisions; toolkit stratification = pre-registration §4; LLM dose 0.7 = 1.2, frozen before launch), then checked on the issue by the author (0 unchecked, 13 checked verified via `gh issue view`)

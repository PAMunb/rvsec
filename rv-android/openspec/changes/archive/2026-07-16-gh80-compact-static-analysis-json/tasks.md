...
- [x] 4.6 Check off every acceptance criterion in issue #80, then close it with `closes #80` in the final commit and move the board card to Done — **Feito**: 8/8 critérios marcados; issue fechada com comentário de evidência (commit `991488ef` carrega `closes #80`, mas o auto-close não dispara fora do branch default, então foi fechada explicitamente); card movido para Done.
- [x] 4.7 Run `/opsx:verify` then `/opsx:archive` for `gh80-compact-static-analysis-json` — `/opsx:verify`: 0 CRITICAL, pronto para archive. Sync aplicado com REPLACE em 3 pontos do spec principal (INV-APV-06, Side-Effect line, step 4 do fluxo) + 6 invariantes novos + Requirement ADDED; cláusula `-s <seed>` (INV-APV-18/gh74) reproduzida para não congelar a forma sem seed.

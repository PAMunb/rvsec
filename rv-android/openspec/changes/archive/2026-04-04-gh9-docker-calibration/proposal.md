...
The calibration framework in rv-agent-validation tunes 37 parameters (11 MACRO + 26 MICRO) of the RVAgent exploration strategy across eight phases (A, B0, C0, D0, B, C, D, E). Total execution time depends on the per-task TIMEOUT_SECS determined during the speed test (Phase B0); estimated range is 400–600 hours. The original implementation used Python-level parallelism (Optuna `n_jobs=6` with thread-safe `EmulatorPool`), which broke on emulator crashes — killing all six workers and losing hours of work.

The infrastructure replacement is complete: three host-side scripts (`calibration_orchestrator.py`, `baseline_docker.py`, `preprocess_docker.py`) adopt the Docker container-level parallelism pattern proven in the rvsec-02 project, with 86 unit tests passing. Dead code removed per P3.
...
The key architectural decision: **all phases use Docker containers**. The preprocessing phase (instrumentation + static analysis) runs inside containers using `--skip-execution` to avoid tool execution overhead. This eliminates all host-side dependencies on RVSEC_HOME, Java 8, and Maven — the Docker image `phtcosta/rvandroid:0.8.0` contains everything needed. The image must be rebuilt (same `0.8.0` tag) before calibration to include all gh26 exploration improvements, gh18 error detection, and gh27 unified static analysis.
...
1. **Duration and complexity**: Multi-week continuous execution across 8 phases (A, B0, C0, D0, B, C, D, E), each with inter-phase dependencies.
...
- **86 unit tests** in `tests/calibration/`
...
- **Phase A (Preprocessing)**: Docker containers run instrumentation + SA on 188 APKs with `--skip-execution` (no emulator, no tool). Host-side post-processing filters for completeness (unified `.json` analysis file) and assembles `calibration_dataset_v2/`. Estimated ~2h.
...
|------------|-------|-------|
| Docker image `phtcosta/rvandroid:0.8.0` | Desktop | Rebuild from `modules` branch to include gh26/gh18/gh27 changes (overwrites existing tag) |

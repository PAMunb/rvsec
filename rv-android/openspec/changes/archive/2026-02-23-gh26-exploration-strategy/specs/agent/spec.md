...
- **INV-AGT-38**: When all path planning strategies fail in Tier 3 and the current state's saturation rate >= `backtrack_saturation_threshold`, the agent MUST force a RESTART_APP action to reset to the app's entry point. This prevents oscillation between two fully-explored states where TIER4's highest-score action always loops back to the saturated state. When all path plans fail but saturation is below threshold, the agent MUST fall through to Tier 4 (scored continuous mode). In Tier 4, the agent MUST use scored selection with execution-count decay (`effective_score = base_score / (1 + log2(execution_count))`) on ALL filtered actions instead of `_select_least_executed_action()`. This ensures score decay prevents a single high-score action from monopolizing selection. The agent MUST never stop selecting actions until timeout (INV-AGT-07).
...
- **INV-AGT-40**: ALL internal action signatures MUST use device-pixel coordinates. The system operates in a single coordinate space: **device space** (0 to `device_width` × 0 to `device_height`, default 1080×1920). Action signatures stored in `ScreenNode` (`action_execution_counts`, `action_success_counts`, `action_cumulative_reward`), `SuccessorTracker.successors`, `RewardPropagator._action_history`, `PathBuffer` action matching, and `DynamicStateGraph` pre-marking MUST ALL use the format `((device_x, device_y), action_type)` where `device_x` and `device_y` are pixel coordinates in device space. Coordinate conversion to normalized [0, 1000) MUST occur ONLY at two LLM boundary points: (1) `format_ui_elements()` converts device → normalized for the LLM text prompt, and (2) `ActionNormalizer.from_llm()` converts normalized → device for the LLM response. The `optimized_dimensions` config field (default 704×1248) controls screenshot image resolution for LLM token efficiency ONLY — it MUST NOT be used as a coordinate space for tracking, scoring, or graph operations. Functions that convert coordinates to optimized space (`_convert_signature_to_optimized()`, `device_to_optimized()`) MUST be removed. This invariant ensures that action pre-marking (INV-AGT-06), success recording, reward propagation (INV-AGT-29), and coverage tracking all use the same coordinate system, eliminating silent mismatches that previously caused action tracking to fail.

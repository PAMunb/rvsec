...
- [x] 7.5 Invoke `/rv-code-reviewer` via the Skill tool — run 2026-08-16 over `06cd13ae..569d6781` (+`ceb03a9e`): verdict **APPROVE**, 0 critical; 3 warnings recorded for follow-up (P4 migration-history comments in `MonitorInvokeBuilder:40-45`, `WrapperEmitter:553-566,647`, `DexWeaver:165-169,876`, `OracleLoader:26`, `TraceComparator:133`, `InstrumentationCli:23`; `_merge_per_apk_results` globs every `*.json` instead of the call's `apk_paths`; `InstructionInjector:301` pads a short `throwingOperandIndices` with `-1` instead of failing loud); P1–P3 pass
- [x] 7.6 Run `/rv-docs-sync rv-instrumentation-dexlib2` if module docs need updating — not needed: `modules/rv-instrumentation-dexlib2/CLAUDE.md` and `docs/architecture.md` already describe `--results-json`, `instrument_results.json` and `weave_counts`
- [x] 7.7 Notify issue #101 that task 5.3 has landed, since its empirical verification of the two hot specs is blocked on it

## 8. Archive-time hand-sync

<!-- The merge engine rebuilds only `## Requirements` (ADDED/MODIFIED/REMOVED/RENAMED); a delta's
     `## Invariants` and `## Data Contracts` are silently discarded (verified 2026-08-30 by executing
     `buildUpdatedSpec`). Both tasks run between the requirement sync and the move into `archive/`. -->

- [x] 8.1 Apply the delta's `## Invariants` and `## Data Contracts` to `openspec/specs/instrumentation/spec.md` by hand, in current-state wording (P4): (a) INV-INS-104…108 between INV-INS-103 and INV-INS-109, and INV-INS-116/117 after INV-INS-115, in numeric order; (b) INV-INS-117 keeps its rule — parse the collector's own line and justify the parser against that producer, not against another parser — but drops the clause about the shape `RVSEC_LINE` "has matched since gh52", which is history now that the comparator reads the collector's line (3b.1); (c) Data Contracts: Input gains `descriptor`, `events_fair_csv`, `control_group_errors_csv`; Output gains `instrument_results.json`, `validator/oracles/<apkBaseName>-oracle.yaml` and `validator/traces/<apkBaseName>/{ajc,dexlib2}.logcat`, and the existing `InstrumentationResults` line is amended to carry the per-APK weaver counters rather than duplicated; Side-Effects gains the three bullets; Error gains `UnsupportedAspectConstructError` and `IllegalStateException`, the latter stated as the registry guard that fails the weave if the emitter and the registry disagree — since 5.3 merges wrappers per call site, a rebinding is unreachable by construction and the guard is an assertion — done 2026-09-14 after `openspec archive` merged the requirements (+7, ~1). The engine first refused: the delta had renamed the base scenario "Multidex oracle profile unavailable", which it reads as a dropped scenario; the delta header was restored to the base name via `/opsx:update`. INV-INS-116 also lost its rhetorical "today"
- [x] 8.2 Verify by grep: `grep -cE '^- \*\*INV-INS-(10[4-8]|11[67])\*\*' openspec/specs/instrumentation/spec.md` = 7; each of the seven ADDED requirement headers appears exactly once; "Ground-Truth Oracle Diversity for Equivalence Claims" appears once and carries the delta text; `openspec validate --specs --strict` green — 7 ids; each of the eight requirement blocks present once and equal to the delta; 10/10 specs valid

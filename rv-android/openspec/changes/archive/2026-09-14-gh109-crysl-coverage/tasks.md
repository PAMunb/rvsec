...
- [x] 6.5 [M2, at sync] Re-derive the two gh105-delta enumeration literals in the main spec during the manual `/opsx:sync` review (design Risks) — **discharged 2026-09-14 by the gh105 hand-sync**, which ran first: INV-INS-118 enumerates the set from the tree (47 `.mop`) instead of "holds 24", and INV-INS-137 hands the clause dispositions to the derived `predicate_ledger.csv` instead of "25 wireable — 24 wired"
- [x] 6.6 [M2, at archive] Hand-sync after `openspec archive`, which rebuilds only `## Requirements`: apply the delta's `## Invariants` (INV-INS-150…158, after INV-INS-148) and its `## Data Contracts` entry (`data/jca_android/coverage_matrix.csv`) to `openspec/specs/instrumentation/spec.md` in current-state wording (P4). Verify: `grep -cE '^- \*\*INV-INS-15[0-8]\*\*' openspec/specs/instrumentation/spec.md` = 9; the ADDED "Cipher Transformation Tables of the Successor Set" and the two MODIFIED gh104 requirements ("Allow-List Conformance to the Expert-Validated CrySL Rules", "Cipher Transformation Tables of the Archived Derived Set") equal to the delta; `grep -c 'Cipher Transformation Tables of the Derived Set$'` = 0; `openspec validate --specs --strict` green — **DONE 2026-09-14**: `openspec archive` merged +4, ~2 after the conflict was resolved via `/opsx:update` (the MODIFIED of the removed gh104 requirement became the ADDED successor-table requirement, and the two gh104 requirements the normalizer falsified received MODIFIED entries); INV-INS-150…158 appended after 148 (154 now reads "every specification of the set", 158 drops the task-7.3 pointer) and `coverage_matrix.csv` added to Output. Verified: 9 ids; the six requirement blocks equal to the delta; no "…of the Derived Set" header left; 11/11 specs valid; `test_gh109_*.py` 13 passed; `gh105_expert_ledger.py --check` exit 0; `gh109_coverage_matrix.py --check --require-complete` 49 rules (45 covered, 3 na-platform, 1 na-value)

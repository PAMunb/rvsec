...
| Cipher tables (successor set) | `CipherTransformationNormalizer` PBE branch + `alg` swap + AES_128 equivalence | monitor regen + diff harness checkpoint + `jca` freeze gate untouched |
...
- [Two open changes touching the same capability (gh105 unarchived)] → gh109's delta ADDs its requirements — among them "Cipher Transformation Tables of the Successor Set" — and MODIFIES the two gh104 requirements the normalizer falsified ("Allow-List Conformance to the Expert-Validated CrySL Rules", "Cipher Transformation Tables of the Archived Derived Set"), because gh104 removed the requirement the original MODIFIED targeted; archive order gh104 → gh105 → gh109. The gh105 delta asserts two enumeration literals this change falsifies ("the successor set holds 24 specifications"; "25 are wireable — of which 24 are wired"): at gh109's sync they are re-derived during the manual `/opsx:sync` review (task 6.5) instead of surviving as stale literals.

...
- [ ] 6.5 Smoke matrix gate (H4): run `rv-experiment run` with 5 APKs × 5 tools (Monkey, ape, aperv:sata_mop, droidbot, fastbot) × 60min timeout against the rebuilt image; record results. **DEFERRED 2026-05-06 (user direction)**: user will validate AVD bump separately via own experiment outside the gh55 archive scope. The gh55 §9 in-container smoke (cryptoapp + ape + aperv:sata_mop + dexlib2 + jca, 2/2 PASS, 18.75% MOP) demonstrates the rebuilt image is functional end-to-end on API 30 x86_64; the formal smoke matrix gate is no longer a pre-archive requirement.
- [ ] 6.6 Larger AVD sample gate: run `scripts/investigate_avd_compat.sh --sample 100 phase2 ...` on both datasets; confirm 0 regressions trend. **DEFERRED 2026-05-06 (user direction)**: subsumed by user's own experiment.
- [ ] 6.7 Update `docs/20260506_plano_env.md` Section 10 with larger-sample data after 6.6. **DEFERRED 2026-05-06 (user direction)**: depends on 6.6.
- [ ] 6.8 If gates 6.5 + 6.6 pass: **no tag bump** — the existing `phtcosta/rvandroid:0.8.0` is updated in place. If any tool fails on API 30, document and decide per-tool fallback (escalate as separate change if needed); rollback path is a rebuild with `--build-arg API_LEVEL=29 --build-arg ARCHITECTURE=x86`. **DEFERRED 2026-05-06 (user direction)**: depends on 6.5/6.6. Image already rebuilt in place at 21:28 (gh55 §9 smoke pass); fallback recipe documented above stays valid for any future regression.
...
- [x] 8.7 Run `/opsx:verify` to confirm implementation matches spec deltas. **Verified 2026-05-07**: 0 CRITICAL, 0 WARNING, 3 non-blocking SUGGESTIONS (deferred-tasks counter, gh-tbd directory disposition per user direction "nao mexer", pre-existing flake8 line-length out of gh55 scope). All 4 spec deltas (core, experiment, platform, tools) have implementation evidence; 4-smoke matrix verified §9.6 fix end-to-end.
- [x] 8.8 Update PR/CHANGELOG noting BREAKING removed env vars (`RV_MEMORY_FILE`, `RV_RVANDROID_URL`, `RV_SKIP_EXPERIMENT`, `RV_JCA_SPEC`) and the AVD bump (`API_LEVEL=30`, `ARCHITECTURE=x86_64`). **DEFERRED 2026-05-07 (user direction)**: no PR opened ("nao precisa de pr, faz direto"); no CHANGELOG file exists in this repo. BREAKING surface is documented in `proposal.md` "Breaking changes for consumers" section + `gh55/design.md` and visible in commit history (commits `ba2c1aa6`, `1fd6523c`, `c94ecfa3`, `01fe1c8e`, `675867dd`, `849169d8`, `5f9d74be` on the `modules` branch).

...
| **Boolean negation inversion (caught in 8.5 code review)** | Mitigated for Docker via §9.6 entry-point translation; broken for standalone | Click `envvar=` resolves the env value to the option's POSITIVE dest, not to the negation. For paired flags `--generate-monitors/--skip-monitors` (dest=`generate_monitors`), setting `RV_SKIP_MONITORS=true` would set `generate_monitors=True` — the OPPOSITE of intent. Affected: `RV_SKIP_MONITORS`, `RV_SKIP_INSTRUMENT`, `RV_SKIP_STATIC_ANALYSIS`, `RV_SKIP_EXECUTION`, `RV_NO_QUARANTINE`. Mitigation: `docker-entrypoint.sh` translates these 5 env vars to explicit negative CLI flags before `exec rv-experiment run`, which override the envvar-resolved positive value via Click's CLI > env precedence. Outside Docker (standalone `uv run rv-experiment run`), the inversion remains a known broken behavior — falls under the "Standalone module execution" gap above and is fixed by the follow-up change. The 9 unit tests in `test_cli_envvar_precedence.py` cover only string-typed flags (`RV_TOOLS`, `RV_TIMEOUTS`, `RV_INSTRUMENTATION_VARIANT`); the inversion was caught by the `/rv-code-reviewer` Skill during Group 8.5, not by the existing tests or by smoke 6.5 (which set no `RV_SKIP_*` vars). See task §9.6 for the entry-point fix. |

...
> **Closed unimplemented on 2026-09-15 by researcher decision:** rv-agent will not be used and is to be deprecated. Every open task is dropped, not done; no code was written and delta specs were not synced to the base.
...
- [ ] 1.1 Write failing tests: `rvagent-tool` maps `task.result.logcat_file` → `RVAgentConfig.logcat_feed_path`; core imports no rv-platform types (`test_toolmaps_logcat_path`, `test_core_has_no_platform_import`) (INV-AGT-60)
  - **DROPPED 2026-09-15** — change closed unimplemented (see the note at the top).

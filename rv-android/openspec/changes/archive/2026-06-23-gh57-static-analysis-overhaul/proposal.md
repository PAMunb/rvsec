...
7. **Refactor `FlowgraphRebuilder.buildCallGraph()` to delegate virtual dispatch to `Scene.v().getCallGraph()`** (the SPARK CG already built by Soot), eliminating the second graph entirely. Edges to library classes quarantined by SPARK's `IGNORED_CLASSES` are recovered via a WTG-level bytecode-scan (analogous to `BUG-INV-ANA-19` for `directlyReachesTarget`). **Controlled by a runtime feature flag** `cgDelegation` (default `true`) for rollback without rebuild.

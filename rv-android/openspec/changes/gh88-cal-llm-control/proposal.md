...
- **`experimento-cal/` + `calibracao/` scaffold (R2)**: one deterministic, testable script per loop state; state transitions are agent-driven following the methodology, with fixed human gates G1–G4:
  ...
  - **smoke-gate**: smoke compose (4 APKs × extreme arms, 90s, 1 rep) plus a checker: `[APE-LLM-CONFIG]` equals the manifest field-by-field per arm, the served model proven correct against SGLang's `/v1/models` endpoint (not the client-side `[APE-LLM-CONFIG-ACK]` echo), coverage > 0, 0 VerifyError.

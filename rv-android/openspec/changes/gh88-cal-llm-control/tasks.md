...
- [x] 2.6 Fix filters bind-mount: the generator mounted a per-phase subdir (`./filters/<phase>`) while filters are written flat to `iterN/filters/`, so `RV_APKS_FILTER` resolved to a missing file inside the container (the run/smoke would break). Mount the flat `./filters` dir (drop the now-unused `phase` param — `iterN/` is already per-phase, no subdir needed) and add regression test `test_filter_mount_resolves_to_written_files` cross-checking each container's `RV_APKS_FILTER` against the mounted filters dir (the gap `test_compose_rotation_and_filters` never covered)
- [x] 2.7 Single-source model path (iter0 smoke finding): derive the `sglang` `--model-path` in both composes from `phase["expected_server_model"]`, removing the hardcoded `Qwen/Qwen3-VL-4B-Instruct` literal in `gen_iteration.py`, so `cala.json` is the one source feeding both the model loaded on the GPU and the manifest `expected_server_model` (they cannot drift; changing the model is one edit). The rendered sglang command MUST stay byte-identical for the BASE model so `docker compose up` does not recreate/reload an already-running sglang container. Add regression test asserting the composes' `--model-path` equals the manifest `expected_server_model`
...
- [x] 3.5 Fix smoke model-proof (iter0 smoke finding — 12/12 tasks failed only the server_model assertion: a1/a8 observed `default`, a3 observed None, while SGLang `/v1/models` served `Qwen/Qwen3-VL-4B-Instruct` with real inference + cov>0): prove the served model via SGLang `/v1/models` == manifest `expected_server_model` instead of the ape `[APE-LLM-CONFIG-ACK] server_model` echo (which reports the client-side `llm_model="default"` sentinel, not the served model). Demote the ACK `server_model` to informational logging (never an abort cause). `/v1/models` reachability: publish port 30000 on the smoke compose OR read `served_model_name` via `docker logs sglang-server`. Update `test_smoke_config_ack_field_by_field` and add a served-model-proof test (served==expected PASSES even when the ACK echoes `default`; served!=expected FAILS). Leave `tool.py` `llm_model="default"` unchanged — it is the required OpenAI `model` request field and the deliberate "serve the single loaded model" sentinel
...
- [x] 5.5 Fix smoke contamination in consolidation (iter0 CONSOLIDATE finding — `consolidate_cal.py` globbed `results/*/*/tasks.json` including the `cala_smoke_*` containers; the 12 smoke identities `(cal_a1/cal_a3/cal_a8 × 4 APKs, rep 1, timeout 90)` survived identity dedup because `timeout` is part of the identity, but `build_per_apk_paired` groups by `(apk, arm)` **without** `timeout`, so each of those 12 cells averaged the 90s smoke run together with the two 300s reps — consolidated count was `892 = 880 + 12`): filter consolidation to identities whose `timeout` equals `manifest["timeout"]` (single source), dropping smoke. VERIFY (`verify_iteration.py`) SHALL apply the SAME run-timeout scope by its OWN manifest read (INV-CAL-04: no import of `consolidate_cal.py`), else it re-derives over the 892-identity set and flags the 12 smoke-arm cells as spurious divergences. Add regression tests `test_consolidate_excludes_smoke_timeout` (one 90s identity + two 300s reps for one `(apk, arm)` → only the 300s reps feed the cell, consolidated count excludes smoke) and confirm VERIFY re-derives over the same scope. Re-run consolidate on iter0 (must report 880 identities / 40 APKs / 11 arms) then VERIFY (must be admissible, 0 divergences)

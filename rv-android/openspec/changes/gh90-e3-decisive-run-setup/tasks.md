...
- [ ] 5.5 Widen the capture grace window in `tool.py` from `timeout_seconds + 15` to `timeout_seconds + 45` (the call site is `Command("adb", cmd_args, timeout_seconds + 15)` at `tool.py:991`), with an inline comment stating it is a hypothesis about censored teardown durations, not a measurement. The spec change is the MODIFIED `ApeRVTool Execution Flow` (step 7), which also carries the `RVToolTimeoutError` contract — grep the `aperv` spec for `+ 15` and confirm no upstream statement of the old value survives

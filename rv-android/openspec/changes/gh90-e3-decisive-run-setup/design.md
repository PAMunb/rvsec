...
| Snap Tolerance Gating (INV-APV-34) | Paired declaration in the arm dictionary (`expected_jar_git_sha` + `expected_jar_sha256`); observed `jar_sha256` compared at smoke time | `test_tolerance_without_declaration_fails`, `test_dangling_jar_sha_declaration_fails`; smoke gate 6.3 |
...
Each contrast is single-factor because arm 2 differs from arm 1 only in the MOP keys and arm 3 differs only in the LLM keys plus the two inert jar-provenance declaration keys of D4, which never reach `ape.properties` and so cannot move behaviour — a property the guard tests assert by diffing the dictionaries rather than by trusting review. Arm 1 is configurationally `sata_mop_act_frontier` (the ANC2 anchor), so the reference is the configuration that already won the previous multi-arm comparison and no new baseline is invented.
...
### D4 — B3's gate is a declaration plus a verification against the installed binary

The obvious mechanism — open `ape-rv.jar` and read a stamped capability entry — **cannot work**. Neither can the runtime banner an earlier draft of this design assumed. Both dead ends are recorded because both are easy to re-derive wrongly.

`ape-rv.jar` carries no build provenance at all. Verified in the `ape` worktree at HEAD `5dcf2259`: `grep -rl "APE-BUILD" src/` returns nothing, `find . -name "BuildInfo*"` returns nothing, and `grep -c "git-commit-id" pom.xml` returns `0`. The change that would have added the stamp — `gh14-build-provenance-stamp` — was **archived without implementation** on 2026-06-21 (`6b7b96f`), superseded by B-1 (build-ship-integrity): shipping the current source into the image makes a runtime banner redundant with build-time provenance (pinned `APE_REF` + image label). `INV-BUILD-09` and `INV-BUILD-11` therefore describe a design that was never built, and `ape/openspec/specs/build/spec.md` stops at INV-BUILD-08. There is no `[APE-BUILD]` line to parse and no reason to write a parser for one.

What *is* readable is the binary itself. `_capture_llm_provenance()` already digests the jar as pushed (`jar_sha256`) for post-hoc correlation, and that digest becomes the gate's verification half. It answers precisely the question the gate needs answered — *is the jar that ran this run the jar the arm was configured for* — and it answers it from the artifact rather than from a claim the artifact makes about itself.

The gate is therefore split across the two moments where each half is actually possible:

| Moment | Mechanism | Enforced by |
|---|---|---|
| Configuration | The arm carrying `llm_snap_tolerance_px=150` also carries `expected_jar_git_sha` (which source revision) and `expected_jar_sha256` (which built artifact); all three present or all three absent | Guard test in the suite |
| Verification | The `jar_sha256` captured at run start equals the arm's `expected_jar_sha256` | Smoke gate 6.3, before the decisive run |

The two declared fields answer different questions and neither substitutes for the other. `expected_jar_sha256` is verifiable and is the gate; `expected_jar_git_sha` is not verifiable from the binary and is documentary — it is what lets a reader of the results get from a digest back to source. Recording only the digest would leave the decisive run's provenance an opaque 64 hex characters.

**The honest limitation, stated because it is the reason this mechanism was once rejected**: a sha256 pins an *artifact*, not a revision. The `ape` build is not bit-reproducible, so rebuilding the same HEAD yields a different digest and any rebuild invalidates the declaration. Two things make that cost acceptable. The failure is not silent: the gate names the expected digest, the observed digest and the declared git sha, so a stale declaration reads as "the jar was rebuilt, or a different jar is installed" — the correct diagnosis and the correct action. And the alternative actually on the table is not the banner, which does not exist, but *no runtime verification at all*, relying on B-1's pinned `APE_REF` and image label. That is sound for the containerized campaign and unavailable for a host-driven smoke, which is how this smoke runs.

Alternatives rejected: applying 150 unconditionally and checking afterwards (the bad combination would already have consumed the decisive run); asking the sister change to add a resource stamp (`d8` converts only `.class` entries, so a `.properties` bundled into the intermediate jar is dropped from `ape-rv.jar` and `getResourceAsStream` returns `null` on device — this is why gh14 reached for a generated constant in the first place); reopening `ape` #14 to implement the banner (a new change in the other repository, outside the scope of `telemetry-proof-llm-efficacy`, re-deciding a settled author decision).
...
**The residual is closed: the author pre-committed the `n_disc ≤ 3` branch on 2026-08-01, before the probe ran.** The open question was real — declaring the probe in advance blocks peeking on its *result*, but does not by itself block that result from feeding back into the *choice of outcome*, and the probe measures the same estimand as the primary contrast (same arms, same binary outcome, only the budget differs). What closes it is pre-commitment: at `n_disc ≤ 3` the primary and secondary outcomes swap — the paired Δ of `mop_unique` under Wilcoxon signed-rank becomes primary, the binary McNemar becomes secondary — and nothing else in §4 moves.

Three things make that a rule rather than the feedback it was flagged as. It is a deterministic function of a number that does not yet exist, so no result is being read into it. Both outcomes were already declared in pre-registration §3 before any datum, so the probe selects which of two pre-declared outcomes decides, not what gets measured. And the branch is forced by arithmetic rather than chosen: at n ≤ 3 the exact two-sided McNemar's floor is 2·(0.5)ⁿ ≥ 0.125, so it cannot reach Holm α=0.025 whatever the data say — more runs cannot buy power for that outcome, and the §8 fourth arm would change the contrast instead of powering the RQ-C1 one.

What remains is smaller, and is recorded rather than solved: an adaptive rule still stakes some of the design's credibility on the probe being a clean diagnostic. That is why task 7.7 keeps the probe's journal stamp separate from the freeze stamp — the order of the two events stays inspectable, so a reader can verify the rule was written before the number existed.
...
| Observed `jar_sha256` differs from the arm's `expected_jar_sha256` | Smoke gate | Fail before the decisive run, naming both digests and the declared git sha | Reinstall the declared jar, or re-record the declaration after a deliberate rebuild |
...
- [The decisive run's primary outcome may be insensitive: `mop_unique` saturated at 4.12–4.41 across all eleven iter0 arms, and 73% of distinct violations appear within 10 s of launch, before exploration begins] → out of scope for this change, which defines arms rather than the analysis plan; recorded here because it conditions how this change's output is read. The registered response is `docs/20260730_preregistro_corrida_decisiva.md`, which makes the binary per-app outcome (exact McNemar on "found ≥1 violation") the primary and demotes the continuous delta to secondary — chosen because it measures the transition that matters, **not** because it has more to see than the continuous delta. The pre-registration's 2026-07-31 revision retracts that power argument explicitly: under the binary definition the corpus is *more* concordant, not less (33/40 whole-corpus, 35/40 among the `aperv` arms, against 25/40 for identity of the continuous value). The choice rests on meaning, and the ordering is not unconditional — D10's `n_disc ≤ 3` branch swaps the two.
- [B3's gate is declarative at configuration time, so a wrong declaration is only caught at smoke time] → the jar is digested at run start, before the first step, so a mismatch surfaces within seconds of the smoke starting and costs no decisive-run wall-clock; and the guard test makes the far more likely error — tolerance and declaration drifting apart — a suite failure. The residual exposure is a rebuild of `ape` *between* the smoke and the decisive run, which invalidates a digest the smoke already accepted; nothing in this change prevents that, and the mitigation is procedural — do not rebuild between the two.
...
- [The Python half depends on a jar built in another repository, and could sit blocked behind it] → every item here is unit-testable offline against recorded iter0 data and needs no device, so the Python half was complete and green before the jar existed. Only the cross-repository gates of group 6 need the binary.

...
The procedure, for whoever runs that first experiment: read the `emulator/boot` bucket from `rv_status.py` and the `elapsed` values from the container logs. If boot `TimeoutError`s appear, the gate is working and the budget is what rises — raise `RV_EMULATOR_BOOT_TIMEOUT`, do not weaken the gate.
...
1. **`RV_APK_INSTALL_TIMEOUT` default.** There is no current value to preserve — `adb install` has no timeout at all — and no measured distribution of install durations. An initial value must be chosen conservatively enough that a slow but healthy install on a contended host is not killed. With no contention run commissioned (D9), it is settled by judgement rather than measurement — the single install the E2E performs is a sanity check, not a distribution. Err long: killing a healthy slow install is the worse failure, and the value is adjustable per host without a rebuild. Record the chosen value and the reasoning here and in `.env.example`. The `core` delta deliberately states the obligation without a number, so it does not point back here for a value this question has not yet produced.
2. **Carrier for the ADB reason (C6).** Whether `EmulatorManager.install_app` keeps `bool` and attaches the reason to a raised exception, or returns a small result object. The spec fixes the outcome (the reason reaches `error_message`), not the mechanism. `EmulatorComponent.install_app` currently catches everything and returns `False`, so it changes either way. The API Design section above shows today's signature for orientation only — it does not pre-select the `bool` branch.
...
4. **Scope of the INV-CORE-43 sweep.** `adb root` at `android.py:283-291` is a known instance and is in scope. Whether the sweep extends beyond `util/android/` in this change, or is bounded to it with the remainder tracked separately, is decided when the grep results are in.

...
- [x] 8.1 Delete `expected_jar_git_sha` and `expected_jar_sha256` from `get_variants()["mop_on_llm_70"]` in `modules/aperv-tool/src/aperv_tool/tools/aperv/tool.py`, with the comment block that justified them. Rewrite the `llm_snap_tolerance_px` comment to current state (P4): why the radius is raised, without the sentence making it conditional on a declared build — the dead-pair-ban rationale stays, the pin does not
- [x] 8.2 Remove both names from `APERV_ORCHESTRATION_KEYS`, so `configure()` rejects them like any other unhonourable top-level key and experiment YAML cannot reintroduce the declaration through the DSL
- [x] 8.3 Drop the INV-APV-34 pairing sentence from the `llm_snap_tolerance_px` comment in `APERV_PROPERTY_MAPPING` (`:188-190`), and rewrite the `jar_sha256` paragraph of `_capture_llm_provenance()`'s docstring (`:924-928`): it records which binary ran, full stop — no "runtime half of the B3 gate", no comparison against a declared digest

...
| §4.RW `*` (match-any) in `call()` return | (unmeasured — audit missed the `*`-return position) | **ADDED — 12th closure, apply-discovered 2026-05-30** (verified SILENT-GAP: `matchCall`'s exact return-equality gate rejected every `call(* ...)` site; `toDescriptor("*")` → `Ljava/lang/*;`). Fix: `matchCall` skips the return gate when `"*".equals(cp.returnType().trim())` — surgical, in `matchCall` ONLY, symmetric to args/name `*` handling; `toDescriptor` untouched. Prerequisite for §4.O/§4.X/§4.V/§4.TT/§4.AT/§4.N to fire on generic/generic_new end-to-end (previously exercised only by concrete-return substitutes). | `CALL_RETURN_WILDCARD` = 240 generic / 67 generic_new / 0 jca; `CallReturnWildcardGrammarTest` |
...
**Closure count: 12 → 11** (§4.{O,N,V,X,TT,AT,Y,T,B,D,I}). **LOC: ~470-560** (~435-525 weaver/engine + ~35 `rvsec-core`). **Apply-discovered (2026-05-30): +§4.RW `*`-return wildcard → 12 closures** (§4.{O,N,V,X,TT,AT,Y,T,B,D,I,RW}); ~8 LOC matcher fix.
...
- `if(...)` AspectJ PCD against always-match matcher (`PointcutMatcher.java:109-114`): aspect=0, jca=0, generic=0, generic_new=8. **Round-8 correction**: round-7's design.md previously claimed 8/16/0/37 — that count conflated AspectJ-PCD `if(<expr>)` clauses with Java-statement `if (cond) { ... }` blocks inside advice bodies. The real PCD count uses the composition-aware regex `(?:^|[&|(])\s*if\s*\(` which yields 0/0/0/8 source (3 at pipeline level after JavaMOP merge). The §4.I closure lowers the guard **in-weaver** (round-11 R11.5 — `IfGuardEmitter` 2-shape lowering of `o==null`/`!Thread.holdsLock(o)`); the round-8 `MonitorRuntime.evaluateIf(ifId, args)` runtime-helper delegation (D13 below) is **RETIRED** — `evaluateIf`/`ifId` exist in neither fork. See §4.I + INV-INS-98.
...
│  │ dex-mutator (round-11 D9: 11 demand- │     production code   │
...
| `grammar-tests/` Maven submodule (NEW) | Executable oracle: one ENABLED test per matrix row (COVERED asserts post-fix behaviour; EXPLICIT-NO-OP asserts UOE; NOT-NEEDED α/β assert demand counters); bidirectional integrity check via `MatrixIntegrityTest`. Post-round-8: zero `@Disabled`, zero SILENT-GAP rows | Synthetic fixtures (and snippets from JCA `.mop` where realistic) | JUnit report; green bar, zero skips |
...
| `pointcut-engine` / `advice-emitter` / `dex-mutator` (MODIFIED) | Existing instrumenter — exercised by `grammar-tests/` AND extended in-change by the eleven round-11 demand-driven closures (§4.{O,N,V,X,TT,AT,Y,T,B,D,I}) incl. the `NamedRefPC` resolver (§4.D) + the §4.D.0 `commonPointcut` composition in `DexWeaver` | DEX bytecode + pointcut expressions | Instrumented DEX with corrected behaviour for the eleven flipped rows |
...
| INV-INS-93 (demand counts via portable Java; `.aj`+`.mop` scan) | `DemandCounter` | `testSourceDemandCountsReproducible` + `testPipelineDemandCountsReproducible` |
...
| Positive `within(typePattern)` matcher (R5 — **round-10 AB / round-11: NOT-NEEDED β**, absorber `coverage-weaver`; `WithinPC.java` NOT modified) | NOT shipped — the sole positive-`within` consumer is `Coverage.aj`'s `excludedPackages()`, absorbed by `coverage-weaver` | `WithinPositiveGrammarTest.withinPositiveAbsorptionAssertion` (path-β assertion) |
...
- **Re-running the full 190-APK validation pipeline.** The bipartite smoke validation (post-implementation) is sufficient evidence of behavioural correctness for the eleven round-11 closures; a full re-run is a separate experiment.
...
- ~~**Verifying that `BaseAspect.notwithin()`'s effective filter is applied by the weaver.**~~ **(round-11: NOW IN SCOPE)** This was a round-7 non-goal, but the §4.D.0 `commonPointcut` composition + §4.B/§4.D closures make applying the exclusion filter an in-change deliverable: `DexWeaverCommonPointcutCompositionTest` asserts a `mop..*` class yields zero matches despite a matching call-site signature. Verifying the filter is applied is required — without it §4.B/§4.D would be inert.
...
**(round-10 — superseded by round-11 ~470-560 LOC, per the banner above):** The scope shrinks further to **~565-660 LOC** (round-10: round-9 minus ~230 LOC §4.E AA-decision minus ~80 LOC §4.W AB-decision plus ~30-50 LOC §4.Y Signature delivery AC-decision; see `EMPIRICAL-DEMAND.md` for the per-row evidence chain). Round-11 then removed §4.R and the D13 monitor-builder helper and reshaped §4.Y to the `rvsec-core` substrate → ~470-560 LOC. The bisect-friendly atomic-commit discipline (one closure per commit) and the matrix integrity tests are preserved.
...
### D16 — Hoist `commonPointcut` evaluation to per-class + cache the exclusion/subtype machinery (VALIDATION-DISCOVERED 2026-05-31, §6.S smoke gate)

**Problem (proven during §6.S validation):** the §6.S ≥10-APK smoke gate exposed a severe performance regression that the unit/grammar tests could not surface (they weave tiny synthetic fixtures, not real 20-60 MiB multi-DEX APKs). On the JCA-226 corpus, one 27.5 MB / 5-DEX APK (`io.github.eucsoh.android_9`) ran **43+ minutes CPU-bound and never finished**; several others took 22-28 min. The pre-gh62 build instrumented the *same* corpus in 1-12 min/APK (all 190 finished). Two live `jstack` thread dumps of the stuck JVM + a 4-agent forensic investigation (code/git/runtime-profile/descriptor) isolated **two sequential CPU-bound phases**, one of them a gh62 regression:

- **P2 — the gh62 regression (introduced by §4.D.0 commit `3af5b3aa`).** `DexWeaver.weave` composes `effective = new CombinedPC(AND, commonAst, pe)` and matches it **per (instruction × advice)** — for the JCA descriptor that is *instructions × 115 advices* match calls per APK. `commonAst` (`!within(RVMObject+) && !adviceexecution() && BaseAspect.notwithin()`) reaches `matchNamedRef`, which calls `BaseAspectExpander.expand(baseAspectExclusions)` — **rebuilding a 23-node `NotWithinPC` AND-tree (the canonical twelve exclusions) from scratch, with no cache, on every single match call**, then walking it through `matchNotWithin → fromDescriptor` (String.replace) and `mergeBindings` (LinkedHashMap copy). The two jstacks land exactly here (`matchCombined/matchNamedRef` → `BaseAspectExpander.expand`, leaves at `mergeBindings`/`fromDescriptor`). Pre-`3af5b3aa` the weave loop matched the bare advice `pe` (no commonAst AND-compose, no `NamedRefPC` path) — `matchNamedRef` and `BaseAspectExpander` did not exist (git blame: all `3af5b3aa`).
- **P1 — pre-existing (gh52 `b7dfece4`/`1c918e84`), NOT the regression but a real inefficiency.** `DexWeaver.expandWrapperReplacementsForApk` → `InheritanceResolver.subtypesOf` → `AndroidClassIndex.load` **reopens a `ZipFile` per ancestor-walk step** (uncached). `AndroidClassIndex` and `InheritanceResolver` are byte-identical PRE vs POST gh62. This runs once per APK before the weave; the old run tolerated it.

**Key correctness insight:** `commonAst`'s verdict is a pure function of `classDef` — `!within`, `!adviceexecution()`, and the `BaseAspect.notwithin()` exclusion clauses all read only the class type and **bind nothing** (every clause returns `Match.empty`). It is therefore **invariant across the method / instruction / advice loops**; re-evaluating it per (instruction × advice) is pure waste, and hoisting it cannot change which join points match (AND is associative; the advice side carries all bindings).

**Choice:**
1. **(P2.1) Evaluate `commonAst` once per class.** At the top of the `for ClassDef` loop, run the commonPointcut class-gate exactly once, reusing the existing class-level probe pattern (`DexWeaver.staticInitMatchesClass` already does this for §4.Y: synthetic `<clinit>` probe + `matcher.match`). If the class is excluded, `continue` — skipping every method/instruction/advice in it. If it passes, the per-(instruction × advice) loop matches **only the advice `pe`**, dropping the per-iteration `new CombinedPC(AND, commonAst, pe)` (both at the main loop and the §4.Y staticinit pre-pass).
2. **(P2.2) Memoize `BaseAspectExpander.expand`** keyed by the exclusion list (constant per weave) so the 23-node tree is built at most once.
3. **(P2.3) Short-circuit `mergeBindings`** when one side carries no bindings (`argBindings` empty and `targetRegister < 0`) — return the other side without the `LinkedHashMap` copy.
4. **(P1) Cache the opened `AndroidClassIndex` / `ZipFile`** (and/or memoize `subtypesOf`/`isAssignableFrom`) so the archive is not reopened per ancestor step; close it once at end-of-APK.

This converts per-APK cost from `O(instructions × 115 advices × 23-node-walk + allocs)` to `O(classes × 23-node-walk)` for the class-gate plus `O(matching-instructions × 115 × cheap CallPC owner-check)` for the advice match — restoring (and, with P1, improving on) the pre-gh62 baseline. **Semantics are preserved** (same join points matched, same bindings, same emitted bytecode); only redundant recomputation is removed.

**Why (not just "make it faster"):** the §6.S gate is the empirical contract that gh62's closures work end-to-end on real APKs; a weave that cannot finish a 5-DEX APK in bounded time makes the 190-APK validation campaign — and any production use of the dexlib2 variant on large apps — infeasible. The regression is purely non-functional (the matcher produces identical results, just astronomically slower), so the fix is a behaviour-preserving optimization, not a closure change. It is documented here (rather than deferred to a new issue) because it is a defect *in gh62's own §4.D code*, discovered by gh62's own §6.S gate, exactly as the §4.RW silent-gap was discovered and closed in-change during apply.

**Alternatives considered:**
- *Defer to a separate post-gh62 performance issue.* Rejected: the regression blocks gh62's §6.S gate and was introduced by gh62 §4.D; leaving it would archive gh62 with a known-broken empirical gate.
- *Only memoize `expand` (P2.2), skip the per-class hoist.* Rejected: memoization alone removes the 23-node *rebuild* but still walks the 23-node tree + allocates a `mergeBindings` map per (instruction × advice) — the per-class hoist is what removes the `× instructions × advices` multiplier.
- *Only fix P2, ignore P1.* Accepted as the minimum to unblock, but the user approved fixing both together; P1 caching is independent, cheap, and speeds all builds.
- *Cache `commonAst` match result in a per-class map inside `PointcutMatcher`.* Rejected per P1 (Simplicity): the hoist in `DexWeaver` (the loop owner) is clearer than threading a memo through the stateless matcher and avoids a cache-invalidation surface.
...
class IfGrammarTest { /* if(...) — COVERED §4.I via fork-free in-weaver 2-shape lowering (R11.5; D13 runtime-delegation RETIRED) */ }
...
| Demand counts diverge from `DemandCounter` | `MatrixIntegrityTest.testSourceDemandCountsReproducible` / `.testPipelineDemandCountsReproducible` | Fail naming the row and the actual count | Update the matrix |
...
| Document↔data link | Demand counts reproducible by `DemandCounter` | `testSourceDemandCountsReproducible` + `testPipelineDemandCountsReproducible` | 2 |
| Grammar coverage | One test method per matrix row, exercising the end-to-end pipeline; round-8 adds parameterized scenarios for every in-change closure (`within` positive simple, `T+` owner/return, `!target/!args`, `(T,..)` trailing-mixed, method-name glob, `target(Type)`, `args(Type)`, `staticinitialization` synthesis, `after throwing`, `BaseAspect.notwithin`, named-pointcut resolver, `if(...)` in-weaver 2-shape lowering) | The per-designator `*GrammarTest` classes | ~100-120 across all rows (closed-enumeration rows + 13 round-8 closure rows + parametric expansions + 8 robustness tests). **Round-8 removals**: ~30 substrate contract tests + 1 Coverage.aj end-to-end test + 1 FQN-remap test are NOT shipped (the underlying production code is also not shipped — see D10 SUPERSEDED). |

...
// AND ClassNameFilter.excludeClassNamePatterns(".*MatrixIntegrityTest.*"). Without the
// exclude filter, Launcher.execute(request) re-discovers MatrixIntegrityTest itself and
// recurses into testSkipCountEqualsSilentGapCount until StackOverflowError. A sentinel
// assertion inside the test verifies the discovered TestPlan contains zero TestIdentifiers
// whose source class is MatrixIntegrityTest, so a regression in the filter fails loudly
// rather than silently re-introducing the recursion bug.

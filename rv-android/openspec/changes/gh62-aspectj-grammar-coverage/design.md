...
| Scope Ledger for Future Closures (R3) | `openspec/changes/gh62-.../ledger.md` (snapshot at archive) | `MatrixIntegrityTest.testSilentGapRowsHaveDisabledTestAndLedgerEntry` cross-checks every gh62-time SILENT-GAP row against ledger. The test resolves the ledger path via active-then-archive fallback (`openspec/changes/gh62-...` then glob `openspec/changes/archive/*-gh62-...`); after gh62 archive the test continues to pass against the moved file |

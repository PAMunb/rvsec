...
- [ ] 0.6.2 Delete `rvsec-mop/src/main/resources/{jca,generic_new}/MultiSpec_1MonitorAspect.aj` (and any other stray `MultiSpec_*MonitorAspect.aj`/`.json`/`.java` in the `.mop` source dirs). The pipeline regenerates these into `out/`/`results/` per run; nothing reads the in-source copies. <!-- DEFERRED 2026-05-29: auto-mode classifier denied the irreversible cross-repo deletion of gitignored files without explicit per-file user direction. NON-BLOCKING — DemandCounter (§3.4) is scoped to *.mop + aspect/Coverage.aj only, so the strays do not pollute counts. Awaiting explicit user OK to `rm` the 2 named files. -->

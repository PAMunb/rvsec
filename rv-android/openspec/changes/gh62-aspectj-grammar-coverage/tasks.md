...
- [ ] 4.PERF.VAL Validate locally on `io.github.eucsoh.android_9.apk` (pure-JVM `instrument` subcommand, NO emulator), reusing pre-generated monitors from `data/results/instrument_jca190_gh62_06/instrument_jca190_gh62_06/monitors/`. Confirm: completes in minutes (not 43+), exit 0, 0 VerifyError, instrumented APK produced. Compare against a small control APK. Record the before/after wall-time.
- [ ] 4.PERF.COMMIT Commit each unit atomically (`fix(gh62): §4.PERF P2.1 hoist commonAst per-class …` etc.) with `refs #62`. LOCAL — user pushes.

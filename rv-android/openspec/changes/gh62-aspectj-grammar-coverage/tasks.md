...
<!-- This change is documentation + a new Maven test-only submodule + a CI PR-check.
     No Python module is touched. No production parser/matcher/emitter code changes
     (only a pom.xml smali property bump under §0).
     Execution order: smali bump (0) -> ledger draft (1) -> matrix scaffold (2) ->
     grammar-tests Maven module (3) -> per-designator test classes (4) ->
     matrix population (5) -> integrity tests + CI gates (6) -> archive (7).
...
- [ ] 1.3a Initial Fix-now bucket (subject to demand verification in §5): trailing-varargs `(T, ..)`; `T+` in `call()` owner; `T+` in `call()` return; `execution(...)` real matcher; `BaseAspect.notwithin()` named-ref expansion; **`after() throwing(...)` end-to-end** (re-classified from EXPLICIT-NO-OP — `DexWeaver.java:560-566` is silent, not documented); `target(Type)` type-matching; `this(name)`/`this(Type)`; `args(Type)` type-matching; `if(...)` semantic evaluation (18 demand in `generic_new/`).
- [ ] 1.3b Initial Follow-up bucket: `get(FieldPattern)`/`set(FieldPattern)` (zero demand after corrected grep, but completeness); `withincode(...)`; `cflow`/`cflowbelow`; `initialization(...)`/`preinitialization(...)`; AspectJ 5 `@*` family (6 designators); positive `within(...)` weaver-side filter; `T+` inside `!within(...)`; SignaturePattern modifiers; advice forms `before`/`after`/`after returning` are COVERED so don't appear here.
- [ ] 1.3c Initial Deferred-by-design bucket: `around` advice (assertion in `EmitterDispatchTest.java:54-59`); `handler(...)` (no DEX-level analogue in scope). Note: `lock`/`unlock` are NOT in this bucket because they are NOT matrix rows at all (design.md OQ3 — zero demand AND outside the closed enumeration). Ledger entries map 1:1 to matrix rows; non-rows do not appear in any bucket.
...
- [ ] 2.12 Commit on `origin/modules`: `docs(gh62): scaffold aspectj grammar coverage matrix (closed enumeration, TBD verdicts)` with `refs #62`. Push.
...
- [ ] 3.2 Create `rvsec-android/rvsec-instrumentation-dexlib2/grammar-tests/pom.xml`. Parent: `rvsec-instrumentation-dexlib2`. Test scope deps: `pointcut-engine`, `advice-emitter`, `dex-mutator`, JUnit 5 (already in the project's BOM), `commonmark-java` (for matrix Markdown parsing — replaces a custom parser). No `main/` source directory; only `src/test/java/` and `src/test/resources/`.
...
- [ ] 3.6 Add `grammar-tests/src/test/java/br/unb/cic/rv/grammar/util/AspectJDesignators.java` — a `Set<String> DESIGNATORS` constant naming every entry in the closed enumeration declared in the delta spec. Source of truth for `MatrixIntegrityTest.testEveryDesignatorHasMatrixRow`.
...
- [ ] 4.8 Add `IfGrammarTest`. Methods: `ifAlwaysMatch_emitterIsArchitectureOfPaper` (`@Disabled` — `IfGuardEmitter.java` is scaffolding, no guard evaluation; 18 demand in `generic_new/` → Fix-now).
...
- [ ] 4.13 Add `AdviceExecutionGrammarTest`. Method: `adviceExecutionAlwaysMatch` — COVERED semantically.
...
- [ ] 4.16 Add `AdviceFormGrammarTest` (NEW per cross-LLM review). Methods: `beforeAdvice` (COVERED via `BeforeEmitter`), `afterAdvice` (COVERED with weaver-side ressalva — see Deepseek analysis), `afterReturningAdvice` (COVERED via `AfterReturningEmitter`), `afterThrowingAdvice` (`@Disabled` SILENT-GAP — `DexWeaver.java:560-566` discards the plan silently), `aroundAdvice` (EXPLICIT-NO-OP — `EmitterDispatch.java:61-65` throws `UnsupportedOperationException`, asserted in `EmitterDispatchTest.java:54-59`).
...
- [ ] 4.20 Run `mvn -pl grammar-tests test`. All non-`@Disabled` tests SHALL pass; the test report SHALL list every `@Disabled` skip with its `gh62 SILENT-GAP:` reason.
- [ ] 4.21 Commit on `origin/modules`: `test(gh62): per-designator grammar test classes (stubs + @Disabled SILENT-GAP)` with `refs #62`. Push.
...
**Goal**: add `MatrixIntegrityTest` so the matrix↔code↔ledger consistency is enforced at every CI run, per INV-INS-88..93. Add a PR-check GitHub Action so production code changes in the relevant submodules are blocked without matching matrix updates.
...
- [ ] 6.11 Add `rv-android/.github/workflows/grammar-pr-check.yml`: a PR-check that fails if a PR modifies any of `rvsec-android/rvsec-instrumentation-dexlib2/{pointcut-engine,advice-emitter,dex-mutator,coverage-weaver}/src/main/` WITHOUT modifying `docs/aspectj_grammar_coverage.md`. Operational enforcement of closure atomicity (replaces the aspirational INV-INS-92 from the earlier draft).
- [ ] 6.12 Commit on `origin/modules`: `test(gh62): MatrixIntegrityTest (bidirectional) + CI step + PR-check for grammar coverage` with `refs #62`. Push.

...
Fidelity is verified against a **golden dataset** (20 screenshots with Java intermediate
outputs at each pipeline step).
...
Generate fixtures from the APE Java pipeline for 20 screenshots (cryptoapp + 4 other apps).
For each screenshot, capture intermediate outputs at every pipeline step. Python tests validate
against these fixtures using the tolerance table in `design.md`.

**Java commit must be pinned** in `tests/fixtures/golden/README.md` to ensure reproducibility.
If Java code changes after fixture generation, re-generate and update.

### Golden Fixture Contents (per screenshot)

| Step | Output | Tolerance |
|------|--------|-----------|
| Resize | `(orig_w, orig_h, new_w, new_h)` | Exact |
| JPEG | First 100 chars of base64 + SHA256 hash | SSIM >= 0.98 |
| Prompt | System message + user text (full strings) | Exact |
| Widget list | Formatted string | Exact |
| Matching | Step name, matched widget, distance | Same widget (±1px Euclidean) |
...
| `tests/fixtures/golden/` | Java pipeline outputs (20 screenshots) | Tier 1 |
...
Group 0: Golden Fixture Preparation (prerequisite)
  Pin Java commit, generate fixtures from APE Java for 20 screenshots
...
- [ ] Golden fixtures generated from pinned Java commit (20 screenshots)

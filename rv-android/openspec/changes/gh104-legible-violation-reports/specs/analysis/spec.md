...
`LogcatParser` (`modules/rv-coverage/src/rv_coverage/parser/log/logcat_parser.py`) is the only place where a violation reported by a woven monitor becomes an `RvErrorLog`. Every downstream count of the thesis — `total_errors`, `unique_errors`, the `errors.csv` a campaign consolidates, the per-event attribution `campaign-analysis` performs — is computed over what this parser returns. Whatever it drops is invisible everywhere; whatever it fabricates looks like data everywhere. This delta makes both impossible to miss: the parser recognises the message envelope the `jca_v2` monitors emit, marks the fields it could not read with named sentinels, counts every line it does not turn into a record, and treats an unclosed quote as what it is — a record logcat cut in half.

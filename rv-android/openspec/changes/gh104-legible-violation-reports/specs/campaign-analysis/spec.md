...
- `CsvDiagnostics` — `rows`, `unique_msg_unparsed`, `unique_msg_disagrees` (the CSV's own `code`/`event` columns differ from the parts of `unique_msg` on the same row) (destination: gates, envelopes' `exclusions`).
...
- **WHEN** `read_errors_csv` opens a file whose header is `apk,rep,timeout,tool,time,spec,class,method,source,code,event,message,unique_msg` and whose one row has `unique_msg=okhttp3.internal.tls.X:::get:::TrustManagerFactorySpec:::UnsafeAlgorithm:::TMF-ALG-01:::g3:::v=1 code=TMF-ALG-01 ev=g3 obj=TrustManagerFactory val='X509' exp='PKIX,SunX509' msg='expecting one of PKIX,SunX509 but found X509'`, `code=TMF-ALG-01`, `event=g3`
- **THEN** the frame SHALL have one row with `violation_type=UnsafeAlgorithm`, `code=TMF-ALG-01`, `event=g3`, `unique_message` equal to the envelope text, and `violation_time_s` from `time`
...
- **WHEN** a run's logcat carries, between heartbeat 3 and heartbeat 4, the `RVSEC` payload `TrustManagerFactorySpec,okhttp3.internal.tls.X,X,get,X.java:12,UnsafeAlgorithm,v=1 code=TMF-ALG-01 ev=g3 obj=TrustManagerFactory val='X509' exp='PKIX,SunX509' msg='expecting one of PKIX,SunX509 but found X509'`
- **THEN** step 3's `violations[]` SHALL hold one `ViolationEvent` with `code=TMF-ALG-01`, `event=g3`, `shape_ok=True`

...
- **AND** the alias row MUST appear in `data/jca_android/alias_table.csv` with `service=TrustManagerFactory`, and the transcribed allow-list of `TrustManagerFactorySpec.mop` MUST still read exactly `PKIX`, so the alias never enters the list it resolves against
...
- **AND** a Java test MUST assert that the class's table and `data/jca_android/alias_table.csv` hold the same 158 rows, `in_api30_allowlist` flag included, so a row added to one and not the other fails
...
- **THEN** it MUST read `Api30CipherTransformationUtil` (`rvsec-core/src/main/java/br/unb/cic/mop/jca/util/`), the class that `jca_android/CipherSpec.mop` names, not a `.mop` list
...
- **WHEN** `jca_android/TrustManagerFactorySpec` reaches event `init` (binding `mf`) on a factory obtained through `getInstance("SunX509")`, the report site of the specification (`TrustManagerFactorySpec.mop:55-57` in the seed: `g3` at `:44-49` only writes the field, `init` reports)
- **THEN** the message MUST be `v=1 code=TRUSTMANAGERFACTORY-ALG-01 ev=init obj=TrustManagerFactory val='SunX509' exp='PKIX' msg='expecting one of PKIX but found SunX509'`
- **AND** `val` MUST come from `mf.getAlgorithm()`, never from `currentAlgorithmInstance`
- **AND** `exp` MUST be the transcribed api30 list joined with `,`, so the message and the allow-list cannot drift apart
...
- **AND** the symmetric case — a trace carrying a value the seed admitted and the api30 transcription does not (`KeyPairGenerator` RSA `3072` or `DiffieHellman`, `TrustManagerFactory` `SunX509`, `SecureRandom` `NativePRNG*`/`Windows-PRNG`, `KeyStore` `PKCS11`/`JKS`/`JCEKS`/`DKS`, `Mac` `HmacPBESHA1`) — MUST be classed `introduced — corrected verdict (narrowed by <api30 file:line>)`, citing the clause, and not as a defect of the transcription

...
Read `design.md` D-5, the `instrumentation` delta `Requirement: Dedupe Identity of a Violation Report` (INV-INS-126), the `core` delta (INV-CORE-57, era declaration). Measure first (9.1); if the discontinuity is zero, stop and report — do not land 9.2.

...
- `tests/parity/test_gh104_specset_gates.py` — created here with `test_jca_android_hunks_all_recorded`, `test_jca_android_has_no_execution_context` and `test_predicate_removal_record_complete` (55 rows; class totals 10/7/1/3/9/25 asserted, summing to 55); Group 6 writes its structural gates in a separate file (`test_gh104_structural_gates.py`) to avoid a merge.
...
| IvParameterSpec | `c1` (`:24-26`), `c2` (`:33-38`) | guarda ×2 | `c1` was the predicate alone → the whole `condition()` goes; `c2` is mixed → strip the predicate conjunct and keep `offset >= 0 && len >= 0 && iv.length >= offset + len` |
...
| PBEParameterSpecSpec | `c1` (`:25-28`), `c2` (`:35-38`) | guarda ×2 | both mixed → strip the predicate conjunct and keep `iterationCount >= 10000`; dropping the whole condition would let a `< 10000` constructor fire `c1` (match) and `c3` (fail + `__RESET`) on one call — a changed, self-contradictory verdict |
...
And the `setProperty(...)` writes: every one of them — 46 sites in 19 of the 21 files (the two stream specifications only import `ExecutionContext`); 49 in the frozen `jca` — is deleted with its event body, one divergence entry per file, no row in `predicate_removal.csv`. **What happens to the declaration depends on the row, not on the body.** An event that stays in the `ere`/`fsm` keeps its declaration with an empty body (the generator needs no body). An event whose transition row is all-`fail` and whose only guard was the predicate — `IvParameterSpec.c3/c4`, `PBEKeySpecSpec.err2/err3`, `SecureRandomSpec.c3/setSeed3`, `SecretKeySpecSpec.c3` — has its **declaration deleted**: kept with an emptied `condition()` it would accuse on every legitimate call of its pointcut, and G-2 would clear it as `orphan-with-clause` through the rule's `REQUIRES` (design D-7 — on `jca_android` G-2 accepts `CONSTRAINTS`/`FORBIDDEN` only for exactly this reason). A guard whose `condition()` mixed a predicate with a real test (`GCMParameterSpecSpec` both `c1`, `PBEKeySpecSpec.c1`, `SecretKeySpecSpec.c1`, `IvParameterSpec.c2` — keeps `offset >= 0 && len >= 0 && iv.length >= offset + len`, `PBEParameterSpecSpec.c1/c2` — keep `iterationCount >= 10000`) keeps the real test; a guard whose `condition()` was the predicate alone (`IvParameterSpec.c1` `:24-26`, `SecureRandomSpec.c2` `:38`, `SecureRandomSpec.setSeed2` `:93`) drops the `condition()` and fires unconditionally, which is the accepting transition the rule intends. Dropping the whole condition of `PBEParameterSpecSpec.c1/c2` would let a `< 10000` constructor fire `c1` (match) and `c3` (fail + `__RESET`) on one call — a changed, self-contradictory verdict — which is why those two are mixed, not predicate-alone.

Also outside `condition()`: the **25 `setObjectAsInAcceptingState`/`unsetObjectAsInAcceptingState` calls** in 19 of the 21 files (e.g. `IvParameterSpec.mop:67`) — bookkeeping of the predicate machinery with no reader left once the propagators are gone; each is deleted and gets a `predicate_removal.csv` row of class `accepting-state`. And the **21 `import br.unb.cic.mop.ExecutionContext;` lines** and the comment at `MessageDigestSpec.mop:25` are deleted with them — divergence-record entries (folded into each file's entry), not rows.
...
**The record.** Every one of the 21 events above, plus the 9 `remove(...)` sites, plus the 25 accepting-state calls, gets a row in `data/jca_android/predicate_removal.csv` (**55 rows**: 21 + 9 + 25, and the class totals below sum to the same 55): `file,line,spec,event,class,lost_accusation,reason`, where `class` is one of `guard` / `total-loss` / `partial-loss` / `provenance` / `remove` / `accepting-state`. Seven rows are `total-loss` (`IvParameterSpec c3/c4`, `PBEKeySpecSpec err2/err3`, `SecureRandomSpec c3/setSeed3`, `SecretKeySpecSpec c3`), one `partial-loss` (`PBEParameterSpecSpec c3`), three `provenance`, ten `guard`, nine `remove`, twenty-five `accepting-state`; the 46 `setProperty` deletions, the 21 import deletions and the comment are divergence entries (one per file), not rows. `test_predicate_removal_record_complete` asserts the 55 and the six class totals. The `lost_accusation` column carries the sentence the site used to raise, verbatim, so that reopening the predicate question later does not require re-deriving this inventory from the seed. This record is what INV-INS-128 leaves in place of a `predicate_omissions.csv`, which this set does not carry.

Verification command after the pass:

```bash
grep -rn "ExecutionContext\|Property\.\|ExecutionContext.instance().validate(\|setProperty(\|AsInAcceptingState(" ../rvsec/rvsec-mop/src/main/resources/jca_android/   # empty — imports, comment and accepting-state calls included (NOT the bare `validate(` — KeyPairGeneratorSpec keeps its local `validate(int)`)
...
| `KeyPairGeneratorSpec:22` `safeAlgorithms` | `RSA, EC, DSA, DiffieHellman, DH` | `KeyPairGenerator.cryptsl:45` `alg in {DSA, DH, RSA}` + four keySize implications | the literal transcription would drop the seed's `EC`; task 2.6 keeps it against the rule and records the divergence |
...
- `backup/gh104-analise/alias_table_conscrypt_android11.csv` — **158 rows**, columns `service,alias,canonical,openssl_provider_line,in_api30_allowlist`: 149 in services the 21 specifications cover and 9 in services with no specification (`AlgorithmParameters` 8 = 5 yes / 3 no, `SecretKeyFactory` 1 yes), kept with the flag. **114 rows** carry `in_api30_allowlist=yes` (44 `no`), i.e. their canonical target is an entry of an api30 allow-list; of the 114, **108** are in covered services and resolve at run time, and 6 are `yes` against a rule the set does not carry.

The count is **114** `yes` (108 resolvable) out of 158. `Cipher.GCM -> AES/GCM/NoPadding` is **not** a `yes` row: `GCM` appears in the `Cipher` list as an operation *mode*, not as an algorithm, and the alias names a whole transformation, so a match on it is spurious. Write 114 / 108 into the README with that reason, so nobody re-derives the number later and thinks the table changed.
...
- `grep -rn "ExecutionContext" jca_android/` returns nothing; the frozen `jca` still returns 134; `predicate_removal.csv` accounts for all 21 events, the 9 `remove(...)` sites and the 25 accepting-state calls (55 rows; `test_predicate_removal_record_complete` green); `constraint_table.csv` exists with one row per api30 clause and per clause-less `.mop` test.
...
- `data/jca_android/alias_table.csv` is byte-identical to `backup/gh104-analise/alias_table_conscrypt_android11.csv` (158 rows: 149 in covered services + 9 `AlgorithmParameters`/`SecretKeyFactory` rows, services without a spec, kept with the flag; 114 `yes`, of which 108 are in covered services and resolve at run time, 6 are `yes` without a spec; 44 `no`); the sampled rows and their verification are named in the README; the two corrected pointers (`:115-116`, `:89-90`) appear nowhere in their old wrong form.

...
Tracked checkboxes: `tasks.md` §2. Wave 1; no dependency; Group 7 (E1) and Group 8 (E4) wait for this group's commit. This is the long pole of wave 1 — dispatch it first.
...
## Task 2.3 — the `ExecutionContext` inventory (24 events over 11 of the 21 files)

The frozen `jca` carries **134** `ExecutionContext` occurrences across the 23 files. Of the 21 that stay, **24 events** read a predicate in their `condition()`: **13 as a guard only** (removing the predicate is a net gain — today an unmet predicate makes the event fail its guard, and the generated method returns before the body and before the transition, so the specification goes silent) and **11 as the basis of an accusation** (removing it is a declared loss of detection).

| spec | event | role | consequence of removal |
|---|---|---|---|
| GCMParameterSpecSpec | `c1` (`:22`, 2-arg ctor) | guarda | ctor accepted on `validLengths` alone |
| GCMParameterSpecSpec | `c1` (`:33`, 4-arg ctor — renamed `c2` in Group 8) | acusa (via `@fail`) | the `RANDOMIZED src` accusation disappears |
...
| KeyPairGeneratorSpec | `init1`, `init2`, `initError` | guarda ×3 | the `preparedDH`-style reads go; the allow-list/keySize halves stay |
...
| SecretKeySpecSpec | `c3` (`:48-49`) | acusa | `!validate(RANDOMIZED, keyMaterial)` half lost — **and see task 2.4: the allow-list half has no base in api30, so this site loses both halves and disappears** |
| SecureRandomSpec | `c2`, `c3`, `setSeed2` | guarda ×3 | — |
| SecureRandomSpec | `setSeed3` (`:100-101`) | acusa | **the four-argument report site disappears entirely** (purely predicate-guarded) |

Plus, outside `condition()`: **the 9 one-argument `remove(Property)` sites** (`KeyGeneratorSpec`, `KeyManagerFactorySpec`, `KeyPairGeneratorSpec`, `KeyStoreSpec` ×2, `MacSpec`, `PBEKeySpecSpec`, `TrustManagerFactorySpec` ×2) all go, which also retires the deprecated `ExecutionContext.remove(Property)` API from this set. Context for the loss: the whole api30 corpus declares only **2** `NEGATES` blocks (`SecretKey.cryptsl`, `PBEKeySpec.cryptsl`), so nine removals were never proportionate to what the rules ask for.

And the `setProperty(...)` writes: every one of them, in every one of the 21 files, is deleted with its event body (an event whose body becomes empty keeps its declaration — the generator needs no body).
...
**The record.** Every one of the 24 events above, plus the 9 `remove(Property)` sites, gets a row in `data/jca_android/predicate_removal.csv`: `file,line,spec,event,class,lost_accusation,reason`, where `class` is one of `guard` / `total-loss` / `partial-loss` / `provenance`. Six rows are `total-loss`, two `partial-loss`, three `provenance`, thirteen `guard`. The `lost_accusation` column carries the sentence the site used to raise, verbatim, so that reopening the predicate question later does not require re-deriving this inventory from the seed. This record is what INV-INS-128 leaves in place of a `predicate_omissions.csv`, which this set does not carry.
...
The seed declares **13** allow-lists. Each one is replaced by the corresponding `CONSTRAINTS` set of `MetaCrySL/generated/api30/<Rule>.cryptsl`, transcribed literally, with one `conformance_record.csv` row naming the `.cryptsl` file, the clause and the anchor (design D-10: api30 for availability, 1.5.2 for recommendation).
...
cd ../rvsec && mvn -q test -pl rvsec/rvsec-core                   # the Cipher util's test AND the alias-table-equals-CSV test
```
...
- `grep -rn "ExecutionContext" jca_android/` returns nothing; the frozen `jca` still returns 134; `predicate_removal.csv` accounts for all 24 events plus the 9 `remove(Property)` sites.
- Every allow-list matches its api30 `CONSTRAINTS` clause modulo the declared normalisation, or has a conformance-record row saying why not — exactly two such rows are expected, the `EC` of task 2.6 and the four ECDSA algorithms of task 2.7; the MD5/SHA-1 cost of task 2.4 (5,892 of 6,048 rows) is recorded as a declared consequence, not as a divergence.
- `data/jca_android/alias_table.csv` is byte-identical to `backup/gh104-analise/alias_table_conscrypt_android11.csv` (158 rows, 114 `yes`); the sampled rows and their verification are named in the README; the two corrected pointers (`:115-116`, `:89-90`) appear nowhere in their old wrong form.
...
- gh101's five gates still pass; `data/jca_android/` holds the five records (headers + this group's rows) and the README with the archive paragraph, the normalisation rule and the alias table's limits.

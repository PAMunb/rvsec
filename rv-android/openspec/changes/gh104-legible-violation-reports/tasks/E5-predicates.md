# Group 8 — E5: predicates

Tracked checkboxes: `tasks.md` §8. After Group 7. Files: `rvsec-core` `ErrorType.java` (+ test), `jca_v2/*.mop` (predicate reads only), `jca_v2/codes.csv` (`REQ`/`FORB` rows), `data/jca_v2/predicate_omissions.csv`, `data/jca_v2/README.md`, `scripts/gh101_predicate_pairing_check.py` (parametrise by set) and its pytest.
...
- 8.1 `ErrorType` gains `RequiredPredicate`, `ForbiddenMethod` (`rvsec-core/.../eh/ErrorType.java`, today six values `:4-9`); Java test; `codes.csv` KIND `REQ`, `FORB`.

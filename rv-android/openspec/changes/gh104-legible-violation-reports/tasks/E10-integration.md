...
- 10.5 Read the device evidence against the pivot's publishable tier (Group 1 task 1.4). Three of its five rows are what the api30 transcription plus the normalisation rule of task 2.4 are supposed to settle, and the device run is the first place that can be observed end to end:

  | row | seen on the frozen set | expected on `jca_android` | which task settles it |
  |---|---|---|---|
  | `SSLContextSpec / UnsafeProtocol / TLS` (8,648 ev / 65 misuses) | reported | not reported — `TLS` is in the api30 `SSLContext` list | 2.3 |
  | `KeyStoreSpec / InvalidKeyStoreType / AndroidKeyStore` (2,005 ev / 12) | reported | not reported — `AndroidKeyStore` is in the api30 `KeyStore` list | 2.3 |

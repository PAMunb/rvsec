...
- **INV-CONF-06**: The `Unknown` taxonomy MUST be closed to exactly five tags — `UnrecognizedConstraint`, `OverlappingDispatch`, `MultiSlicedOrder`, `UnresolvedSignature`, `UntranslatableConstraint` — each with its declared field schema. The component MUST NOT emit an untagged refusal, and MUST NOT emit a tag outside this set.
...
M0 SHALL run before M1–M4 for each specification and SHALL answer three questions, all decidable from artifacts the design already produces. **M0.1 — does it index?** The generated monitor builds a `MapOfMonitor` when the specification's parameter binding is effective; a specification with `0/N` binding or with no declared parameter compiles to one monitor for the whole program, and parametric slicing is a no-op in it. **M0.2 — is the accusation site reachable?** A specification with an empty `@match` and no `@fail` cannot accuse under any trace. **M0.3 — does the pointcut resolve?** Each resolved signature is checked a posteriori against the `android.jar` index.
...
M3 SHALL declare the counting rule of every denominator it publishes. The rule R1 counts one clause per `;` inside `CONSTRAINTS`, with comments removed and `&&` conjunctions not split; under other rules the same corpus yields different totals, and any upstream number entering a publication needs its rule beside it.
...
| `.mop` ↔ rule pairing | `22 of 24` | the two unpaired specifications named in the `disposition` column of `order_alphabet_map.csv`, an artifact the component does not produce — **not** re-running the declared-type rule, which is the component's own rule |

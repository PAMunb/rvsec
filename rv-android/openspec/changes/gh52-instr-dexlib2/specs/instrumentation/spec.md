...
- `UnsupportedAspectConstructError` — raised when a descriptor advice references a construct that the dexlib2 weaver does not implement (currently: `around`, `cflow`, `cflowbelow`, `handler`, `get`, `set`). The error message MUST cite `docs/LIMITATIONS.md` and the offending advice name.
...
- **INV-INS-18**: The Python public API `instrument_apks(apks_dir, results_dir) → InstrumentationResults` MUST behave identically (return type, exception types, side-effect surface) regardless of the variant chosen. Differences MUST be confined to `InstrumentationResults.variant`, error messages, and the contents of the produced APKs.
...
The system MUST provide an alternative to the AspectJ-based instrumentation pipeline that operates exclusively over DEX bytecode using `dexlib2`, eliminating the `dex2jar → ajc → d8` round-trip and the JVMS §4.10.1.9 type-consistency conflict it induces on R8-optimized APKs. This pipeline MUST be implemented as a Maven multi-module Java project (`rv-android/modules/rv-instrumentation-dexlib2/`) wrapped by a Python module (`rv-android/modules/rv-instrumentation-dexlib2-py/`) that exposes the same `instrument_apks(apks_dir, results_dir) → InstrumentationResults` contract used by the legacy pipeline.

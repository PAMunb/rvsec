...
- [~] 5.4 Implemented `RegisterAllocator.allocate(MutableMethodImplementation, RegisterRequest) → RegisterAllocation`. When the request is NONE/null → return `RegisterAllocation.NONE`. Currently uses `bumpRegisterCount` alone, which violates INV-INS-26. **Confirmed exercised** via the gh52 cryptoapp smoke run (Apr 25): `AfterReturningEmitter` (RegisterRequest.scratch(1)) was matched against an internal call inside `androidx.core.util.Preconditions.checkArgument(boolean, Object)`, the allocator bumped registerCount 4→5 without shifting the bytecode, and the boolean param at v2 became Undefined → `VerifyError: type Undefined unexpected as arg to if-eqz/if-nez`. Compounded by INV-INS-27 (`AfterReturningEmitter` also inserted its monitor `invoke-static` between the matched invoke and its `move-result-object`, killing the move-result semantics). **Required follow-up to fix both at once**: refactor allocator to take `Method` (so `MethodUtil.getParameterRegisterCount` drives the localCount-vs-spill decision), apply `spillLowRegisters` analogously to `coverage-weaver.CoverageWeaver` (task 6.4 — landed), AND coordinate the shift with the emitter so its plan's argBindings are rewritten to post-shift positions; the `InstructionInjector` for AFTER-style insertion must also skip past `move-result*` when present (INV-INS-27). Currently the smoke run shows com.mine.autoshine boots cleanly post-fix (coverage VerifyError eliminated), cryptoapp still crashes on the advice-side bugs.

...
- **Modules changed**: `rv-agent` (ranking, strategies, services, metrics, config), `rvagent-tool` (variants, mapping, lifecycle, guard tests). CI workflow file `.github/workflows/ci.yml` at the rvsec reactor root (one level above `rv-android/`); its per-module loop excludes only `rv-agent` (and `rv-agent-validation`), so reactivation reverts the `rv-agent` exclusion — `rvagent-tool` is already in the loop.

...
- **T4 — Tempo do advice.** CrySL não distingue `before`/`after`, e o `after` do corpus é after-finally — roda em chamada que lança, comportamento documentado no próprio corpus (`jca_android/MacSpec.mop`). Os pointcuts dos `.mop` usam a **sintaxe** AspectJ, mas quem os realiza em produção é o instrumentador **dexlib2** (a variante `ajc` permanece como default do projeto); a semântica de tempo do advice é obrigação de qualquer variante. Nota de método; sem solução.

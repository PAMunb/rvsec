...
1. **A moeda está no app**, e o fonte é nosso — `examples/cryptoapp` ✅. `CipherUtil.encrypt` instancia `new Random()` **sem semente e a cada chamada** (`app/src/main/java/br/unb/cic/cryptoapp/cipher/CipherUtil.java:16`) e desvia em `random.nextInt(10) > 6` (`:19`): o ramo verdadeiro — 7, 8, 9, portanto **30%** — é o `aes()`, e é nele que está o `new SecretKeySpec(byteKey, "AES")` do evento nº 8 (`:40`); os 70% restantes vão para `des()` (`:48`), de onde vêm os eventos 3/4/5. Isso bate com o `location` do oráculo, que já separava `CipherUtil.aes` de `CipherUtil.des`. Os 30 cliques do script existem só para que os dois ramos caiam: P(nenhum AES) = 0,7³⁰ ≈ 2,3·10⁻⁵. Ou seja, ~1 em 43 mil execuções perde **justamente o discriminante** — e um falso MISS depois do conserto leria como "o conserto não funcionou".
...
**O desenho que resolve**, e que precisa entrar na change se a L3-a for algum dia desbloqueada: **condicionar o veredito a uma testemunha independente do ramo**. A entrada em `CipherUtil.aes` produz uma linha `RVSEC-COV`, emitida pelo *coverage weaving* — não pela chamada de monitor que a truncagem apaga. O gate deixa de ser "o evento nº 8 apareceu?" e passa a ser: se `RVSEC-COV` mostra `CipherUtil.aes` **e** o evento nº 8 não aparece → defeito; se `CipherUtil.aes` não aparece → execução **inconclusiva**, repetir. O falso MISS de 2,3·10⁻⁵ deixa de existir como veredito. Complementos: clicar **até** a testemunha em vez de 30 vezes fixas; e `adb logcat -G` contra a fonte 3.

**E há um conserto de raiz disponível, que só ficou visível agora** ✅: o fonte do `cryptoapp` é nosso (`examples/cryptoapp`, projeto Gradle). A fonte 1 pode ser eliminada em vez de contornada — dando ao `encrypt` uma escolha explícita de algoritmo (por UI ou por `Intent` extra), ou hasteando o `Random` para campo com semente fixa em vez de instanciá-lo sem semente a cada chamada (`CipherUtil.java:16`). O custo é real e precisa ser pesado contra a testemunha: muda o APK sob teste, obriga a re-datar o `cryptoapp-oracle.yaml` — cuja proveniência declarada é um run de APE-RV de 2026-04 — e o `apks_examples/cryptoapp.apk` versionado deixa de corresponder ao fonte. A testemunha, ao contrário, não toca no APK e vale mesmo para os oráculos já datados. Decidir entre as duas pertence à change, não a este documento.
...
| `examples/cryptoapp` | o **fonte** do APK sob teste (projeto Gradle, nosso): `CipherUtil.java:16-19` é o sorteio DES/AES e `:40` é o `SecretKeySpec` do evento nº 8 | §6.5 |

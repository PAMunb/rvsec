...
2. **Python config + command build.** `RVStaticAnalysisConfig` resolves all tool paths through a four-level priority chain (explicit args > `rvsec_root` layout > `RVSEC_HOME` env > cwd parent). It *can* verify that the gator launcher, the analysis-client JAR, `android.jar` and the MOP/targets source exist and enforce INV-ANA-33 (exactly one of `mop_dir` / `targets_file`), but every one of those checks sits behind `validate_on_init`: `model_post_init` calls `_validate_configuration()` only when the flag is true. On the `rv-experiment` path it is false (`rv-experiment/config.py`, the `RVStaticAnalysisConfig(...)` call in `get_static_analysis_config`), so a run driven by an experiment reaches the JVM with none of those paths checked, and a missing tool surfaces as a launcher failure instead of a configuration error. Standalone callers keep the default (true) and get the eager checks. `get_tool_command()` then assembles the argv, deliberately using `sys.executable` (not the literal `python`) so the launcher runs on hosts without `/usr/bin/python`. Concretely, `argv = [sys.executable, <lib>/gator/gator, 'a', '-p', cryptoapp.apk, '--client-jar', rvsec-analysis-client.jar, '--out', cryptoapp.apk.json, '-client', 'RvsecAnalysisClient', '-clientParam', 'mopDir=.../jca', '-cgAlgorithm', 'spark', '--timeout', '600', '--jvm-memory', '12G', '-clientParam', 'codePackage=br.unb.cic.cryptoapp']`.
...
8. **Streaming JSON write + enrichment.** `JsonReportWriter` walks the model and writes each section straight to the output stream, flushing after every section, and asks the `ReachabilityEnricher` callback for each node's flags as it goes (the writer itself never calls the index — INV-ANA-30). The runtime order in code is components → reachability → windows → transitions, which is also what the class Javadoc now names. Components is written first because it is cheap manifest-derived data, so even a WTG timeout keeps the window→activity lookup intact (D14). If `skipWtg=true` or the WTG handle is null, `transitions[]` is emitted empty but `windows[]` is still fully populated (INV-ANA-20). Concretely, `JsonReportWriter.write()` calls `RvsecAnalysisClient.writeComponentsSection` → `writeReachabilitySection` → `writeWindowsSection` → (if `wtg!=null`) `writeTransitionsSection`, each followed by `w.flush()`.

9. **Completion sentinel + fsync.** After all sections flush, the writer appends the literal top-level field `"complete": true` — but only when the caller passed `emitSentinel=true` — and calls `fos.getFD().sync()` to force the bytes to disk (INV-ANA-31). A run killed before this point simply lacks the sentinel; the parser then knows the sample is partial. This is the single bit that distinguishes a clean run from a timeout, and it answers to the analysis, not to the write: the client writes this file twice, once before WTG construction and once after, so the pre-WTG write asks for no sentinel and the post-WTG one asks for it. The exception is `skipWtg=true`, where the client returns right after the first write — that write *is* the run's end, and it emits the sentinel. Without this distinction a run killed inside WTG construction left a file that ended in `"complete": true` with `transitions: []`, which is what the five timed-out APKs of the `SA_RERUN_gh91_wtg` campaign have on disk. The trailing bytes of a finished run are `,"complete":true}\n` then `FileDescriptor.sync()`.

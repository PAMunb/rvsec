...
the identical flow runs inside `phtcosta/rvandroid:0.9.4`, built as layer 4 of a four-image
...
flowchart TB
    subgraph CHAIN["Install: 4-layer image chain (docker/)"]
        ...
        IRVA["rvandroid:0.9.4<br/>rvsec clone + Maven reactor + uv sync --no-dev<br/>(ape-rv.jar rebuilt; instr-cli.jar gate)"]
...
| rvandroid:0.9.4 image | image chain layer 4 | Clones rvsec `modules` branch, runs Maven reactor, `uv sync --no-dev`; rebuilds ape-rv.jar (gh71); build gate on instr-cli.jar; `LABEL rvsec.branch` |

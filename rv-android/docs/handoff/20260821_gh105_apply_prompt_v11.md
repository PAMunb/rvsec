# Handoff — aplicação da change gh105-predicate-wiring (checkpoint 32/74, o Grupo 4 fechou o quarto arquivo seguido)

**Data**: 2026-08-21 · **Branch**: `modules` · **Último commit**: `d64f3a40`
...
**Predecessor deste documento**: `docs/handoff/20260821_gh105_apply_prompt_v10.md` (checkpoint 31/74).
...
8. **Códigos sem caminho de execução**: `IVPARAMETERSPEC-CONSTR-00`/`-01`, `SECURERANDOM-CONSTR-00`,
   `PBEKEYSPEC-CONSTR-01` e agora `PBEPARAMETERSPEC-CONSTR-01`/`-03`. `VIOLATED` na aridade 1 sem
...
17. **Um evento inteiro pode não acusar nada, e o censo não mostra.** O `c2` era uma alternativa
    completa do `ORDER` da regra, com leitura, escrita e transição — e zero relatos em qualquer
    configuração, porque a guarda `return false` levava tudo embora antes do `handleEvent`.
    **Um sítio que o grafo conta como `read:condition-guard` pode estar escondendo um evento mudo,
    não só uma leitura mal colocada.** Antes de migrar um arquivo, pergunte de cada evento com
    guarda: *o que ele acusa hoje, medido?*
...
## Números medidos (estado atual, reproduzidos da fonte em 2026-08-21 após a 4.7)
...
leituras do `GCMParameterSpecSpec` estão em guarda; a cadeia `randomized[src]` tem produtor no
store novo desde a 4.5; então **a janela F2 deste arquivo está aberta e é invisível**, porque a
guarda converte o elo quebrado em silêncio em vez de relato. Quando a 4.8 mover as leituras, a
janela vira relato.
...
- `data/gh105/evidence/`: `f1-group-three-the-seventeen.md`, `f2-CipherSpec.md`,
  ...
  **`f2-PBEParameterSpecSpec.md`**, `reach-probe/`, `f1-IvParameterSpec-report-count.md`,
...
15. **A baseline precisa ser reescrita quando um achado é reparado**, não só quando um sítio muda
    de chave. O `--write` preserva `retired`.
...
Leia primeiro docs/handoff/20260821_gh105_apply_prompt_v11.md, depois os quatro artefatos em
openspec/changes/gh105-predicate-wiring/ e as seis evidências do Grupo 4 em
data/gh105/evidence/ (f2-CipherSpec.md, f2-reach-probe.md, f2-IvParameterSpec.md,
f2-SecureRandomSpec.md, f2-PBEKeySpecSpec.md, f2-PBEParameterSpecSpec.md), e siga
...
censo não mostra isso. Meça também se o elo #11 do ledger fica fiado de ponta a ponta, porque
a 5.4 já herdou o #24 e o #25 e não pode acrescentar um segundo acusador para nenhum deles.
Traga as decisões de projeto ao pesquisador antes de editar, com medição junto — e diga
sobre o que a sua medição é: "nenhuma trace separa as duas opções" é afirmação sobre o
corpus, não sobre o programa.
```

...
- **Implicações**: 
  ...
  - Considerar quantização 4-bit para modelos acima de 7B parâmetros :: com ollama isso fica a cargo do nome do proprio modelo (sendo que aparentemente os default ja sao quantizados em 4bits, o que é estranho pois consomem muita memoria) ... para o huggingface ja tem uma quantizacao (de 8 bits se nao me engano) mas podemos usar de 4 bits
...
- **Estratégias especializadas**: batch, single, vision, mop_vision :: sera feito o merge de mop_vision com vision ... acho que nem compensa citar mop_vision aqui para nao causar confusao
...
- **Over-emphasis em elementos MOP** pode prejudicar exploração natural da aplicação :: excelente, acho que deveria ate dar mais enfase
...
- **Abordagem conservadora**: Priorizar exploração natural e sistemática antes de focar em elementos MOP
...
- **Módulo independente**: Sistema de otimização separado, sem reutilizar test framework :: devemos discutir bastante a respeito para realmente planejar essa otimizacao
...
- Fornecer coordenadas explícitas no formato: `"button 'OK' at position (540, 1306)"` :: ok, esse "at position" sera adicionando em que momento? no visitor ou no fragment (.py) ... lembre que so vale para modelos com visao (independente da estrategia) e que nao eh exatamente util nos itens do ScreenDescription ... pois a ideia do vision seria explorar itens nao listados no dump do uiautomator, renderizados dinamicamente ... mas a coordena poderia servir de "guia" para a llm se localizar e selecionar uma coordenada "certa"
- Instruir modelo a usar EXATAMENTE as coordenadas fornecidas :: mesma coisa acima
...
:: nao entendi como isso seria feito ... ja esta definido no plano?
...
└── vision.xml (independente - NÃO herda!) ❌
    └── mop_vision.xml (extends="vision") :: nao! sera removido apos o merge com o vision ... esta cheio de referencias a mop_vision ainda ... verifique todas e faça as atualizacoes devidas (lembrando do merge com vision)
```
...
- `history_section.xml`: Seção de histórico de ações :: acho que nao esta levando em consideracao que temos conext modes (stateless e rich) e essa informacao de historico eh apenas para o stateless
- `mop_vision_instructions.xml`: Instruções focadas em MOP :: o que conteria nele? sera que vale a pena ter isso?

#### **Problemas Críticos Identificados**

**1. Redundâncias Significativas**:
- `vision_system_consolidated` duplica muito conteúdo de `system_intro` + `system_guidelines` :: sim, pois nao havia heranca ... agora vision vai herdar
...
:: devemos ter um mapeamento detalhado dos templates, inclusive indicando os que nao sao mais usados e devem ser movidos para backup ... nao vamos deixar lixo 

#### **Oportunidades de Otimização Imediatas**

**Consolidação de Templates**:
- **Merge** `mop_vision` → `vision` com flag condicional `mop_focus`
- **Eliminar** `vision_system_intro` → usar `system_intro` :: por causa da heranca ...
- **Padronizar** nomes de fragmentos single (standard → single) :: single quer dizer single action generation ... apenas uma acao por vez, em cada interacao com a llm, cada inferencia
...
- `vision_system_consolidated.xml`: **98 linhas** - candidato principal para compressão :: ainda sera usado? se nao me engano ele foi uma gambiarra para fazer um template grande (sem fragments ) para facilitar testes ... mas se ainda for usado com certeza deve ser quebrado em fragments reusaveis
- `context_status.xml`: **49 linhas** - lógica complexa pode ser simplificada
- `ui_patterns_guide.xml`: **24 linhas** - pode ser mais conciso :: ainda teremos ui pattern match ... acho nossa solucao de pattern match muito basica e nao confiavel ... acredito que devemos pensar na remocao completa da funcionalidade
...
- **FrontierModels Cleanup**: Manter apenas Claude + ChatGPT, remover dependencies (poetry):
...
- **Exemplo**: "Últimas 3 iterações: tentativa login → erro validação → correção campos" :: muito otimista essa sua descricao da acao, devemos usar a str do ItemAction

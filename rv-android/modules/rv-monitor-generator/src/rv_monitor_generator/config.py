...
class RVGeneratorConfig(BaseValidatedModel):
    ...
    javamop_bin: Optional[str] = Field(
        default=None, description="Path to JavaMOP binary executable"
    )
    rvmonitor_bin: Optional[str] = Field(
        default=None, description="Path to RV-Monitor binary executable"
    )
    mop_specs_dir: Optional[str] = Field(
        default=None, description="Directory containing MOP specification files"
    )
    aspects_dir: Optional[str] = Field(
        default=None, description="Directory containing custom AspectJ files"
    )
    ...
    def _resolve_paths(self) -> None:
        ...
        if (
            all(path is not None for path in tool_paths)
            and self.mop_specs_dir is not None
        ):
            ...
            if self.aspects_dir is None:
                ...
                self.aspects_dir = os.path.join(
                    os.path.dirname(self.mop_specs_dir), "aspect"
                )
            return
    ...
    def _resolve_from_rvsec_root(self, rvsec_root: str) -> None:
        ...
        if self.rvmonitor_bin is None:
            self.rvmonitor_bin = os.path.join(
    ...
    def _validate_tool_binary(self, binary_path: str, tool_name: str) -> None:
        ...
        if not os.access(binary_path, os.X_OK):
            raise ConfigurationError(
                f"{tool_name} binary not executable: {binary_path}"
            )
    ...
    def _validate_mop_specifications(self) -> None:
        ...
        """
        mop_files = glob.glob(
            os.path.join(self.mop_specs_dir, f"*{constants.EXTENSION_MOP}")
        )
        if not mop_files:
            raise ConfigurationError(
    ...
    def _validate_tool_functionality(self) -> None:
        ...
        try:
            result = subprocess.run(
                [self.javamop_bin, "-h"], capture_output=True, timeout=10, text=True
            )
            # JavaMOP may return non-zero exit status with -h, but should produce output
            if not result.stdout and not result.stderr:
                raise ConfigurationError(
                    f"JavaMOP produced no output: {self.javamop_bin}"
                )
        except (subprocess.TimeoutExpired, FileNotFoundError) as e:
            raise ConfigurationError(
                f"JavaMOP functionality test failed: {self.javamop_bin}\nError: {e}"
            )

        # Test RV-Monitor functionality
        try:
            result = subprocess.run(
                [self.rvmonitor_bin, "-h"], capture_output=True, timeout=10, text=True
            )
            # RV-Monitor may return non-zero exit status with -h, but should produce output
            if not result.stdout and not result.stderr:
                raise ConfigurationError(
                    f"RV-Monitor produced no output: {self.rvmonitor_bin}"
                )
        except (subprocess.TimeoutExpired, FileNotFoundError) as e:
            raise ConfigurationError(
                f"RV-Monitor functionality test failed: {self.rvmonitor_bin}\nError: {e}"
            )

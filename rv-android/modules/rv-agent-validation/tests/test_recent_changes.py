...
def test_action_normalizer():
    """Test ActionNormalizer coordinate conversion from [0, 1000) to device pixels."""
    logger.info("=" * 60)
    ...
    # Test LLM action normalization with [0, 1000) -> pixel conversion
    # Qwen3-VL returns normalized coords in [0, 1000) range
    llm_action = {
        "tool_name": "android_click",
        "tool_args": {"x": 500, "y": 500}  # Center in [0, 1000) space
    }
    ...
    # Check coordinate conversion: (500, 500) in [0, 1000) -> (540, 960) in 1080x1920
    expected_x = int((500 / 1000) * 1080)  # = 540
    expected_y = int((500 / 1000) * 1920)  # = 960
    ...
    assert normalized["original_coords"] == (500, 500), f"Original coords mismatch"
    ...
    logger.info(f"  Coordinate conversion: (500, 500) [0,1000) -> ({normalized['x']}, {normalized['y']}) pixels")

    # Test edge case: coordinates near bounds
    llm_action_edge = {
        "tool_name": "android_click",
        "tool_args": {"x": 826, "y": 572}  # Typical Qwen3-VL output
    }
    normalized_edge = normalizer.from_llm(llm_action_edge)
    expected_x_edge = int((826 / 1000) * 1080)  # = 892
    expected_y_edge = int((572 / 1000) * 1920)  # = 1098
    assert normalized_edge["x"] == expected_x_edge, f"Expected x={expected_x_edge}, got {normalized_edge['x']}"
    assert normalized_edge["y"] == expected_y_edge, f"Expected y={expected_y_edge}, got {normalized_edge['y']}"
    logger.info(f"  Edge case: (826, 572) -> ({normalized_edge['x']}, {normalized_edge['y']})")

    # Test SET_TEXT action
    llm_text_action = {
        "tool_name": "android_type_text",
        "tool_args": {"x": 500, "y": 300, "text": "test@email.com"}
    }
    ...
    # Test algorithm action normalization (no conversion needed)
...
def test_llm_with_normalizer():
    ...
    ui_text = """
    UI Elements:
    1. [Button] "Login" at center - clickable
    2. [EditText] "Username" - editable
    3. [EditText] "Password" - editable
    """

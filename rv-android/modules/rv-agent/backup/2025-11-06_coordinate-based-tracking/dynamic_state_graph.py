...
class DynamicStateGraph:
    ...
    def record_action(
        ...
        """
        Record action execution on a screen.
        ...
        if screen_hash in self.states:
            node = self.states[screen_hash]
            logger.info(f"🔒 GRAPH: Recording action ID={action_id} on state {screen_hash[:8]}")
            ...
            logger.warning(f"⚠️  GRAPH: Cannot record action ID={action_id} - state {screen_hash[:8]} not found!")

...
class LoopDetector:
    ...
    Detections:
    - Consecutive similar actions (type/coordinate-based)
    - Spatial loops (clicks clustered in same area)
    - Action sequence repetitions (same sequence repeating)
    """
    ...
    SEQUENCE_MIN_LENGTH = 2  # minimum sequence length to detect
    SEQUENCE_REPETITION_THRESHOLD = 3  # how many times sequence must repeat
    ...
    def detect_action_sequence_repetition(
        self,
        recent_actions: List[Dict[str, Any]]
    ) -> tuple[bool, List[str], int]:
        """
        Detect if a sequence of actions is repeating.

        Detects patterns like: [A, B, C, A, B, C, A, B, C]
        where the sequence [A, B, C] repeats 3 times.

        Args:
            recent_actions: Recent action history window

        Returns:
            Tuple of (is_repetition, pattern, repetitions)
            - is_repetition: True if sequence repetition detected
            - pattern: List of action signatures in the repeating sequence
            - repetitions: Number of times the pattern repeated
        """
        if len(recent_actions) < self.SEQUENCE_MIN_LENGTH * self.SEQUENCE_REPETITION_THRESHOLD:
            return False, [], 0

        # Convert actions to simple signatures for comparison
        action_signatures = [
            self._get_action_signature(action)
            for action in recent_actions
        ]

        # Try different pattern lengths (2, 3, 4 actions)
        for pattern_length in range(self.SEQUENCE_MIN_LENGTH, 5):
            result = self._check_pattern_repetition(
                action_signatures,
                pattern_length,
                self.SEQUENCE_REPETITION_THRESHOLD
            )
            if result[0]:  # Found repetition
                return result
    ...
    def _get_action_signature(self, action: Dict[str, Any]) -> str:
        """
        Create a simple signature for an action.

        Examples:
        - TYPE_TEXT → "TYPE"
        - CLICK at (352, 680) → "CLICK:(350,680)" (rounded to 50px)
        - SCROLL down → "SCROLL:down"
        - BACK → "BACK"
        """
        action_type = action.get("action_type", "UNKNOWN")

        if action_type == "TYPE_TEXT" or action_type == "SET_TEXT":
            return "TYPE"

        elif action_type in ["CLICK", "LONG_CLICK"]:
            # Round coordinates to 50px grid to group nearby clicks
            x = round(action.get("x", 0) / 50) * 50
            y = round(action.get("y", 0) / 50) * 50
    ...
    def _check_pattern_repetition(
        ...
        """
        Check if a pattern of given length repeats threshold times.

        Args:
            signatures: List of action signatures
            pattern_length: Length of pattern to look for
            threshold: Minimum number of repetitions required

        Returns:
            Tuple of (found, pattern, count)
        """
        if len(signatures) < pattern_length * threshold:
            return False, [], 0

        # Take the last N actions where N = pattern_length * threshold
        recent = signatures[-(pattern_length * threshold):]

        # Split into blocks of pattern_length
        blocks = []
        for i in range(0, len(recent), pattern_length):
            if i + pattern_length <= len(recent):
                blocks.append(recent[i:i + pattern_length])

        # Check if we have enough blocks and they're all identical
        if len(blocks) >= threshold:
            first_block = blocks[0]
            if all(block == first_block for block in blocks[:threshold]):
                self.logger.warning(
                    f"🔁 Action sequence repetition: pattern {first_block} "
                    f"repeated {threshold}x (length={pattern_length})"
                )
                return True, first_block, threshold

...
Implements DFS exploration that prioritizes depth while incorporating static analysis guidance
for monitored operation coverage. Uses coordinate-based action tracking to handle non-deterministic
action IDs across UIAutomator parsing sessions.
"""
...
class DFSStrategy(BaseStrategy):
    ...
    - Uses coordinates instead of IDs for action tracking
    ...
    def get_guidance(
        ...
        ### Architectural Decisions:
        - Prioritizes untested actions first (by coordinates)
        ...
        logger.info("=" * 80)
        logger.info(f"📋 RAW SCREEN DESCRIPTION (hash={current_hash[:8]}):")
        ...
        # Identify untested actions by coordinates (converted to optimized space)
        if node:
            untested = [
                action for action in all_actions
                if self._convert_signature_to_optimized(action.coords_for_matching) not in node.executed_actions
            ]
        else:
    ...
    def _convert_signature_to_optimized(self, signature: Tuple[Tuple[int, int], str]) -> Tuple[Tuple[int, int], str]:
        ...
        RVAgent uses optimized screenshots for LLM processing, so action coordinates
        from _action_to_dict are in optimized space. However, coords_for_matching returns
        device space coords. This method converts signatures to match using CoordinateConverter.
        ...
        if self.converter:
            ...
            # Fallback to direct conversion if converter not available
    ...
    def select_next_action(
        ...
        """
        Select next action using pure DFS algorithm with coordinate-based tracking.

        Implements depth-first search without LLM:
        1. If state is new, add to stack
        2. If untested actions exist (by coords), select one (DEEPEN)
        ...
        if screen_hash not in self.graph.states:
            ...
            logger.info(f"   Hash: {screen_hash[:16]}... (full: {screen_hash})")
            logger.info(f"   Previously executed actions (signatures): {sorted(node.executed_actions)}")
        ...
        # CRITICAL DEBUG: Trace untested filtering logic using action signatures
        logger.info(f"🔍 FILTERING - State {screen_hash[:8]} has {len(node.executed_actions)} executed signatures: {sorted(node.executed_actions)}")

        untested = []
        for action in all_actions:
            # Convert signature from device space to optimized space for comparison
            action_signature = self._convert_signature_to_optimized(action.coords_for_matching)
            if action_signature in node.executed_actions:
                logger.debug(f"   ❌ SKIP ID={action.id} sig={action_signature} (already executed) - {action.text[:40] if action.text else 'no-text'}")
            else:
                logger.debug(f"   ✅ KEEP ID={action.id} sig={action_signature} (untested) - {action.text[:40] if action.text else 'no-text'}")
        ...
        if untested:
            ...
            for i, action in enumerate(untested[:5]):  # Show first 5
                ...
                logger.info(f"      {i+1}. ID={action.id} sig={signature} {action.text[:50] if action.text else 'no-text'} {mop}")
        ...
        if untested:
            ...
            # Use signature (coords + type) for stable tracking across parsing sessions
            # coords_for_matching returns DEVICE space, but we need OPTIMIZED space for graph
            action_signature_device = top_action.coords_for_matching
            action_signature = self._convert_signature_to_optimized(action_signature_device)
            logger.info(f"✅ DFS DEEPEN: {action_dict['action_type']} ID={top_action.id} sig={action_signature} (priority {self._get_mop_priority(top_action)})")
            logger.info(f"   🔒 Pre-marking action with sig={action_signature} as executed on state {screen_hash[:8]}")
            self.graph.record_action(screen_hash=screen_hash, action_signature=action_signature)
    ...
    def _action_to_dict(self, action: Any) -> Dict[str, Any]:
        ...
        """
        # Use ItemAction's computed action_type property (extracts from text)
        # This correctly identifies click, set_text, scroll, etc.
        action_type_internal = getattr(action, 'action_type', 'unknown')

        # Map internal action types to execution action types
        action_type_map = {
            'click': 'CLICK',
            ...
            'scroll_right': 'SCROLL',
            'key_event': 'BACK',
            'unknown': 'CLICK'  # Fallback to CLICK
        }

        action_type = action_type_map.get(action_type_internal, 'CLICK')
        ...
        # Generate text input for TYPE_TEXT actions
        text_input = None
        if action_type == 'TYPE_TEXT':
            # Generate simple test text for EditText fields
            text_input = "test123"
        ...
        result = {
            ...
            "id": action.id  # Keep for logging compatibility
        }

        # Add text field only for TYPE_TEXT actions
        if text_input:

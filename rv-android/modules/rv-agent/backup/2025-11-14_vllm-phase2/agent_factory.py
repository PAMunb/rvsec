...
class AgentFactory:
    ...
    def create_agent(
        ...
        # Create UI coverage tracker (needed by ScreenProcessor)
    ...
    def _create_llm_client(config: RVAgentConfig) -> LLMClient:
        ...
        # Create base LLM using vLLM server (OpenAI-compatible API)
        # Model: Qwen3-VL-4B-FP8 (16K context window)
        # Sampling parameters follow Qwen3-VL official recommendations
        # NOTE: LLMClient will handle tool calling via direct HTTP API calls
        llm_base = ChatOpenAI(
            base_url=llm_config.get('base_url', 'http://localhost:8000/v1'),
            api_key="EMPTY",  # vLLM server does not require API key
            model=llm_config.get('model', './models/qwen3-vl-4b-fp8'),
            temperature=0.6,  # Qwen3-VL recommended: 0.6
            top_p=0.95,       # Qwen3-VL recommended: 0.95
            max_tokens=2048,  # Maximum output tokens per request
            timeout=config.llm_timeout,
            http_client=httpx.Client(timeout=httpx.Timeout(config.llm_timeout, connect=5.0))
        )

        logger.info("LLM created without bind_tools() - using direct HTTP API for tool calling")
        ...
        try:
            ...
            logger.warning(f"Falling back to v12")
            from rv_agent.prompts import v12 as prompt_module

        # Create LLM client (will use direct HTTP API for tool calling)
        return LLMClient(
            config=config,
            llm=llm_base,

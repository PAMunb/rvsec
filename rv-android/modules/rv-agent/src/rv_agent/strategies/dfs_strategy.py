...
class DFSStrategy(ExplorationStrategy):
    ...
    def select_next_action(
        ...
        """
        logger.debug(
            f"DFS: Processing state {current_hash[:8]}, depth={self.current_depth}"
        )
        ...
        if current_hash not in self.graph.states:
            ...
            logger.info(
                f"DFS: New state at depth {self.current_depth}, {node.total_actions} actions"
            )
            logger.info(f"     Hash: {current_hash[:16]}...")
        else:
            node = self.graph.states[current_hash]
            logger.info(
                f"DFS: Revisited state (visit {node.visit_count}, executed: {len(node.executed_actions)})"
            )
            logger.info(f"     Hash: {current_hash[:16]}...")
        ...
        if text_action:
            ...
            logger.info("DFS DEEPEN (TEXT): Selected SET_TEXT action")
        ...
        if filtered_actions:
            ...
            if scroll_action:
                self.current_depth += 1
                logger.info(
                    "DFS SCROLL: All visible actions tested, scrolling to reveal more content"
                )
            ...
            if selected_action is not None:
                ...
                logger.info(
                    f"DFS CONTINUOUS: Selected LEAST-EXECUTED action ID={selected_action.id}"
                )
                logger.info(f"  Signature: {action_signature}")
                logger.info(f"  Priority: {self._get_mop_priority(selected_action)}")
        ...
        logger.info(
            f"DFS: No actions available on state {current_hash[:8]}, returning BACK"
        )

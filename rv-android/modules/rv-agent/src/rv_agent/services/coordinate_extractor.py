...
def pixels_to_normalized(
    ...
    height: int = DEFAULT_DEVICE_HEIGHT,
) -> Tuple[int, int]:
    """
                    ...
                    elements.append(
                        {
                            ...
                            "center": (
                                center_x,
                                center_y,
                            ),  # Keep original pixels for reference
                            "center_normalized": (
                                norm_x,
                                norm_y,
...
def validate_with_tolerance(
    generated: Tuple[int, int], ground_truth: List[Tuple[int, int]], tolerance: int = 50
) -> bool:
    """
    ...
    for gt_coord in ground_truth:
        distance = (
            (generated[0] - gt_coord[0]) ** 2 + (generated[1] - gt_coord[1]) ** 2
        ) ** 0.5

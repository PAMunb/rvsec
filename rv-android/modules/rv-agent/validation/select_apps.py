...
def main():
    ...
    print(f"\nApps by category:")
    for cat in sorted(apps_by_category.keys(), key=lambda x: len(apps_by_category[x]), reverse=True):
    ...
    for category, quota in CATEGORY_QUOTAS.items():
        available = [a for a in apps_by_category.get(category, [])
                     if a['file'] not in selected]
    ...
    with open(output_file, 'w') as f:
        ...
        for app in sorted(selected.values(), key=lambda x: x['categories'][0] if x['categories'] else 'ZZZ'):

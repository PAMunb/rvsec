...
from .conftest import launch_app, force_stop_app, go_home, SGLANG_URL, SGLANG_MODEL, DEVICE_ID
...
class TestPureAlgorithmMode:
    ...
    def test_short_exploration(self, device, cryptoapp, device_id, check_emulator, results_dir):
        ...
        # Save results
        result_file = results_dir / f"pure_algo_{datetime.now().strftime('%H%M%S')}.json"
    ...
    def test_exploration_with_multiple_apps(self, device, device_id, check_emulator, results_dir):
        ...
        for r in results:
            print(f"    {r['app']}: {r['iterations']} iterations, {r['unique_states']} states")
...
class TestMultimodeExecution:
    ...
    def test_agent_creation_multimode(self, device, cryptoapp, check_emulator, check_sglang):
    ...
    def test_multimode_exploration(self, device, cryptoapp, device_id, check_emulator, check_sglang, results_dir):
        ...
        llm_pct = (routing.llm_executed / total_actions * 100) if total_actions > 0 else 0

        # Save results
        result_file = results_dir / f"multimode_{datetime.now().strftime('%H%M%S')}.json"
        with open(result_file, 'w') as f:
            json.dump({
                "mode": "multimode",
                "duration_s": duration,
    ...
    def test_multimode_proportion(self, device, cryptoapp, device_id, check_emulator, check_sglang):
...
class TestLLMOnlyMode:
    ...
    def test_agent_creation_llm_only(self, device, cryptoapp, check_emulator, check_sglang):
    ...
    def test_llm_only_exploration(self, device, cryptoapp, device_id, check_emulator, check_sglang, results_dir):
...
class TestMemorySystems:
    ...
    def test_dynamic_graph_population(self, device, cryptoapp, device_id, check_emulator):
...
class TestRecoveryMechanisms:
    ...
    def test_recovers_from_home_screen(self, device, cryptoapp, device_id, check_emulator):

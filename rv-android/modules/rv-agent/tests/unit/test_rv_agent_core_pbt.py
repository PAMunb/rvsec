...
class TestRVAgentCoreFunctionality:
    ...
    @given(
        mode=agent_modes,
        timeout=st.floats(min_value=0.1, max_value=1.0)  # Use shorter timeout to avoid long waits
    )
    @settings(deadline=2000, max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_run_method_properties(self, mode, timeout):
        """Test properties of the run method execution."""
        # Create mock config
        config = MagicMock(spec=RVAgentConfig)
        config.get_agent_mode.return_value = mode
        config.package_name = "com.example.test"
        ...
        # For non-LLM modes, set LLM client to None
        if mode == "pure_algorithm":
            mock_deps["llm_client"] = None

        agent = RVAgent(config=config, **mock_deps)

        # Mock the graph to avoid actual execution - make it raise an exception to stop early
        with patch.object(agent, 'graph') as mock_graph:
            # Make the graph invoke raise an exception after one call to stop the loop early
            mock_graph.invoke.side_effect = Exception("Stop execution for test")

            # Mock routing manager counters
            mock_deps["routing_manager"].get_decision_counters.return_value = {
                "total_actions": 1,
                "llm_executed": 0 if mode == "pure_algorithm" else 1,
                "algorithm_chosen": 1 if mode == "pure_algorithm" else 0,
                "llm_percentage": 0.0 if mode == "pure_algorithm" else 100.0,
                "algorithm_percentage": 100.0 if mode == "pure_algorithm" else 0.0,
                "llm_validation_failed": 0,
                "forced_back": 0,
                "primary_total": 1
            }
            ...
            # Mock memory stats
            mock_deps["memory_coordinator"].get_all_statistics.return_value = {
                "total_interactions": 1
            }

            # Mock device launch to avoid actual device calls
            mock_deps["device"].launch_app.return_value = None
        ...
        # Status should be completed (the exception is caught and handled)
...
class TestRVAgentStateManagement:
    """Property-based tests for RVAgent state management."""
    ...
    @given(
        initial_iteration=st.integers(min_value=0, max_value=100),
        start_time=st.floats(min_value=time.time()-3600, max_value=time.time()),
        timeout=timeout_strategy
    )
    @settings(deadline=2000, max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_initial_state_properties(self, initial_iteration, start_time, timeout):
        """Test properties of initial state setup."""
        # Create mock config
        config = MagicMock(spec=RVAgentConfig)
        config.get_agent_mode.return_value = "pure_algorithm"
        config.package_name = "com.example.test"
        ...
        # Initialize state (similar to what happens in run method)
        state = {
        ...
        assert isinstance(state["visited_states"], list)
        assert isinstance(state["state_transitions"], list)
        assert state["previous_screen_hash"] is None
        assert state["external_navigation_count"] == 0
        assert isinstance(state["recent_action_window"], list)
        assert state["llm_tokens_input"] == 0
        assert state["llm_tokens_output"] == 0
        assert state["llm_time_ms"] == 0.0
        assert isinstance(state["action_history_summary"], str)
        assert isinstance(state["exploration_summary"], str)
        assert isinstance(state["memory_insights"], str)
        assert isinstance(state["navigation_path"], str)
        assert state["decision_path"] == "algorithm"
        assert state["decision_maker"] == "unknown"
        assert state["loop_detected"] is False
        assert state["used_fallback"] is False
        assert state["force_back_action"] is False
        assert state["llm_action"] is None
        assert state["has_tool_calls"] is False
        assert state["llm_reasoning"] == ""
        assert state["validation_path"] == ""
        assert state["ui_xml"] == ""

    @given(
        elapsed_time=st.floats(min_value=0.0, max_value=3600.0),
        timeout=timeout_strategy
    )
    @settings(deadline=2000, max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_timeout_logic_properties(self, elapsed_time, timeout):
        """Test properties of timeout logic."""
        # This test verifies the timeout logic used in the run method
        ...
        # Elapsed time should equal the difference
        assert abs(elapsed - elapsed_time) < 0.1  # Small tolerance for timing precision

...
class TestCoordinateExtractorHypothesis:
    ...
    def test_validate_with_tolerance(self, gen_x, gen_y, tolerance):
        ...
        assert coordinate_extractor.validate_with_tolerance(generated_point, ground_truth_self, tolerance) is True
        ...
        assert coordinate_extractor.validate_with_tolerance(point_inside, ground_truth_edge, tolerance) is True

        # 3. Test a point just outside the tolerance boundary
        point_outside = (gen_x + tolerance + 1, gen_y)
        assert coordinate_extractor.validate_with_tolerance(point_outside, ground_truth_edge, tolerance) is False

    @given(elements=st.lists(st.fixed_dictionaries({'description': st.text(alphabet=st.characters(blacklist_characters='\n'), max_size=50)}), min_size=1, max_size=10))
...
safe_xml_text = st.text(alphabet=st.characters(
...
class_name_attr = st.text(alphabet=st.characters(whitelist_categories=('L',)), min_size=1, max_size=10).map(lambda s: f"android.widget.{s}")
resource_id_attr = st.text(alphabet=st.characters(whitelist_categories=('L', 'N')), min_size=1, max_size=20).map(lambda s: f"com.example.app:id/{s}")
...
node_strategy = st.fixed_dictionaries({
    ...
    lambda d: ' '.join(f'{k}="{v}"' for k, v in d.items() if v is not None)
...
class TestXmlExtractionHypothesis:
    ...
    def test_extract_clickable_elements_with_coords_count(self, nodes):
        ...
        elements = coordinate_extractor.extract_clickable_elements_with_coords(xml_content)

...
def mock_screen_description():
    ...
    action2_click = ItemAction(id=201, event=WidgetEventType.CLICK, text="Username", target_view={})
    action2_input = ItemAction(id=202, event=WidgetEventType.TEXT_CHANGE, text="Username", target_view={})
    item2 = ScreenItem(
...
class TestActionMapper:
    ...
    def test_get_device_coordinates(self):
        ...
        llm_coords = (360, 640)
        dev_x, dev_y = action_mapper.get_device_coordinates(llm_coords, OPTIMIZED_DIMS, DEVICE_DIMS)
    ...
    def test_map_coordinates_to_action_chooses_smallest_element(self, mock_screen_description):
        """Test that the smallest overlapping element is chosen."""
        # Add an overlapping item that is smaller than item 2
        small_item_action = ItemAction(id=401, event=WidgetEventType.CLICK, text="inner", target_view={})
    ...
    def test_map_coordinates_out_of_bounds_clamping(self, mock_screen_description, caplog):

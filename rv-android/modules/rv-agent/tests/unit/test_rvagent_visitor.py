...
class TestMOPEnrichment:
    ...
    def test_methods_have_reaches_mop_flags(self, static_data):
        """Methods in static_data have reaches_mop flags set from .reach file."""
    ...
    def test_main_activity_callbacks_reach_mop(self, static_data):
        ...
        # showScreenMessageDigest should reach MOP
    ...
    def test_mop_enrichment_through_parser(self, static_data, main_activity_xml):
        """MOP enrichment works through full parser flow."""
        ...
        for item in screen_desc.items:
            for action in item.actions:
                if action.reaches_mop or action.directly_reaches_mop:
                    mop_actions.append(action)

        # MainActivity buttons navigate to activities that call crypto
        # So their callbacks should have reaches_mop=True
        assert len(mop_actions) > 0, "Expected at least one action with MOP marker"
...
class TestParserIntegration:
    ...
    def test_parse_main_activity_screen(self, static_data, main_activity_xml):
        """Parse MainActivity screen with MOP enrichment."""
        parser = UIAutomator2Parser(visitor_class=RVAgentVisitor)
        ...
        # Check that buttons have MOP markers
        mop_found = False
        for item in screen_desc.items:
            for action in item.actions:
                if action.reaches_mop or action.directly_reaches_mop:
        ...
        # MainActivity buttons should have reaches_mop markers
        # (they navigate to activities that call crypto methods)
        assert mop_found, "Expected at least one action with MOP marker"

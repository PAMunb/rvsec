...
class TestCoordinateUtils:
    ...
    def test_device_to_optimized(self):
        ...
        opt_x, opt_y = coordinate_utils.device_to_optimized(
            540, 960, DEVICE_DIMS, OPTIMIZED_DIMS
        )
        assert (opt_x, opt_y) == (352, 624)

        # Origin
        opt_x, opt_y = coordinate_utils.device_to_optimized(
            0, 0, DEVICE_DIMS, OPTIMIZED_DIMS
        )
    ...
    def test_device_to_optimized_with_validation_success(self):
        ...
        opt_x, opt_y = coordinate_utils.device_to_optimized(
            540, 960, DEVICE_DIMS, OPTIMIZED_DIMS, validate=True
        )
        assert (opt_x, opt_y) == (352, 624)
    ...
    def test_device_to_optimized_with_validation_failure(self):
        ...
        with pytest.raises(ValueError, match="X coordinate -1 out of optimized bounds"):
            coordinate_utils.device_to_optimized(
                -2, 960, DEVICE_DIMS, OPTIMIZED_DIMS, validate=True
            )

        # A device_y that results in opt_y > optimized_height
        with pytest.raises(
            ValueError, match="Y coordinate 1249 out of optimized bounds"
        ):
            coordinate_utils.device_to_optimized(
                540, 1922, DEVICE_DIMS, OPTIMIZED_DIMS, validate=True
            )
    ...
    def test_optimized_to_device(self):
        ...
        # Center point
        dev_x, dev_y = coordinate_utils.optimized_to_device(
            352, 624, DEVICE_DIMS, OPTIMIZED_DIMS
        )
    ...
    def test_optimized_to_device_clamping(self):
        ...
        dev_x, dev_y = coordinate_utils.optimized_to_device(
            800, 1400, DEVICE_DIMS, OPTIMIZED_DIMS
        )
        ...
        # Negative values
        dev_x, dev_y = coordinate_utils.optimized_to_device(
            -10, -10, DEVICE_DIMS, OPTIMIZED_DIMS
        )
        assert dev_x == 0
        assert dev_y == 0
    ...
    def test_bounds_device_to_optimized(self):
        ...
        opt_bounds = coordinate_utils.bounds_device_to_optimized(
            bounds, DEVICE_DIMS, OPTIMIZED_DIMS
        )
    ...
    def test_bounds_device_to_optimized_invalid_input(self, invalid_bounds):
        ...
        assert (
            coordinate_utils.bounds_device_to_optimized(
                invalid_bounds, DEVICE_DIMS, OPTIMIZED_DIMS
            )
            == invalid_bounds
        )
    ...
    def test_calculate_center_optimized(self):
        ...
        center_x, center_y = coordinate_utils.calculate_center_optimized(
            bounds, DEVICE_DIMS, OPTIMIZED_DIMS
        )
    ...
    def test_calculate_center_optimized_invalid_input(self, invalid_bounds):
        ...
        assert coordinate_utils.calculate_center_optimized(
            invalid_bounds, DEVICE_DIMS, OPTIMIZED_DIMS
        ) == (0, 0)
    ...
    def test_validate_optimized_coords_success(self):
        ...
        assert (
            coordinate_utils.validate_optimized_coords(352, 624, OPTIMIZED_DIMS) is True
        )

    def test_validate_optimized_coords_failure(self):
        """Test validation failure for out-of-bounds coordinates."""
        with pytest.raises(
            ValueError, match="X coordinate 800 out of optimized bounds"
        ):
            coordinate_utils.validate_optimized_coords(800, 600, OPTIMIZED_DIMS)

        with pytest.raises(
            ValueError, match="Y coordinate 1300 out of optimized bounds"
        ):

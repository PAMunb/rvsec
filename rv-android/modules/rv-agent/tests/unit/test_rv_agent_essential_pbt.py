...
class TestRVAgentCoreProperties:
    ...
    @given(mode=agent_modes, package=package_name_strategy, timeout=timeout_strategy)
    @settings(
        deadline=2000,
        max_examples=10,
    ...
    @given(current_hash=screen_hash_strategy, previous_hash=screen_hash_strategy)
    @settings(
        deadline=2000,
        max_examples=10,
        suppress_health_check=[HealthCheck.function_scoped_fixture],
    ...
    @given(iterations=st.integers(min_value=0, max_value=5), has_action=st.booleans())
    @settings(
        deadline=2000,
        max_examples=10,
        suppress_health_check=[HealthCheck.function_scoped_fixture],
    ...
    def test_memory_stats_structure(self, num_interactions, activity_name):
        ...
        expected_stats = {
            ...
            "coverage_percentage": min(
                100.0, (num_interactions / max(num_interactions, 1)) * 100
            ),
        }

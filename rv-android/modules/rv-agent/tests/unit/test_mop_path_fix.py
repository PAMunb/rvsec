...
def _make_transition_manager_with_mop(
    ...
    for hop in range(1, hop_count):
        mid_id = f"w_mid{hop}"
        mid_win = _make_window(mid_id, f"Intermediate{hop}", activity=f"com.example.Intermediate{hop}")
...
class TestStepOneOnlyResolution:
    ...
    def test_two_hop_path_returns_step1_only(self):
        ...
        possible_actions = [
            _make_possible_action("btn_mid1", coordinates=(150, 300))
        ]
        ...
        assert len(result) == 1, "Only step 1 should be returned regardless of path length"
    ...
    def test_direct_hop_returns_single_action_with_coords(self):
        ...
        possible_actions = [
            _make_possible_action("btn_direct", coordinates=(200, 400))
        ]
...
class TestCooldownExpiry:
    ...
    def test_cooldown_blocks_planning_within_window(self):
        ...
        for delta in range(PLAN_COOLDOWN_AFTER_FAILURE):
            ...
            assert result is False, (
                f"Expected False at delta={delta} (within cooldown)"
            )
...
class TestCooldownReplanningAllowed:
    ...
    def test_cooldown_removed_after_successful_replan(self):
        ...
        pb.plan_mop_path(activity, None, [], current_iteration=PLAN_COOLDOWN_AFTER_FAILURE)
...
class TestMopResolveTracking:
    ...
    def test_tracking_emitted_on_failure(self):
        ...
        with patch("rv_agent.strategies.rvagent_strategy.path_buffer.track") as mock_track:
            pb.plan_mop_path(
                "com.example.ActivityA", None, [], current_iteration=1
            )

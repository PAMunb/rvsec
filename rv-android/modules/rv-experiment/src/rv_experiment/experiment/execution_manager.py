...
class ExecutionManager:
    ...
    def __init__(self, storage: TaskStorage, config: ExperimentConfiguration, event_bus: Optional[EventBus] = None):
        ...
        self.base_results_dir = self.config.output_dir
    ...
    def register_tool(self, tool: AbstractTool) -> None:
        ...
        with error_context(component="ExecutionManager", phase="tool_registration"):
            if not hasattr(tool, 'name') or not tool.name:
                raise RVExperimentError("Tool must have a valid name", experiment_id="current")
    ...
    def register_app(self, app: App) -> None:
        ...
        Args:
            app: App instance
        """
        # Example: Using auto-introspection with minimal context
        try:
            if not hasattr(app, 'name') or not app.name:
                raise RVExperimentError("App must have a valid name", experiment_id="current")
            
            self.apks[app.name] = app
            self.logger.debug(f"Registered app: {app.name}")
        except Exception as e:
            # Auto-introspection will capture class name, method, file, line automatically
            self.error_handler.handle_error_with_introspection(e, app_name=getattr(app, 'name', 'unknown'))
    ...
    def run_all_tasks(self) -> bool:
        ...
        try:
            ...
            for i, task in enumerate(tasks, 1):
                self.logger.info(
                    f"EXPERIMENT STATUS: Executing task {i}/{len(tasks)} (Total progress: {len(completed_tasks) + i - 1}/{len(tasks) + len(completed_tasks)})")
                ...
                self.logger.info(
                    f"EXPERIMENT STATUS: Overall progress: {completed_count}/{total_count} ({progress_pct:.1f}%)")
            ...
            # Example: Using fluent context builder - cleaner than manual dict construction
            context_data = {
            ...
            if self.current_task:
                context_data["current_task_id"] = self.current_task.id
                context_data["current_task_config"] = self.current_task.config.to_dict()

            # Use fluent context building - more readable and maintainable
            self.error_handler.create_context()\
                .with_component("ExecutionManager")\
                .with_phase("experiment_execution")\
                .with_data(**context_data)\
    ...
    def run_task(self, task: Task) -> bool:
        ...
        self.logger.info(
            f"TASK CONFIG: Clean-logcat={task.config.clean_logcat}, Skip-install={task.config.skip_installation}")
        ...
        self.logger.info(
            f"TASK RESOLUTION: App path: {app.instrumented_path if hasattr(app, 'instrumented_path') else 'N/A'}")
        ...
        try:
            ...
            execution_duration = (
                    datetime.now() - task_start_time).total_seconds() if 'task_start_time' in locals() else 0
            ...
            self.logger.error(
                f"TASK EXCEPTION: Task {task.id} failed with exception after {execution_duration:.2f} seconds")
    ...
    @ErrorHandler.handle_errors(component="ExecutionManager", phase="static_analysis_file_copy")

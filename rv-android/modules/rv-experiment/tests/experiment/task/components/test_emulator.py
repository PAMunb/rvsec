...
from rv_experiment.experiment.task.components.emulator import EmulatorComponent
...
def emulator_component(mock_task, mock_event_bus):
    with patch('rv_android_core.util.emulator_manager.EmulatorManager') as manager_class, \
            patch('rv_android_core.util.logging.manager.LoggingManager') as logging_manager, \
            patch('rv_android_core.util.error.error_handler.ErrorHandler.get_instance') as error_handler:

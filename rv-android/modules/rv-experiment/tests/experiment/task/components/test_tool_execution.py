...
from rv_experiment.experiment.task.components.tool_execution import ToolExecutionComponent
...
def tool_execution_component(mock_task, mock_tool, mock_event_bus, mock_error_handler, mock_logger):
    with patch('rv_android_core.util.error.error_handler.ErrorHandler.get_instance', return_value=mock_error_handler), \
            patch('rv_android_core.util.logging.manager.LoggingManager.get_instance') as mock_logging_manager:
...
def test_execute_tool_without_event_bus(mock_task, mock_tool, mock_logger):
    with patch('rv_android_core.util.error.error_handler.ErrorHandler.get_instance'), \
            patch('rv_android_core.util.logging.manager.LoggingManager.get_instance') as mock_logging_manager:
        mock_logging_manager.return_value.get_logger.return_value = mock_logger

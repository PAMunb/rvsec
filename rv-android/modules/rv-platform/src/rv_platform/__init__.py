...
Registers external tools (rvagent, rvsmart) into the rv-tools registry on
...
def _register_external_tools():
    ...
    # Register RVSmart tool if available and not already registered
    if not registry.is_tool_registered("rvsmart"):
        try:
            from rvsmart_tool.tools.rvsmart.tool import RVSmartTool
            registry.register_tool_class(RVSmartTool)
        except ImportError as e:
            logging.getLogger(__name__).warning(f"rvsmart tool not available: {e}")
        except Exception as e:

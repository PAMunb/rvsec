...
class ResultManager:
    ...
    def _write_task_coverage_data(self, writer: csv.writer, task: Any) -> None:
        ...
        try:
            ...
            if hasattr(task, 'repository') and task.repository:
                ...
                total_activities = len(repository.get_static_activities()) if hasattr(repository,
                                                                                      'get_static_activities') else 0
        ...
        try:
            ...
            if hasattr(task, 'repository') and task.repository:
                ...
                for i, error in enumerate(errors, 1):
                    ...
                    unique_msg = error.get('unique_msg',
    ...
    def _extract_task_data(self, task: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            task_data = {
                "start_time": getattr(task.result, 'start_time', datetime.now()).timestamp() if hasattr(task,
                                                                                                        'result') else None,
                "summary": {},
    ...
    def _create_experiment_summary(self, completed_tasks: List[Any]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Experiment summary: {total_tasks} tasks, {unique_apks} APKs, {unique_tools} tools, {avg_method_coverage:.2f}% avg coverage, {total_errors} total monitored operations errors")

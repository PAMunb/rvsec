...
from rv_android_core.llm import LLMRole
from rv_android_core.parser.screen.abstract_parser import AbstractScreenParser
from rv_android_core.parser.screen.parser_factory import ParserType, ParserFactory
                    ...
                    for edge in edges[:5]:  # Limit to 5 transitions for brevity
                        try:
                            ...
                            for event in events[:2]:  # Limit events per transition
                                try:
                                    event_type_name = event.type.name if hasattr(event, 'type') and hasattr(event.type,
                                                                                                            'name') else "unknown event"

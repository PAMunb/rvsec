...
from rv_android_core.llm.constants import PromptStrategyType, LLMType
...
from rv_android_core.parser.screen.parser_factory import ParserType
...
class ComponentConfigurator:
    ...
    def _initialize_registries(self):
        ...
        if not self._registries['llm'].get_names():
            self._registries['llm'].register_lazy('ollama', 'rv_android_core.llm.ollama_llm', 'OllamaLLM')
            self._registries['llm'].register_lazy('huggingface', 'rv_android_core.llm.huggingface_llm', 'HuggingFaceLLM')
            self._registries['llm'].register_lazy('frontier', 'rv_android_core.llm.frontier_models', 'FrontierModel')

        # Register strategy types
        if not self._registries['strategy'].get_names():
            self._registries['strategy'].register_lazy(
                PromptStrategyType.STANDARD, 'rv_android_core.llm.prompt.strategy.strategies.standard_strategy',
                'StandardStrategy')
            self._registries['strategy'].register_lazy(
                PromptStrategyType.BATCH_ACTION, 'rv_android_core.llm.prompt.strategy.strategies.batch_action_strategy',
                'BatchActionStrategy')

        # Register parser types
        if not self._registries['parser'].get_names():
            self._registries['parser'].register_lazy(
                'droidbot', 'rv_android_core.parser.screen.droidbot.droidbot_parser', 'DroidBotParser')
            self._registries['parser'].register_lazy(
                'uiautomator', 'rv_android_core.parser.screen.uiautomator.uiautomator_parser', 'UIAutomator2Parser')

        # Register visitor types
        if not self._registries['visitor'].get_names():
            self._registries['visitor'].register_lazy(
                'basic', 'rv_android_core.parser.screen.visitor.basic_visitor', 'BasicTextVisitor')
            self._registries['visitor'].register_lazy(
                'default', 'rv_android_core.parser.screen.visitor.default_visitor', 'DefaultTextVisitor')
            self._registries['visitor'].register_lazy(
                'enhanced', 'rv_android_core.parser.screen.visitor.enhanced_visitor', 'EnhancedTextVisitor')
    ...
    def _get_config_manager(self):
        ...
        if self.config_manager is None:
            ...
            from rv_android_core.config.configuration_manager import ConfigurationManager

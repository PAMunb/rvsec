...
from accelerate.commands.config.config import config_command

from rv_android_core.config.component_configurator import ComponentConfigurator
from rv_android_core.llm.data_structures import LLMMessage, LLMRole, LLMTextContent, LLMResponse
from rv_android_core.llm.service.response_processor import ResponseProcessor
from rv_android_core.util.logging.constants import CONTEXT_COMPONENT
from rv_android_core.util.logging.manager import LoggingManager
from .config import (
    MODELS_TO_TEST, STRATEGIES_TO_TEST, REPETITIONS_PER_CONFIG, WARMUP_RUNS,
...
from ..llm.constants import PromptStrategyType, ScreenParserType
from ..llm.ollama_llm import OllamaLLM
from ..parser.screen.visitor.visitor_factory import VisitorFactory
...
class LLMEvaluator:
    ...
    def __init__(self, prompts_dir: str = None, output_dir: str = "."):
        """
        ...
        self.current_configurator = None
    ...
    def run_evaluation(self) -> Tuple[str, str, str]:
        ...
        try:
            ...
            self.logger.info(f"  - {len(STRATEGIES_TO_TEST)} strategies")
    ...
    def _group_configurations_by_model(self,
                                       configurations: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def _evaluate_model_group(self, model_name: str, configurations: List[Dict[str, Any]]) -> None:
        """
    ...
    def _initialize_model(self, model_name: str) -> None:
        ...
        try:
            # Create configurator for this model
            self.current_configurator = ComponentConfigurator()
            # self.current_configurator.set_llm("ollama", model_name)
            # TODO
            self.current_configurator.set_llm(
                llm_type=OllamaLLM.NAME,
                model=model_name,
                base_url="http://192.168.0.20:11434"
            )
            self.current_configurator.set_strategy(PromptStrategyType.BATCH_ACTION)
            self.current_configurator.set_parser(ScreenParserType.DROIDBOT)
            self.current_configurator.set_visitor(VisitorFactory.DEFAULT)
    ...
    def _perform_warmup(self, sample_config: Dict[str, Any]) -> None:
        """
        ...
        prompt_id, system_file, user_file = self.prompt_pairs[0]

        for i in range(WARMUP_RUNS):
            try:
                self._execute_single_run(sample_config, prompt_id, system_file, user_file, is_warmup=True)
                self.logger.debug(f"Warm-up run {i + 1}/{WARMUP_RUNS} completed")
            except Exception as e:
                self.logger.warning(f"Warm-up run {i + 1} failed: {e}")

    def _evaluate_configuration(self, config: Dict[str, Any]) -> None:
        """
        ...
        for prompt_id, system_file, user_file in self.prompt_pairs:
            ...
            for run_number in range(1, REPETITIONS_PER_CONFIG + 1):
                try:
                    result = self._execute_single_run(config, prompt_id, system_file, user_file, run_number)
    ...
    def _execute_single_run(self,
        ...
        try:
            # Update configurator with current configuration
            self._update_configurator(config)

            # Load prompts
            system_prompt, user_prompt = self._load_prompts(system_file, user_file)
            ...
            response, parsed_actions, parsing_errors = self._execute_with_timeout(messages)
    ...
    def _update_configurator(self, config: Dict[str, Any]) -> None:
        """
        Update the current configurator with configuration parameters.

        Args:
            config: Configuration dictionary
        """
        if not self.current_configurator:
            raise RuntimeError("No configurator initialized")
        ...
        # Update strategy
        self.current_configurator.set_strategy(config['strategy'])
        # TODO update visitor

    def _load_prompts(self, system_file: str, user_file: str) -> Tuple[str, str]:
        """
    ...
    def _execute_with_timeout(self, messages: List[LLMMessage]) -> Tuple[
        ...
        def _generate():
            try:
                ...
                llm = self.current_configurator.create_llm()
                response = llm.generate(messages, self.current_configurator.llm_config)
    ...
    def _calculate_summary_statistics(self) -> None:
        ...
        for config_key, runs in config_groups.items():
            ...
            summary.update({
                'model': config_key[0],
                'strategy': config_key[1],
                'prompt_id': config_key[2],
                'temperature': config_key[3],
                'top_p': config_key[4],
                'max_tokens': config_key[5],
                'top_k': config_key[6],
                'total_runs': len(runs)
            })
    ...
    def _cleanup_current_model(self) -> None:
        ...
        if self.current_configurator:
            try:
                # Try to clean up LLM if it was created
                llm = getattr(self.current_configurator, '_llm_instance', None)

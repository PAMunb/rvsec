...
class TestConfigurationManager:
    ...
    def config_manager(self, mock_config):
        ...
        with patch('rvandroid.config.configuration_manager.Configuration') as mock_config_cls:
    ...
    @patch('rvandroid.config.configuration_manager.AbstractTool')
    def test_get_tools(self, mock_abstract_tool, config_manager, mock_config):
        ...
        # Setup mock returns
        mock_config.get.return_value = [tool1, tool2, tool3]

        # Call method with specific tool selection
        selected_tools = config_manager.get_tools(["monkey", "humanoid"])

        # Verify result
        assert len(selected_tools) == 2
        assert selected_tools[0] == tool1
        assert selected_tools[1] == tool3

        # Verify config methods were called
        mock_config.get.assert_called_once_with("available_tools", [])

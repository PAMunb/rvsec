...
class RVInstrumentation:
    ...
    def instrument_apks(self, apks_dir: str, results_dir: str, force_instrumentation=False) -> Dict[
        ...
        try:
            ...
            self._logger.error("Failed to prepare instrumentation environment", extra={
                'error': str(e),
        ...
        for index, app in enumerate(apks, 1):
            self._logger.info(f"Processing APK {index}/{total_apks}: {app.name}", extra={
                'app_name': app.name,
            ...
            try:
                ...
                self._logger.error(f"General error while instrumenting APK: {app.name}", extra={
                    'app_name': app.name,
                    'app_path': app.path,
                    'error': str(ex),
    ...
    def prepare_instrumentation(self, results_dir: str) -> None:
        ...
        """
        self._logger.info("Preparing instrumentation environment", extra={
            'results_dir': results_dir,
    ...
    def instrument(self, app: App, result_dir: str, force_instrumentation: bool = False) -> None:
        ...
        if os.path.exists(instrumented_apk):
            if force_instrumentation:
                self._logger.info(f"Force re-instrumentation: removing existing APK", extra={
                    'app_name': app.name,
                ...
                self._logger.info(f"Skipping already instrumented APK: {app.name}", extra={
                    'app_name': app.name,
                    'pipeline_stage': 'skip_existing'
                })
                return

        start = time.time()
        self._logger.info(f"Starting APK instrumentation: {app.name}", extra={
            'app_name': app.name,
            'app_path': app.path,
        ...
        try:
            ...
            if not os.path.exists(signed_apk):
                raise InstrumentationError(
                    f"Failed to create signed APK: {signed_apk}",
                    None
                )
            ...
            self._logger.error(f"Instrumentation pipeline failed for APK: {app.name}", extra={
                'app_name': app.name,
                'error': str(e),
    ...
    def __decompile_apk(self, app: App) -> None:
        ...
        """
        self._logger.info(f"Starting APK decompilation: {app.name}", extra={
            'app_name': app.name,
    ...
    def __execute_maven(self) -> None:
        ...
        """
        self._logger.info("Executing Maven dependency resolution", extra={
    ...
    def __include_generated_monitors(self) -> None:
        ...
        """
        self._logger.info("Including generated runtime verification monitor artifacts", extra={
    ...
    def __weave_monitors(self, app: App) -> None:
        ...
        """
        self._logger.info(f"Starting AspectJ monitor weaving for: {app.name}", extra={
            'app_name': app.name,
    ...
    def __create_apk(self, app: App) -> str:
        ...
        """
        self._logger.info(f"Creating instrumented APK for: {app.name}", extra={
            'app_name': app.name,
    ...
    def __d8(self, app: App, monitored_jar: str) -> str:
        ...
        """
        self._logger.info(f"Compiling instrumented classes to DEX format: {app.name}", extra={
            'app_name': app.name,
        ...
        self._logger.info(f"Integrating instrumented DEX into APK", extra={
    ...
    def __sign_apk(self, app: App, unsigned_apk: str) -> str:
        ...
        self._logger.info(f"Starting APK signing process: {app.name}", extra={
            'app_name': app.name,
            'unsigned_apk': unsigned_apk,
        ...
        if not os.path.exists(signed_apk):
            raise InstrumentationError(
                f"dex2jar APK signing failed: {signed_apk}",
                None
            )
        ...
        self._logger.info(f"APK signing completed successfully: {signed_apk}", extra={
            'app_name': app.name,
            'signed_apk': signed_apk,
    ...
    def __jarsigner(self, signed_apk: str) -> None:
        ...
        jarsigner_cmd = Command('jarsigner', [
            ...
            '-digestalg', 'SHA-256',
            '-keystore', self.config.keystore_file,
            signed_apk, 'server',
            '-storepass', self.config.keystore_password
        ])

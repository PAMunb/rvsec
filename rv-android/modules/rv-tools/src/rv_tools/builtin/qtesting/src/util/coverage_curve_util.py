...
def copy_restart_coverage_file(target_based_coverage_file, original_coverage_file):
    ...
    for f in file_list:
        if ".ec" in f:
            shutil.copy2(
                os.path.join(target_based_coverage_file, f), original_coverage_file
            )
...
def file_compare(file1, file2):
    ...
    if "curve" not in file1 and ".ec" in file1:
        ...
        created_time_1 = os.path.getctime(
            os.path.join(TARGET_BASED_COVERAGE_FILE, file1)
        )
    else:
    ...
    if "curve" not in file2 and ".ec" in file2:
        ...
        created_time_2 = os.path.getctime(
            os.path.join(TARGET_BASED_COVERAGE_FILE, file2)
        )
    else:
        ...
        if "curve" not in f and ".ec" in f:
            print(
                (
                    ...
                    + f
                    + " created time: "
                    + str(os.path.getctime(os.path.join(TARGET_BASED_COVERAGE_FILE, f)))
                )
            )
        else:
            print(
                (
                    ...
                    + f
                    + " created time: "
                    + str(os.path.getctime(os.path.join(original_coverage_file, f)))
                )
            )
    ...
    for f in file_list:
        ...
        if "curve" not in f:
            ...
            (
                ...
                class_coverage,
                activity_coverage,
            ) = get_coverage_data(app_source_file)
...
def generate_test_report(new_coverage_file, app_source_file):
    ...
    cmd = (
        "gradle jacocoTestReportMergeWithParameter -PnewCoverageFilePath="
        + new_coverage_file
    )
    os.system(cmd)
...
def get_coverage_data(app_source_file):
    ...
    file_path = (
        app_source_file
        + "/build/reports/jacoco/jacocoTestReportMergeWithParameter"
        + "/jacocoTestReportMergeWithParameter.xml"
    )
    if APP_SOURCE_FILE == "/home/tim/AndroidStudioProjects/Q-testing/runnerup":
        file_path = (
            app_source_file
            + "/app/build/reports/jacoco/jacocoTestReportMergeWithParameter"
            + "/jacocoTestReportMergeWithParameter.xml"
        )
...
def save_curve_data(
    ...
    for data_id in list(sorted_instruction_data.keys()):
        if data_id == "final":
            f.write(
                "time: final\t"
                + "instruction_coverage: "
                + str(round(sorted_instruction_data[data_id], 4))
                + "\t"
            )
            f.write(
                "branch_coverage: " + str(round(sorted_branch_data[data_id], 4)) + "\t"
            )
            f.write("cxty_coverage: " + str(round(sorted_cxty_data[data_id], 4)) + "\t")
            f.write("line_coverage: " + str(round(sorted_line_data[data_id], 4)) + "\t")
            f.write(
                "method_coverage: " + str(round(sorted_method_data[data_id], 4)) + "\t"
            )
            f.write(
                "class_coverage: " + str(round(sorted_class_data[data_id], 4)) + "\t"
            )
            f.write(
                "activity_coverage: "
                + str(round(sorted_activity_data[data_id], 4))
                + "\n"
            )
        else:
            f.write(
                "time: "
                + str(int(data_id) * TIME_INTERVAL)
                + "\t"
                + "instruction_coverage: "
                + str(round(sorted_instruction_data[data_id], 4))
                + "\t"
    ...
    f.write(
        "30min:\t"
        + "instruction_coverage: "
        + str(round(sorted_instruction_data[MAX_ID / 2], 4))
        + "\t"
    )
    ...
    f.write(
        "activity_coverage: " + str(round(sorted_activity_data[MAX_ID / 2], 4)) + "\n"
    )
    f.write(
        "60min:\t"
        + "instruction_coverage: "
        + str(round(sorted_instruction_data["final"], 4))
        + "\t"
    )
...
def calculate_coverage():
    ...
    TARGET_BASED_COVERAGE_FILE = (
        Benchmark + APK_NAME.split(".")[0] + "-output-1/coverage_merge_1"
    )
    ORIGINAL_COVERAGE_FILE = (
        Benchmark + APK_NAME.split(".")[0] + "-output-1/coverage_original_1"
    )
    ...
    (
        ...
        class_data,
        activity_data,
    ) = get_curve_data(ORIGINAL_COVERAGE_FILE, NEW_COVERAGE_FILE, APP_SOURCE_FILE)
    save_curve_data(
        instruction_data,
        branch_data,
        cxty_data,

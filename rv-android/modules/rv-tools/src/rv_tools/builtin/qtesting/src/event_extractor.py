...
class EventExtractor(object):
    ...
    def __init__(
        ...
        self.command_file_path = (
            out_path + "/event_output/" + "command_" + episode + ".txt"
        )
        ...
        for node in node_list:
            if node.tag == "node":
                ...
                if len(node):
                    ...
                    self.analyze_node(
                        node,
                        parent_clickable or self.str2bool(attrib["clickable"]),
                        parent_long_clickable
                        or self.str2bool(attrib["long-clickable"]),
                        parent_checkable or self.str2bool(attrib["checkable"]),
                        parent_scrollable or self.str2bool(attrib["scrollable"]),
                    )
                else:
                    cis = self.getCIS()
                    self.logs.append("leaf node ok!!")
                    executable_obj = UIExecutableObject(
                        cis, attrib["package"], attrib["class"]
                    )
                    executable_obj.set_event_property(
                        attrib["clickable"],
                        attrib["long-clickable"],
                        attrib["scrollable"],
                        attrib["checkable"],
                        attrib["text"],
                        attrib["resource-id"],
                        ...
                        attrib["index"],
                        attrib["enabled"],
                        self.get_view_instance(attrib["class"]),
                        self.get_id_instance(attrib["resource-id"]),
                    )
                    self.logs.append("cis = " + cis)
                    if parent_long_clickable:
                        self.logs.append(
                            "parent node's long-clickable is true, set its own's long-clickable as true"
                        )
                        executable_obj.set_long_clickable("true")
                    if parent_clickable:
                        self.logs.append(
                            "parent node's clickable is true, set its own's clickable as true"
                        )
                        executable_obj.set_clickable("true")
                    if parent_checkable:
                        self.logs.append(
                            "parent node's checkable is true, set its own's checkable as true"
                        )
    ...
    def add_invokable_action(self, action):
        self.logs.append(
            ...
            + "> into the app state's invokable actions list"
        )
    ...
    def generate_click_action(self, executable_obj, action_type):
        ...
        action.set_action_property(
            self.actionIDCounter,
            executable_obj.cis,
            executable_obj.text,
            action_type,
            self.activity_name,
        ...
        if executable_obj.text != "":
            action_cmd = (
                action_type
                + "(className='"
                + executable_obj.class_name
                + "',instance='"
                + str(executable_obj.instance)
                + "')\n"
            )
            ...
            action_cmd = (
                action_type
                + "(resource-id='"
                + executable_obj.resource_id
                + "', instance='"
                + str(executable_obj.id_instance)
                + "')\n"
            )
        elif executable_obj.content_desc != "":
            action_cmd = (
                action_type + "(content-desc='" + executable_obj.content_desc + "')\n"
            )
        else:
            return ""
        ...
        if executable_obj.resource_id != "":
            ...
            if executable_obj.has_multi_lines_text():
                action_cmd = (
                    action.click
                    + "(textContains='"
                    + executable_obj.get_first_line_text()
                    + "')\n"
                )
            else:
            ...
            action_cmd = (
                action.click + "(content-desc='" + executable_obj.resource_id + "')\n"
            )
        else:
            action_cmd = (
                action.click
    ...
    def generate_check_textview_action(self, executable_obj):
        ...
        if executable_obj.text != "":
            ...
            action_cmd = (
                action.click + "(content-desc='" + executable_obj.content_desc + "')\n"
            )
        else:
            return ""
        action.set_action_cmd(action_cmd, executable_obj.class_name)
    ...
    def detect_invokable_actions(self):
        for executable_obj in self.ui_executable_objects:
            ...
            if (
                ".TextView" in view_type
                or ".Button" in view_type
                or ".ToggleButton" in view_type
            ):
                self.logs.append(
                    "clickable = "
                    + str(executable_obj.clickable)
                    + ", long_clickable = "
                    + str(executable_obj.long_clickable)
                )
                ...
                self.logs.append(
                    self.debug + "I: up to now, detected invokable actions: "
                )
        ...
        self.logs.append("==========")
        self.logs.append(
            self.debug + "I: the final detected invokable actions on this app state: "
        )
        self.print_invokable_actions()
    ...
    def add_system_actions(self):
        self.logs.append(
            self.debug
            + "I: add possible SYSTEM actions for the app state in the Activity: "
            + self.activity_name
        )
        if self.activity_name == self.entry_activity_name:
            ...
            self.actionIDCounter += 1
            view_cis = ""
            menu_action.set_system_action_property(
                self.actionIDCounter,
                view_cis,
                ActionConstValue.MenuActionCmd,
                self.activity_name,
            )
            self.add_invokable_action(menu_action)
        back_action = Action()
        self.actionIDCounter += 1
        back_action.set_system_action_property(
            self.actionIDCounter,
            self.CIS,
            ActionConstValue.BackActionCmd,
            self.activity_name,
        )

...
class Preprocess(object):
    ...
    def extract_vector_from_node(self, node):
        ...
        if bounds_string == "" or not bounds_string:
            ...
            rb_bounds_string = bounds_string[
                bounds_string.rfind("[") : len(bounds_string)
            ]
            lt_x = int(lt_bounds_string[1 : lt_bounds_string.find(",")])
            lt_y = int(
                lt_bounds_string[
                    lt_bounds_string.find(",") + 1 : lt_bounds_string.rfind("]")
                ]
            )
            rb_x = int(rb_bounds_string[1 : rb_bounds_string.find(",")])
            rb_y = int(
                rb_bounds_string[
                    rb_bounds_string.find(",") + 1 : rb_bounds_string.rfind("]")
                ]
            )
        ...
        if len(md5_str_10) > 40:
            logging.error("length is beyond 40")
        for i in range(0, len(md5_str_10)):
            node_vector[len(self.widget_class_list) + 46 + i] = (
                float(md5_str_10[i]) / 10
            )
            if i >= 39:
                break
        ...
        if len(md5_str_10) > 40:
            logging.error("length is beyond 40")
        for i in range(0, len(md5_str_10)):
            node_vector[len(self.widget_class_list) + 86 + i] = (
                float(md5_str_10[i]) / 10
            )
            if i >= 39:
                break
        ...
        if len(md5_str_10) > 40:
            logging.error("length is beyond 40")
        for i in range(0, len(md5_str_10)):
            node_vector[len(self.widget_class_list) + 126 + i] = (
                float(md5_str_10[i]) / 10
            )
            if i >= 39:
                break

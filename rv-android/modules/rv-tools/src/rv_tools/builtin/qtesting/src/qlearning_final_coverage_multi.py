...
class QLearning(object):
    ...
    def setup(self):
        ...
        self.model = tf.keras.models.load_model(
            self.bundle_dir + "/models/SiameseLSTM.h5",
        ...
        path_to_state_feature = os.path.abspath(
            os.path.join(bundle_dir, self.bundle_dir + "/util/STATE-FEATURES.txt")
        )
    ...
    def update_recycler_info(self, action_string, current_state, next_state):
        ...
        if '@"recycler items_' in action_string and value == INIT_QVALUE:
            ...
            self.recycler_label = action_string[
                action_string.rindex("@") + 2 : len(action_string) - 2
            ]
            if (
                self.current_similarity > DIFFERENCE_RATE_THRESHOLD
                and self.most_similar_state_id > 0
            ):
                self.recycler_label = (
                    str(self.most_similar_state_id) + "_" + self.recycler_label
                )
            else:
                self.recycler_label = (
                    str(self.current_state_id) + "_" + self.recycler_label
                )
        else:
            self.resolve_recycler_flag = 0
        if self.resolve_recycler_flag == 1:
            if self.recycler_label not in self.items_transition:
                ...
                self.items_transition[self.recycler_label] == next_state
                and self.items_count[self.recycler_label] != -1
            ):
                self.items_count[self.recycler_label] += 1
            else:
                self.items_count[self.recycler_label] = -1
    ...
    def resolve_recycler_items(self, parent_node, path_label):
        if "RecyclerView" in str(parent_node.get("class")):
            ...
            if len(parent_node) == 0:
                return
            items_string = (
                "recycler items_" + self.current_activity_name + "_" + path_label
            )
            ...
            if (
                ...
                checking_items_string = (
                    str(self.most_similar_state_id) + "_" + items_string
                )
            else:
                checking_items_string = str(self.current_state_id) + "_" + items_string
            if (
                checking_items_string in self.items_count
                and self.items_count[checking_items_string] >= 3
            ):
                count = 0
                remove_list = []
    ...
    def get_current_state_retry(self, episode):
        for tryTimes in range(2):
            ...
            if (
                "WAITFORASECOND" != next_state
                and "ERROR STATE" != next_state
                and "UNKNOWN" != next_state
            ):
                return next_state
    ...
    def get_widget_hierarchy_state(self):
        ...
        for child in root1:
            if (
                ...
                and child.get("package") == "com.android.systemui"
    ...
    def get_random_action(self, current_state):
        ...
        for event in events:
            if (
                event not in action_table
                and "com.android.systemui:id/home_button" not in event
                and "com.android.systemui:id/recent_apps" not in event
            ):
        ...
        while (
            "com.android.systemui:id/home_button" in action_string
            or "com.android.systemui:id/recent_apps" in action_string
        ):
            random_num = random.randint(1, len(events))
            action_string = events[random_num - 1]
    ...
    def get_action(self, current_state, episode):
        ...
        return (action_string, action_cmd)
    ...
    def execute_action(self, action_cmd):
        if self.system_flag == 2:
            ...
            action_parameter_value = str(action_cmd)[
                first_quote_index : last_quote_index + 1
            ]
            event_cmd = (
                "timeout 2s python "
                + self.bundle_dir
    ...
    def get_memory_based_reward2(self, next_state, eposide):
        ...
        if self.system_flag != 0:
            ...
            next_feature_vector = self.intermediate_model.predict(
                np.array(next_state_vector)
            )
            ...
            similarity, self.most_similar_state_id = self.compare_states2(
                memory_feature_vectors, next_feature_vector
            )
    ...
    def compare_states2(memory_feature_vectors, next_feature_vector):
        ...
        for similar_id in memory_feature_vectors:
            ...
            temp_similarity = np.exp(
                -np.sum(np.abs(memory_feature_vector - next_feature_vector), axis=1)
            )
            if temp_similarity > similarity:
                similarity = temp_similarity
                most_similar_id = similar_id

        return (similarity, most_similar_id)
    ...
    def update_qtable(self, current_state, next_state, action, reward):
        ...
        if current_state not in self.q_table:
            print(
                (
                    "the current states is not in q_table and the current activity is "
                    + self.current_activity_name
                )
            )
            return
    ...
    def get_final_coverage(self):
        ...
        file_path = (
            ...
            + "/build/reports/jacoco/jacocoTestReportMerge"
            + str(TEST_INDEX)
            + "/jacocoTestReportMerge"
            + str(TEST_INDEX)
            + ".xml"
        )
    ...
    def qlearning_loop(self):
        ...
        while episode < STEP1:
            ...
            for step in range(10):
                ...
                t1 = time.time()
                next_state = self.get_current_state_retry(
                    str(episode) + "_" + str(step)
                )
                t2 = time.time()
                c_get_state1 = t2 - t1
    ...
    def fix_error_state_cant_escape(self):
        if (
            self.current_package_name != self.package_name
            and self.current_activity_name == self.previous_activity_name
        ):
            cmd = (
                "adb -s "
                + DEVICE_ID

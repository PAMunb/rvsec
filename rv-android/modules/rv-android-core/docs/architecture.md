...
| INV-CORE-25 | RvErrorLog deduplication via unique_msg, composed in exactly one place | Computed as `"{class}:::{method}:::{spec}:::{error_type}:::{code}:::{event}:::{identity_message}"` — seven `:::` parts, at event granularity. `code`/`event` are the `code=`/`ev=` values of the message envelope, or `UNSPECIFIED` when the record carries none. `identity_message` is `message` without the trailing evidence keys ` vfp='…'` and ` vcls='…'` (INV-CORE-63); a message without them is used byte for byte, and `message` itself keeps them. Readers (e.g. rv-platform's `errors.csv` writer) read this key, never rebuild it |
...
2. **LogcatManager** captures raw logcat to a file on disk. Before each capture it runs `adb -s <serial> logcat -G 16M` (`LOGCAT_BUFFER_SIZE`), then `logcat -c`, then the unchanged capture command; a failed sizing is logged at WARNING and the capture proceeds (INV-CORE-64).

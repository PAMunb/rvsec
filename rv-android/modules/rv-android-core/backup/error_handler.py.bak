...
class ErrorHandler:
    ...
    def _handle_circuit_breaker_open_error(self, error: CircuitBreakerOpenError, context: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        """
        self.logger.warning(f"Circuit breaker protection activated: {error.message}")
        if hasattr(error, 'command_signature') and error.command_signature:
            self.logger.warning(f"Blocked command: {error.command_signature}")
        if hasattr(error, 'failure_count') and error.failure_count:
            self.logger.warning(f"Previous failures: {error.failure_count}")
        
        # Log context if available
        if context:
            component = context.get('component', 'unknown')
            tool_name = context.get('tool_name', 'unknown')
            self.logger.warning(f"Tool: {tool_name}, Component: {component}")

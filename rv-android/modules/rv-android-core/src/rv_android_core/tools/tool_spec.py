...
class ToolSpec(BaseValidatedModel):
    ...
    name: str = Field(description="Tool name for identification and registry")
    description: str = Field(
        description="Detailed description of tool capabilities and purpose"
    )
    url: str = Field(description="Tool repository, documentation, or download URL")
    version: str = Field(description="Tool version string for compatibility tracking")
    process_pattern: Optional[str] = Field(
        default=None, description="Process pattern for tool cleanup and management"
    )

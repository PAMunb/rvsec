...
from rv_android_core.util.performance.configuration import PerformanceMonitorConfig
...
class PerformanceMonitor:
    ...
    This monitor can be configured to reduce overhead in production
    environments through CLI arguments passed to the system.
    """

    _instance = None
    _lock = threading.Lock()
    _config: Optional[PerformanceMonitorConfig] = None

    @classmethod
    def configure(cls, config: PerformanceMonitorConfig):
        """
    ...
    def get_instance(cls):
        """Get the singleton instance with global configuration."""
        with cls._lock:
    ...
    def _is_enabled(self) -> bool:
        ...
        return self._config is None or self._config.enabled

    def _should_log_metric(self) -> bool:
        ...
        return self._config.log_level != 'DISABLED'
    ...
    def record_metric(self, name: str, value: float, unit: str = "", context: Optional[Dict[str, Any]] = None):
        """
        Record a general metric if monitoring is enabled.
        ...
        # Respect max_samples configuration
        if self._config and self._config.max_samples > 0:
            if len(self.metrics) >= self._config.max_samples:
                # Remove oldest metrics to make room
                self.metrics = self.metrics[-(self._config.max_samples - 1):]
    ...
    def measure_time(self, name: str, context: Optional[Dict[str, Any]] = None):
        ...
        """
        if not self._is_enabled():
            # If monitoring is disabled, just yield without measuring
            yield
            return

...
# TAG_RVSEC and TAG_RVSEC_COV are re-exported through this module's namespace:
# consumers import them from `rv_android_core.domain.log` alongside the log models
# they tag. They are unused inside this file by design — do not remove.
from rv_android_core.util.logging.constants import (  # noqa: F401
    TAG_RVSEC,
    TAG_RVSEC_COV,
)
...
# The evidence keys a `-NOBS-` envelope may append after `msg`: ` vfp='…'`, then
# ` vcls='…'`, each optional, in that order, at the very end of the message. The
# match must start right after a closing quote (the end of `msg`), and each value is
# read escape-aware (`\'` and any other `\x` pair stay inside the value). A `vfp=`
# written inside a quoted value cannot match: the producer escapes every quote inside
# a value, so the text there reads `vfp=\'`, never `vfp='`.
_EVIDENCE_SUFFIX = re.compile(
    r"(?<=')(?: vfp='(?:[^'\\]|\\.)*')?(?: vcls='(?:[^'\\]|\\.)*')?$"
)
...
class RvErrorLog(BaseValidatedModel):
    ...
    # The six fields below are the keys of the v1 message envelope
    # `v=1 code=<SPEC>-<KIND>-<NN> ev=<event> obj=<SimpleClass> val='<observed>'
    # exp='<expected>' msg='<text>'` the jca_android monitors write into the seventh
    # comma field. They are plain optional strings rather than a sub-object because the
    # model already carries six flat fields and a single consumer would not pay for the
    # extra nesting. `code` and `event` default to the sentinel UNSPECIFIED and never to
    # "": a record produced by the frozen `jca` set, or persisted before this change,
    # carries no envelope at all, and a reader must be able to tell that from an
    # envelope whose value happened to be empty.
    code: str = Field(
        default="UNSPECIFIED",
        description="Stable violation code from the message envelope (`code=`), or UNSPECIFIED",
    )
    event: str = Field(
        default="UNSPECIFIED",
        description="Automaton event that failed, from the message envelope (`ev=`), or UNSPECIFIED",
    )
    obj: str = Field(
        default="", description="Simple class of the monitored object (`obj=`)"
    )
    val: str = Field(default="", description="Observed value (`val=`)")
    exp: str = Field(default="", description="Expected value or list (`exp=`)")
    msg: str = Field(
        default="", description="Human-readable text of the envelope (`msg=`)"
    )
    truncated: bool = Field(
        default=False,
        description="True when logcat cut the envelope: its last quoted value is unclosed",
    )
    # Evidence keys of a `-NOBS-` envelope. Their values are already inside `message`,
    # so they stay out of `to_dict()` (the written schema keeps its keys) and out of
    # the identity (`unique_msg` strips them, see `identity_message`).
    value_fingerprint: str = Field(
        default="", description="Fingerprint of the untraced value (`vfp=`)"
    )
    value_class: str = Field(
        default="", description="Runtime class(es) of the bound object (`vcls=`)"
    )
    ...
    def identity_message(self) -> str:
        """`message` without the trailing evidence keys `vfp` and `vcls` (INV-CORE-63).

        The evidence values differ from object to object and from run to run, so a
        key that kept them would count every run of one misuse as a different
        violation. Only the trailing position after `msg` is recognised; a message
        with no evidence key is returned byte for byte.
        """
        match = _EVIDENCE_SUFFIX.search(self.message)
        return self.message[: match.start()] if match else self.message

    @computed_field
    @property
    def unique_msg(self) -> str:
        """Unique identifier for this property violation, at **event** granularity.

        The key is `class:::method:::spec:::error_type:::code:::event:::message` — seven
        `:::`-separated parts — which is deliberately finer than the
        `(apk, class, method, spec)` key the thesis and the journal article use to count
        *unique misuses*. The seventh part is `identity_message`: `message` without the
        trailing evidence keys `vfp` and `vcls`, which stay in `message` itself. Every
        extra component carries real information: `error_type` separates a sequence
        violation from a constraint violation in the same method, `code` and `event`
        name the transition of the automaton that failed, and `message` is generated by
        the specification and names the offending parameter — `expecting one of
        {SHA-256, SHA-384, SHA-512} but found MD5` and `… but found SHA-1` are two
        genuinely different misuses of one method, not one misuse counted twice.

        `code` and `event` are the `code=` and `ev=` values of the message envelope. A
        record whose message carries no envelope — every record of the frozen `jca` set,
        and every record persisted before this change — holds the sentinel `UNSPECIFIED`
        in both, never the empty string, so a legacy record still has a readable
        seven-part key that a reader can tell apart from an envelope record whose event
        was named.

        **This is the only place the key is built.** It is `__hash__` and `__eq__` of the
        model, so a formula copied into a writer or a script would fork the identity the
        moment this one changed — which is exactly what adding `code` and `event` does.

        Two practical consequences, and the reason they are written here. First,
        `unique_errors` — and the `mop_errors_unique` column of `summary.csv` derived
        from it — is **not** numerically comparable to a unique-misuse count. Second,
        the identity changed with this key: a deduplicated count computed under the
        five-part key is not comparable to one computed under the seven-part key, and
        any published figure has to say which era it belongs to. All the figures are
        correct at their own granularity, and a reader comparing them without knowing
        this will conclude one of them is broken.

        `source` is deliberately absent: two occurrences of the same misuse at different
        source lines are one misuse, so a line number must never enter the identity of a
        violation.
        """
        return (
            f"{self.class_full_name}:::{self.method}:::{self.spec}:::{self.error_type}"
            f":::{self.code or 'UNSPECIFIED'}:::{self.event or 'UNSPECIFIED'}:::{self.identity_message}"
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        `source` is included so the line information survives into `errors.csv`.
        Excluding a field from the *identity* of a violation and *discarding* it are
        different decisions: the position must not identify a violation, but it is
        still the most direct pointer to where it happened and the evidence needed to
        audit a frame-form normalization after a campaign has run.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RvErrorLog":
        ...
        return cls(
            ...
            code=data.get("code") or "UNSPECIFIED",
            event=data.get("event") or "UNSPECIFIED",
            obj=data.get("obj", ""),
            val=data.get("val", ""),
            exp=data.get("exp", ""),
            msg=data.get("msg", ""),
    ...
    def __str__(self):
        return (
            ...
            f"code={self.code}, event={self.event}, "

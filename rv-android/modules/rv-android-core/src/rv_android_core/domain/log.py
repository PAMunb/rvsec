...
class RvErrorLog(BaseValidatedModel):
    ...
    def unique_msg(self) -> str:
        ...
        *unique misuses*. The seventh part is `identity_message`: `message` without the
        trailing evidence keys `vfp` and `vcls`, which stay in `message` itself. Every extra component carries real information: `error_type`
        separates a sequence violation from a constraint violation in the same method,
        `code` and `event` name the transition of the automaton that failed, and
        `message` is generated by the specification and names the offending parameter —
        `expecting one of {SHA-256, SHA-384, SHA-512} but found MD5` and `… but found
        SHA-1` are two genuinely different misuses of one method, not one misuse counted
        twice.

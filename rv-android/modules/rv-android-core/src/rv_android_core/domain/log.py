...
class RvErrorLog(BaseValidatedModel):
    ...
    def unique_msg(self) -> str:
        ...
        The key is `class:::method:::spec:::error_type:::message`, which is deliberately
        finer than the `(apk, class, method, spec)` key the thesis and the journal
        article use to count *unique misuses*. The two extra components carry real
        information: `error_type` separates a sequence violation from a constraint
        violation in the same method, and `message` is generated by the specification
        and names the offending parameter — `expecting one of {SHA-256, SHA-384,
        SHA-512} but found MD5` and `… but found SHA-1` are two genuinely different
        misuses of one method, not one misuse counted twice.

        The practical consequence, and the reason it is written here: `unique_errors`
        — and the `mop_errors_unique` column of `summary.csv` derived from it — is
        **not** numerically comparable to a unique-misuse count. Both figures are
        correct at their own granularity, and a reader comparing them without knowing
        this will conclude one of them is broken. The coarsening to the analysis key
        happens downstream, at aggregation time.

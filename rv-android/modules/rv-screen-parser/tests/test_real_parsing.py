...
class TestRealParsing:
    ...
    def test_available_test_data(self):
        ...
        assert (
            len(self.available_images) > 0
        ), "Should have PNG images in test_images directory"
        assert (
            len(self.available_dumps) > 0
        ), "Should have UIAutomator dumps in test_images directory"
        assert (
            len(self.image_dump_pairs) > 0
        ), "Should have at least one image/dump pair"

        print(
            f"Found {len(self.available_images)} images and {len(self.available_dumps)} dumps"
        )
    ...
    def test_parse_real_uiautomator_dumps(self):
        ...
        for img_path, dump_path in self.image_dump_pairs[:3]:  # Test first 3 pairs
            ...
            assert (
                len(root_node.items) > 0
            ), f"Should have at least one UI element in {dump_path.name}"
    ...
    def test_screenshot_analysis_with_real_images(self):
        ...
        for img_path, dump_path in self.image_dump_pairs[:2]:  # Test first 2 pairs
            ...
            ), f"Should analyze {img_path.name} successfully"
            assert hasattr(
                analysis_result, "texts"
            ), "Analysis result should have texts attribute"
            assert hasattr(
                analysis_result, "buttons"
            ), "Analysis result should have buttons attribute"
            assert hasattr(
                analysis_result, "error_indicators"
            ), "Analysis result should have error_indicators attribute"
    ...
    def test_end_to_end_parsing_workflow(self):
        ...
        for img_path, dump_path in self.image_dump_pairs[:2]:  # Test first 2 pairs
            ...
            print(
                f"  Screenshot analysis: {len(analysis_result.texts)} texts, {len(analysis_result.buttons)} buttons"
            )

            # Both should have found some content
            assert (
                len(root_node.items) > 0 or len(analysis_result.texts) > 0
    ...
    def test_multiple_real_dumps_consistency(self):
        ...
        for img_path, dump_path in self.image_dump_pairs[:5]:  # Test first 5 pairs
            ...
            try:
                ...
                results.append(
                    {"dump": dump_path.name, "error": str(e), "success": False}
                )
        ...
        if successful_parses:
            avg_elements = sum(r["elements_count"] for r in successful_parses) / len(
                successful_parses
            )
    ...
    def test_parser_with_various_dump_formats(self):
        ...
        for dump_path in self.available_dumps[:3]:  # Test first 3 dumps
            ...
            ), f"Should parse {dump_path.name} regardless of format variations"

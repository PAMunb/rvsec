...
class TestDefaultTextVisitor:
    ...
    def test_ensure_standard_system_actions(self):
        ...
        system_items = [item for item in self.visitor.items 
                       if hasattr(item, 'base_description') and 'System' in str(item.base_description)]
        assert len(system_items) >= 0  # May or may not add items depending on implementation

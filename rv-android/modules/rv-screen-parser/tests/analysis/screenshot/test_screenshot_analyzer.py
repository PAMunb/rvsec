...
from rv_screen_parser.screenshot.detectors import get_error_detector, get_text_detector
from rv_screen_parser.screenshot.models import ErrorType, InteractiveElementType
...
class TestCryptoAppScreenshots:
    ...
    def test_initial_screen_comprehensive_analysis(self, analyzer, image_paths):
        ...
        assert (
            len(result.texts) >= 4
        ), f"Expected ≥4 texts (title + 3 buttons), got {len(result.texts)}"

        # Button detection validation
        assert (
            len(result.buttons) >= 3
        ), f"Expected ≥3 buttons (main menu buttons), got {len(result.buttons)}"

        # Error detection validation (should be none on clean interface)
        assert (
            len(result.error_indicators) == 0
        ), f"Expected 0 errors on clean interface, got {len(result.error_indicators)}"
        ...
        for expected_part in essential_text_parts:
            found = any(
                expected_part in detected_text for detected_text in detected_texts
            )
            assert (
                found
            ), f"Expected to find '{expected_part}' in detected texts: {detected_texts}"

        # Verify button confidence scores are reasonable
        for button in result.buttons:
            assert (
                button.confidence >= 0.3
            ), f"Button confidence too low: {button.confidence}"
    ...
    def test_form_screen_comprehensive_analysis(self, analyzer, image_paths):
        ...
        assert (
            len(result.texts) >= 4
        ), f"Expected ≥4 texts on form screen, got {len(result.texts)}"

        # Button detection validation (should find GENERATE HASH button)
        assert (
            len(result.buttons) >= 1
        ), f"Expected ≥1 button (GENERATE HASH), got {len(result.buttons)}"

        # Error detection validation (clean form should have no errors)
        assert (
            len(result.error_indicators) == 0
        ), f"Expected 0 errors on clean form, got {len(result.error_indicators)}"
        ...
        for expected_part in form_text_parts:
            ...
            assert (
                found
            ), f"Expected to find '{expected_part}' in form texts: {detected_texts}"
    ...
    def test_error_screen_critical_validation(self, analyzer, image_paths):
        ...
        assert (
            len(result.texts) >= 4
        ), f"Expected ≥4 texts on error screen, got {len(result.texts)}"
        assert (
            len(result.buttons) >= 1
        ), f"Expected ≥1 button on error screen, got {len(result.buttons)}"

        # CRITICAL: Error detection validation
        assert (
            len(result.error_indicators) >= 2
        ), f"CRITICAL: Expected ≥2 error indicators (red icons), got {len(result.error_indicators)}"
    ...
    def test_component_specific_validation(self, analyzer, image_paths):
        ...
        if len(error_result.error_indicators) >= 2:
            ...
            red_errors = [
                e
                for e in error_result.error_indicators
                if e.error_type
                in [ErrorType.VISUAL_INDICATOR, ErrorType.VALIDATION_ERROR]
            ]
            ...
            for error in red_errors:
                ...
                assert (
                    150 <= error.bbox.y <= 400
                ), f"Error positioned outside form area: y={error.bbox.y}"
        ...
        assert (
            len(initial_result.buttons) >= 3
        ), f"Initial screen should detect main menu buttons, got {len(initial_result.buttons)}"
        assert (
            len(form_result.buttons) >= 1
        ), f"Form screen should detect form button, got {len(form_result.buttons)}"
    ...
    def test_regression_baseline_validation(self, analyzer, image_paths):
        ...
        for screen_name, image_path in image_paths.items():
            ...
            for key_text_part in baseline["key_text_parts"]:
                found = any(
                    key_text_part in detected_text for detected_text in detected_texts
                )
                assert (
                    found
                ), f"{screen_name}: Expected key text part '{key_text_part}' not found in {detected_texts}"
    ...
    def test_backward_compatibility_validation(self, analyzer, image_paths):
        ...
        for screen_name, image_path in image_paths.items():
            ...
            analyzer.current_result = (
                pydantic_result  # Ensure current_result is updated
            )
            ...
            assert isinstance(
                dict_result, dict
            ), "extract_information should return dict"
            required_keys = [
                "texts",
                "buttons",
                "error_indicators",
            ...
            for key in required_keys:
                assert (
                    key in dict_result
                ), f"Missing key '{key}' in backward compatibility result"
                assert isinstance(
                    dict_result[key], list
                ), f"Key '{key}' should be a list"
            ...
            button_diff = abs(
                len(dict_result["buttons"]) - len(pydantic_result.buttons)
            )
            assert (
                button_diff <= 2
            ...
            assert len(dict_result["error_indicators"]) == len(
                pydantic_result.error_indicators
    ...
    def test_performance_validation(self, analyzer, image_paths):
        ...
        for screen_name, image_path in image_paths.items():
            ...
            print(
                f"Performance - {screen_name}: {result.processing_time:.3f}s, "
    ...
    def test_detection_summary_integration(self, analyzer, image_paths):
        ...
        for screen_name, image_path in image_paths.items():
            ...
            assert (
                summary["total_elements"] == result.total_elements
            ), "Summary total should match result total"

            # Verify component summaries are included when elements are detected
            if result.texts:
                assert (
                    "text_summary" in summary
                ), "Should include text summary when texts detected"
            if result.buttons:
                assert (
                    "button_summary" in summary
                ), "Should include button summary when buttons detected"
            if result.error_indicators:
                assert (
                    "error_summary" in summary
                ), "Should include error summary when errors detected"
    ...
    def test_board_game_detection_comprehensive(self, analyzer, image_paths):
        ...
        assert (
            len(result.texts) >= 2
        ), f"Expected ≥2 texts (game instructions), got {len(result.texts)}"

        # Interactive elements detection (board game pieces and positions)
        board_elements = [
            e
            for e in result.interactive_elements
            if e.type
            in [
        ...
        assert (
            len(board_elements) >= 1
        ), f"Expected ≥1 board game elements, got {len(board_elements)}"

        # Verify that detected board game elements have reasonable confidence
        for element in board_elements:
            assert (
                element.confidence >= 0.4
            ), f"Board game element confidence too low: {element.confidence}"

            # Verify element has proper geometric properties
            assert (
                element.bbox.width > 10
            ), f"Element width too small: {element.bbox.width}"
            assert (
                element.bbox.height > 10
            ), f"Element height too small: {element.bbox.height}"
        ...
        found_count = sum(
            1
            for part in game_text_parts
            if any(part in text for text in detected_texts)
        )
        assert (
            found_count >= 1
        ), f"Expected ≥1 game text parts, found {found_count} in {detected_texts}"

        # Should detect minimal or no errors on game screen
        assert (
            len(result.error_indicators) <= 2
        ), f"Game screen should have minimal errors, got {len(result.error_indicators)}"
    ...
    def test_hardware_topology_visualization_comprehensive(self, analyzer, image_paths):
        ...
        assert (
            result.success == True
        ), "Analysis should succeed on topology visualization screen"
        assert (
            result.total_elements > 0
        ), "Should detect elements in the topology visualization"

        # Text detection validation (technical specifications)
        assert (
            len(result.texts) >= 8
        ), f"Expected ≥8 texts (technical labels), got {len(result.texts)}"

        # Button detection validation (Options button)
        assert (
            len(result.buttons) >= 1
        ), f"Expected ≥1 button (Options), got {len(result.buttons)}"
        ...
        found_count = sum(
            1
            for part in topology_text_parts
            if any(part in text for text in detected_texts)
        )
        assert (
            found_count >= 3
        ), f"Expected ≥3 topology text parts, found {found_count} in {detected_texts}"

        # Should detect minimal errors on technical visualization
        assert (
            len(result.error_indicators) <= 1
        ), f"Topology screen should have minimal errors, got {len(result.error_indicators)}"

        # Interactive elements should be detected for custom components
        assert (
            len(result.interactive_elements) >= 5
        ), f"Expected ≥5 interactive elements, got {len(result.interactive_elements)}"
    ...
    def test_hex_strategy_game_comprehensive(self, analyzer, image_paths):
        ...
        assert (
            len(result.texts) >= 1
        ), f"Expected ≥1 text (Hex title), got {len(result.texts)}"

        # Button detection validation (game controls)
        assert (
            len(result.buttons) >= 2
        ), f"Expected ≥2 buttons (game controls), got {len(result.buttons)}"
        ...
        ), f"Should detect some text elements, got {detected_texts}"
        ...
        assert (
            len(board_elements) >= 10
        ), f"Expected ≥10 board game elements, got {len(board_elements)}"

        # High contrast game should have minimal false positive errors due to adaptive threshold
        assert (
            len(result.error_indicators) <= 3
        ), f"High contrast game should have minimal errors, got {len(result.error_indicators)}"
    ...
    def test_settings_screen_with_color_dialog_comprehensive(
        self, analyzer, image_paths
    ):
        """
        ...
        assert (
            result.total_elements > 0
        ), "Should detect elements in the settings screen"

        # Text detection validation
        assert (
            len(result.texts) >= 6
        ), f"Expected ≥6 texts (settings options), got {len(result.texts)}"

        # Button detection validation
        assert (
            len(result.buttons) >= 2
        ), f"Expected ≥2 buttons (WHITE, DEFAULT), got {len(result.buttons)}"
        ...
        found_count = sum(
            1
            for part in settings_text_parts
            if any(part in text for text in detected_texts)
        )
        assert (
            found_count >= 2
        ), f"Expected ≥2 settings text parts, found {found_count} in {detected_texts}"

        # Should detect interactive elements (switches, color selectors)
        switch_elements = [
            e
            for e in result.interactive_elements
            if e.type == InteractiveElementType.SWITCH
        ]
        assert (
            len(switch_elements) >= 1
        ), f"Expected ≥1 switch element, got {len(switch_elements)}"

        # Settings screen should have no error indicators
        assert (
            len(result.error_indicators) == 0
        ), f"Settings screen should have no errors, got {len(result.error_indicators)}"
    ...
    def test_color_picker_interface_comprehensive(self, analyzer, image_paths):
        ...
        assert (
            len(result.texts) >= 1
        ), f"Expected ≥1 text (Regular text), got {len(result.texts)}"
        ...
        assert (
            settings_related
        ), f"Expected settings/color related text in {detected_texts}"
        ...
        clickable_elements = [
            e
            for e in result.interactive_elements
            if e.type == InteractiveElementType.CLICKABLE
        ]
        total_interactive = len(result.interactive_elements) + len(result.buttons)
        assert (
            total_interactive >= 10
        ), f"Expected ≥10 interactive color elements, got {total_interactive}"
    ...
    def test_hourly_reminder_settings_comprehensive(self, analyzer, image_paths):
        ...
        assert (
            result.success == True
        ), "Analysis should succeed on hourly reminder screen"
        assert (
            result.total_elements > 0
        ), "Should detect elements in the hourly reminder screen"
        ...
        assert (
            len(result.buttons) >= 2
        ), f"Expected ≥2 buttons (UI elements), got {len(result.buttons)}"
        ...
        assert (
            total_interactive >= 5
        ), f"Expected ≥5 total interactive elements, got {total_interactive}"
    ...
    def test_sudoku_puzzle_game_comprehensive(self, analyzer, image_paths):
        ...
        assert (
            len(result.buttons) >= 2
        ), f"Expected ≥2 buttons (Rules, Solve), got {len(result.buttons)}"
        ...
        assert (
            len(board_elements) >= 15
        ), f"Expected ≥15 board game elements, got {len(board_elements)}"

        # Clean interface should have no error indicators
        assert (
            len(result.error_indicators) == 0
        ), f"Clean Sudoku interface should have no errors, got {len(result.error_indicators)}"
...
class TestErrorDetectorRealWorld:
    ...
    def error_image_path(self):
        ...
        """
        return str(
            Path(__file__).parent.parent.parent / "images" / "cryptoapp_009_errors.png"
        )
    ...
    def test_red_color_detection(self, error_image_path):
        ...
        assert (
            len(errors) >= 1
        ), f"Should detect red error indicators, got {len(errors)}"

        # At least one error should be color-based
        color_errors = [
            e
            for e in errors
            if "color" in str(e.error_type).lower()
            or e.error_type == ErrorType.VISUAL_INDICATOR
        ]
    ...
    def test_validation_error_classification(self, error_image_path):
        ...
        validation_errors = [
            e
            for e in result.error_indicators
        ...
        for error in validation_errors:
            assert (
                error.confidence >= 0.4
            ), f"Validation error should have high confidence: {error.confidence}"

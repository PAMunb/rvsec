...
class ItemAction(BaseValidatedModel):
    ...
    id: int = Field(
        ..., description="Unique identifier for the action within the screen context"
    )
    text: str = Field(
        ...,
        description="Human-readable action description with format-specific conventions",
    )
    ...
    target_view: Dict[str, Any] = Field(
        default_factory=dict, description="Target UI element properties and metadata"
    )
    coordinates: Optional[Tuple[int, int]] = Field(
        default=None, description="Explicit action coordinates (x, y)"
    )
    widget_id: Optional[str] = Field(default=None, description="Target widget ID")
    callback_signature: Optional[str] = Field(
        default=None, description="Target callback method signature"
    )
    text_input: Optional[str] = Field(
        default=None, description="Text value for TEXT_CHANGE actions"
    )
...
class ScreenItem(BaseValidatedModel):
    ...
    view: Dict[str, Any] = Field(
        ..., description="Raw UI element data from screen analysis"
    )
    base_description: str = Field(..., description="Human-readable element description")
    actions: List[ItemAction] = Field(
        default_factory=list, description="Available actions for this element"
    )
    complement: Dict[str, Any] = Field(
        default_factory=dict, description="Additional element metadata and annotations"
    )
...
class ScreenDescription(BaseValidatedModel):
    ...
    items: List[ScreenItem] = Field(
        ..., description="All UI elements with their available actions"
    )
    events_by_id: Dict[int, ItemAction] = Field(
        default_factory=dict, description="Mapping of action IDs to action objects"
    )
...
class Node(BaseValidatedModel):
    ...
    children: List["Node"] = Field(
        default_factory=list, description="Child nodes in UI hierarchy"
    )
    ...
    clickable: bool = Field(
        default=False, description="Element supports click interactions"
    )
    scrollable: bool = Field(
        default=False, description="Element supports scroll interactions"
    )
    checkable: bool = Field(
        default=False, description="Element supports check/uncheck interactions"
    )
    long_clickable: bool = Field(
        default=False, description="Element supports long click interactions"
    )
    ...
    content_description: str = Field(
        default="", description="Accessibility content description"
    )
    ...
    # Spatial properties
    bounds: List[List[int]] = Field(
        default_factory=lambda: [[0, 0], [0, 0]],
        description="Element bounding box coordinates",
    )
    ...
    actionable: bool = Field(
        default=False, description="Element supports any form of interaction"
    )
    element_type: UiElementType = Field(
        default=UiElementType.UNKNOWN, description="Generic element type classification"
    )
    ...
    def __init__(self, **data_dict):
        ...
        logging_manager = LoggingManager.get_instance()
        self._logger = logging_manager.get_logger(
            "rv_screen_parser.models.Node", {CONTEXT_COMPONENT: "Node"}
        )

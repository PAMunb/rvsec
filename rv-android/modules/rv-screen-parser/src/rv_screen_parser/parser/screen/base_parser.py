...
class BaseScreenParser(Generic[T], ABC):
    ...
    def __init__(
        ...
        if self.visitor_class is None:
            from rv_screen_parser.parser.screen.visitor.default_visitor import (
                DefaultTextVisitor,
            )
    ...
    def parse_screen(
        ...
        with self.error_handler.error_context(
            ...
            phase="screen_parsing",
            parser_type=self.parser_name,
        ):
            # Common validation logic
            if not self.validate_state_data(state_data):
                self.logger.error(
                    f"Invalid {self.parser_name} state data: missing required fields"
                )
            ...
            self.logger.info(
                f"Parsing {self.parser_name} state for activity: {activity}"
            )

            # Delegate to implementation-specific parsing
            screen_description = self._parse_implementation(
                state_data, static_data, activity
            )
    ...
    def create_visitor(
        self, static_data: Optional[StaticAnalysisData], activity: str
    ) -> AbstractScreenVisitor:
        """
        ...
        """
        self.logger.debug(
            f"Creating visitor: class={self.visitor_class.__name__}, activity={activity}"
        )
    ...
    def log_processing_summary(self, data_type: str, item_count: int) -> None:
        ...
        """
        self.logger.info(
            f"Processed {item_count} {data_type} from {self.parser_name} data"
        )

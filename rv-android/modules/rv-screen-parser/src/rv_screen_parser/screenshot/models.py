    ...
    def validate_positive_dimensions(cls, v):
        ...
        if v <= 0:
            raise RVValidationError(
                f"Image dimension must be positive, got {v}",
...
class DetectedText(BaseValidatedModel):
    ...
    is_button_like: bool = Field(default=False, description="Text appears to be button label")
    is_error_like: bool = Field(default=False, description="Text appears to be error message")
...
class DetectedButton(BaseValidatedModel):
    ...
    def validate_aspect_ratio(cls, v):
        ...
        if v <= 0:
            raise RVValidationError(
                f"Aspect ratio must be positive, got {v}",
...
class ErrorIndicator(BaseValidatedModel):
    ...
    red_dominance: Optional[float] = Field(None, ge=0, description="Red color dominance ratio")
    circularity: Optional[float] = Field(None, ge=0, le=2.0, description="Shape circularity measure")
    icon_type: Optional[IconType] = Field(None, description="Icon type for icon-based errors")
    error_category: Optional[str] = Field(None, description="Error category classification")
    
    # Dialog-specific fields
    centering: Optional[float] = Field(None, ge=0.0, le=1.0, description="Dialog centering measure")
    contains_texts: Optional[int] = Field(None, ge=0, description="Number of contained texts")
    contains_error_texts: Optional[int] = Field(None, ge=0, description="Number of error texts")
    contains_buttons: Optional[int] = Field(None, ge=0, description="Number of contained buttons")
    parent_dialog: Optional[int] = Field(None, ge=0, description="Reference to parent dialog")
    
    # Validation-specific fields
    field_x: Optional[int] = Field(None, ge=0, description="Associated form field X coordinate")
    field_y: Optional[int] = Field(None, ge=0, description="Associated form field Y coordinate") 
    field_width: Optional[int] = Field(None, gt=0, description="Associated form field width")
    field_height: Optional[int] = Field(None, gt=0, description="Associated form field height")
    
    # Toast-specific fields
    contains_error_text: Optional[bool] = Field(None, description="Contains error text flag")
...
class InteractiveElement(BaseValidatedModel):
    ...
    circularity: Optional[float] = Field(None, ge=0, le=2.0, description="Shape circularity for joysticks")
    aspect_ratio: Optional[float] = Field(None, gt=0, description="Width to height ratio")
    button_count: Optional[int] = Field(None, ge=0, description="Number of buttons for D-pad")
    parent_dpad: Optional[int] = Field(None, ge=0, description="Reference to parent D-pad")
...
class ScreenshotAnalysisResult(BaseValidatedModel):
    ...
    image_path: str = Field(..., min_length=1, description="Path to analyzed screenshot")
    dimensions: ImageDimensions = Field(default_factory=lambda: ImageDimensions(width=0, height=0),
                                       description="Screenshot dimensions")
    texts: List[DetectedText] = Field(default_factory=list, description="Detected text elements")
    buttons: List[DetectedButton] = Field(default_factory=list, description="Detected button elements")
    error_indicators: List[ErrorIndicator] = Field(default_factory=list, 
                                                  description="Detected error indicators")
    interactive_elements: List[InteractiveElement] = Field(default_factory=list,
                                                          description="Detected interactive elements")
    processing_time: float = Field(default=0.0, ge=0.0, description="Analysis processing time in seconds")
    ...
    def total_elements(self) -> int:
        ...
        return (len(self.texts) + len(self.buttons) + 
                len(self.error_indicators) + len(self.interactive_elements))
...
def _setup_screenshot_model_error_handlers():
    ...
    logger = logging_manager.get_logger(
        "screenshot.models",
        {CONTEXT_COMPONENT: "ScreenshotModels"}
    )
    
    @ErrorHandler.handle_errors(component="ScreenshotModels", phase="validation")
    def handle_screenshot_validation_error(error: RVValidationError, context: dict) -> bool:
    ...
    error_handler.register_handler(RVValidationError, handle_screenshot_validation_error)

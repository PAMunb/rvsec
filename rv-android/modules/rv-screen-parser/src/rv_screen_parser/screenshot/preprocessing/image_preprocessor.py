...
class ImagePreprocessor:
    ...
    def __init__(self):
        ...
        self.logger = logging_manager.get_logger(
            "screenshot.preprocessor", {CONTEXT_COMPONENT: "ImagePreprocessor"}
        )

    @ErrorHandler.handle_errors(
        component="ImagePreprocessor", phase="grayscale_conversion", reraise=True
    )
    def preprocess_grayscale(self, image: np.ndarray) -> np.ndarray:
        ...
        if image is None:
            raise RVParsingError(
                "Cannot process null image", parser_type="ImagePreprocessor"
            )
        ...
        try:
            ...
            self.logger.debug(
                f"Converted to grayscale: {image.shape} -> {blurred.shape}"
            )
            return blurred
    ...
    @ErrorHandler.handle_errors(
        component="ImagePreprocessor", phase="binary_conversion", reraise=True
    )
    ...
    def preprocess_for_edge_detection(
        ...
        try:
            ...
            self.logger.debug(
                f"Generated edge image with thresholds {low_threshold}-{high_threshold}"
            )
    ...
    def enhance_contrast(
        self, image: np.ndarray, alpha: float = 1.5, beta: int = 0
    ) -> np.ndarray:
        """

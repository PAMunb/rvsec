...
from ..models import BoundingBox, DetectedText
...
class TextDetector:
    ...
    def __init__(self):
        ...
        self.logger = logging_manager.get_logger(
            "screenshot.text_detector", {CONTEXT_COMPONENT: "TextDetector"}
        )
    ...
    @ErrorHandler.handle_errors(
        component="TextDetector", phase="text_extraction", reraise=True
    )
    def extract_text(self, gray_image: np.ndarray) -> List[DetectedText]:
        ...
        if gray_image is None:
            raise RVParsingError(
                "Cannot extract text from null image", parser_type="TextDetector"
            )
        ...
        for config_idx, config in enumerate(self.ocr_configs):
            try:
                self.logger.debug(
                    f"Running OCR configuration {config_idx + 1}/{len(self.ocr_configs)}"
                )

                text_data = pytesseract.image_to_data(
                    gray_image, output_type=pytesseract.Output.DICT, config=config
                )
                ...
                self.logger.warning(
                    f"OCR configuration {config_idx + 1} failed: {str(e)}"
                )
        ...
        self.logger.info(
            f"Extracted {len(filtered_texts)} text elements from {len(detected_texts)} detections"
        )
        return filtered_texts

    def _process_ocr_results(
        self, text_data: dict, seen_texts: Set[str]
    ) -> List[DetectedText]:
        """
        ...
        for i in range(len(text_data["text"])):
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to create DetectedText for '{text}': {validation_error}"
                )
    ...
    def is_button_text(self, text: str) -> bool:
        ...
        if any(
            label == text_lower or label in text_lower
            for label in self.button_text_indicators
        ):
            return True
    ...
    def get_text_classification_summary(self, texts: List[DetectedText]) -> dict:
        ...
        return {
            ...
            "average_confidence": (
                sum(t.confidence for t in texts) / total_texts
                if total_texts > 0
                else 0.0
            ),
        }

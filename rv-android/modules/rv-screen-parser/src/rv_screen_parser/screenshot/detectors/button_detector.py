...
from ..models import DetectedButton, DetectedText, DetectionMethod
...
class ButtonDetector:
    ...
    def __init__(self):
        ...
        self.logger = logging_manager.get_logger(
            "screenshot.button_detector", {CONTEXT_COMPONENT: "ButtonDetector"}
        )
    ...
    @ErrorHandler.handle_errors(
        component="ButtonDetector", phase="button_detection", reraise=True
    )
    def detect_buttons(
        self,
        ...
        if binary_image is None or original_image is None:
            raise RVParsingError(
                "Cannot detect buttons with null images", parser_type="ButtonDetector"
            )
        ...
        self.logger.info(
            f"Detected {len(buttons)} buttons ({len(shape_buttons)} shape-based, {len(text_buttons)} text-based)"
        )
        return buttons

    def _detect_shape_buttons(
        self,
        binary_image: np.ndarray,
        original_image: np.ndarray,
        texts: List[DetectedText],
    ...
    def _analyze_contour_for_button(
        self,
        contour,
        original_image: np.ndarray,
        texts: List[DetectedText],
        ...
        detection_method = (
            DetectionMethod.SHAPE_WITH_TEXT if button_text else DetectionMethod.SHAPE
        )
    ...
    def _calculate_contrast_score(
        ...
        try:
            ...
            if (
                button_mask_y >= 0
                and button_mask_x >= 0
                and button_mask_y + h <= bg_h
                and button_mask_x + w <= bg_w
            ):
                mask[
                    button_mask_y : button_mask_y + h, button_mask_x : button_mask_x + w
                ] = 0
    ...
    def _detect_text_buttons(
        self, texts: List[DetectedText], existing_buttons: List[DetectedButton]
    ) -> List[DetectedButton]:
        """
        ...
        for text_element in texts:
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to create text-based DetectedButton: {validation_error}"
                )
                # Continue processing other text elements

        return text_buttons

    def _find_text_inside_button(
    ...
    def _text_overlaps_with_buttons(
        self, text_element: DetectedText, buttons: List[DetectedButton]
    ) -> bool:
        """
    ...
    def get_button_detection_summary(self, buttons: List[DetectedButton]) -> dict:
        ...
        shape_buttons = sum(
            1
            for b in buttons
            if b.detection_method
            in [DetectionMethod.SHAPE, DetectionMethod.SHAPE_WITH_TEXT]
        )
        text_buttons = sum(
            1 for b in buttons if b.detection_method == DetectionMethod.TEXT
        )
        ...
        return {
            ...
            "average_confidence": (
                sum(b.confidence for b in buttons) / total_buttons
                if total_buttons > 0
                else 0.0
            ),
        }

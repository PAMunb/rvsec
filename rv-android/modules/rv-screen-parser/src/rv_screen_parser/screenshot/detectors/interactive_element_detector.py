...
class InteractiveElementDetector:
    ...
    def __init__(self):
        ...
        self.switch_indicators = ["on", "off", "enable", "disable", "toggle", "switch"]

    @ErrorHandler.handle_errors(
        component="InteractiveElementDetector", phase="element_detection", reraise=True
    )
    def detect_interactive_elements(
        ...
        navigation = self._detect_navigation_elements(
            binary_image, original_image, texts
        )
        elements.extend(navigation)

        # Strategy 5: Generic clickable element detection
        clickables = self._detect_clickable_elements(
            binary_image, original_image, texts, elements
        )
        elements.extend(clickables)

        # Strategy 6: Board game element detection
        board_game_elements = self._detect_board_game_elements(
            binary_image, original_image, texts
        )
        ...
        self.logger.info(
            f"Detected {len(elements)} interactive elements ({len(sliders)} sliders, {len(switches)} switches, {len(input_fields)} input fields, {len(navigation)} navigation, {len(clickables)} clickables, {len(board_game_elements)} board game elements)"
        )
        return elements
        ...
        for contour in contours:
            try:
                ...
                if (
                    ...
                    and w > width * 0.2
                    and h < height * 0.02
                    and width * 0.1 <= w <= width * 0.9
                ):

                    # Check for slider thumb/handle nearby
                    thumb_confidence = self._detect_slider_thumb(
                        original_image, x, y, w, h
                    )

                    # Calculate confidence based on aspect ratio and context
                    confidence = (
                        0.5 + min(aspect_ratio / 20, 0.3) + thumb_confidence * 0.2
                    )
                    ...
                    if nearby_text and any(
                        word in nearby_text.lower()
                        for word in ["volume", "brightness", "speed", "level", "amount"]
                    ):
    ...
    def _detect_switches(
        ...
        for contour in contours:
            try:
                ...
                if (
                    1.5 <= aspect_ratio <= 3.0
                    and width * 0.02 <= w <= width * 0.15
                    and height * 0.01 <= h <= height * 0.08
                ):
                    ...
                    roundness_score = (
                        1.0 - (len(approx) / 20) if len(approx) > 4 else 0.8
                    )
                    ...
                    nearby_text = self._find_nearby_text(
                        x, y, w, h, texts, distance=100
                    )
                    if nearby_text:
                        if any(
                            indicator in nearby_text.lower()
                            for indicator in self.switch_indicators
                        ):
                            confidence = min(confidence + 0.3, 1.0)
                        elif any(
                            word in nearby_text.lower()
                            for word in ["notifications", "wifi", "bluetooth", "auto"]
                        ):
                            confidence = min(confidence + 0.2, 1.0)
                    ...
                    if confidence >= 0.5:
                        switch = self._create_interactive_element(
                            ...
                            h,
                            confidence,
                            InteractiveElementType.SWITCH,
                            (
                                f"Switch: {nearby_text}"
                                if nearby_text
                                else "Toggle switch"
                            ),
                        )
    ...
    def _detect_input_fields(
        ...
        for contour in contours:
            try:
                ...
                if (
                    3 <= aspect_ratio <= 10
                    and width * 0.15 <= w <= width * 0.9
                    and height * 0.03 <= h <= height * 0.12
                ):
                    ...
                    rectangularity = (
                        1.0
                        if len(approx) == 4
                        else max(0, 1.0 - abs(len(approx) - 4) * 0.1)
                    )
                    ...
                    if nearby_text or inside_text:
                        ...
                        if any(
                            ...
                            for indicator in self.input_field_indicators
                        ):
                            confidence = min(confidence + 0.4, 1.0)
                        elif any(char in text_content for char in ["@", ".", "_", "-"]):
                            confidence = min(confidence + 0.3, 1.0)
                        elif (
                            "hint" in text_content.lower()
                            or "placeholder" in text_content.lower()
                        ):
                            confidence = min(confidence + 0.2, 1.0)

                    if confidence >= 0.5:
                        field_type = self._classify_input_field(
        ...
        for text_element in texts:
            ...
            if confidence >= 0.4:
                ...
                nav_element = self._create_interactive_element(
                    ...
                    expanded_h,
                    confidence,
                    InteractiveElementType.NAVIGATION,
    ...
    def _detect_clickable_elements(
        self,
        binary_image: np.ndarray,
        original_image: np.ndarray,
        texts: List[DetectedText],
        existing_elements: List[InteractiveElement],
    ) -> List[InteractiveElement]:
        """
        ...
        for contour in contours:
            try:
                ...
                if (
                    width * 0.05 <= w <= width * 0.4
                    and height * 0.03 <= h <= height * 0.2
                    and area > (width * height) * 0.002
                ):
                    ...
                    position_score = self._calculate_position_clickability(
                    ...
                    if confidence >= 0.4:
                        clickable = self._create_interactive_element(
                            ...
                            h,
                            confidence,
                            InteractiveElementType.CLICKABLE,
                            (
                                f"Clickable: {associated_text}"
                                if associated_text
                ...
                self.logger.warning(
                    f"Error analyzing contour for clickable element: {e}"
                )

        return clickables

    def _detect_slider_thumb(
        self, image: np.ndarray, track_x: int, track_y: int, track_w: int, track_h: int
    ) -> float:
    ...
    def _calculate_position_clickability(
        self, x: int, y: int, w: int, h: int, img_width: int, img_height: int
    ) -> float:
    ...
    def _find_nearby_text(
        ...
        for text_element in texts:
            ...
            dist = math.sqrt(
                (center_x - text_center_x) ** 2 + (center_y - text_center_y) ** 2
            )
            if dist <= distance:
    ...
    def _find_text_inside_element(
        self, x: int, y: int, w: int, h: int, texts: List[DetectedText]
    ) -> Optional[str]:
    ...
    def _create_interactive_element(
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to create InteractiveElement: {validation_error}"
            )
            return None

    def get_interactive_element_summary(
        ...
        return {
            ...
            "average_confidence": (
                sum(e.confidence for e in elements) / total_elements
                if total_elements > 0
                else 0.0
            ),
        }

    def _detect_board_game_elements(
        self,
        binary_image: np.ndarray,
        original_image: np.ndarray,
        texts: List[DetectedText],
    ...
    def _identify_grid_patterns(
        self, contours, width: int, height: int
    ) -> List[Tuple[int, int]]:
        """
    ...
    def _classify_board_game_element(
        self,
        area: float,
        aspect_ratio: float,
        circularity: float,
    ...
    ) -> Tuple[InteractiveElementType, float]:
        """
        ...
        if saturation > 100:  # Colored elements are likely game pieces
            ...
            saturation < 50 and value > 200
        ):  # Light/white elements might be board positions
            confidence = min(confidence + 0.15, 1.0)

        # Factor 3: Size consistency (board game elements are often uniform)
        size_score = self._calculate_size_consistency_score(
            area, grid_positions, img_width, img_height
        )
        confidence = min(confidence + size_score * 0.2, 1.0)

        # Factor 4: Spatial positioning (elements in grid patterns)
        position_score = self._calculate_position_consistency_score(
            x, y, w, h, grid_positions
        )
    ...
    def _calculate_size_consistency_score(
        self,
        area: float,
        grid_positions: List[Tuple[int, int]],
        img_width: int,
    ...
    def _calculate_position_consistency_score(
        self, x: int, y: int, w: int, h: int, grid_positions: List[Tuple[int, int]]
    ) -> float:
        """
        ...
        for gx, gy in grid_positions:
            distance = math.sqrt(
                (element_center_x - gx) ** 2 + (element_center_y - gy) ** 2
            )

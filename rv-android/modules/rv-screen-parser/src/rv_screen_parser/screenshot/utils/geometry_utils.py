...
class GeometryUtils:
    ...
    def __init__(self):
        ...
        self.logger = logging_manager.get_logger(
            "screenshot.geometry",
            {CONTEXT_COMPONENT: "GeometryUtils"}
        )
    
    @ErrorHandler.handle_errors(component="GeometryUtils", phase="calculation")
    def calculate_overlap_percentage(self, x1: int, y1: int, w1: int, h1: int, 
    ...
    def filter_overlapping_elements(self, elements: List[Dict[str, Any]], 
                                  overlap_threshold: float = 0.6) -> List[Dict[str, Any]]:
        """
        ...
        for element in sorted_elements:
            ...
            for included_element in included:
                ...
                try:
                    overlap = self.calculate_overlap_percentage(x1, y1, w1, h1, x2, y2, w2, h2)
        ...
        self.logger.debug(f"Filtered {len(elements)} elements to {len(included)} non-overlapping elements")
        return included
    
    def calculate_center_point(self, x: int, y: int, width: int, height: int) -> Tuple[int, int]:
        """
    ...
    def is_point_inside_rectangle(self, point_x: int, point_y: int,
    ...
    def calculate_distance_between_centers(self, x1: int, y1: int, w1: int, h1: int,
                                         x2: int, y2: int, w2: int, h2: int) -> float:
        """
    ...
    def expand_rectangle(self, x: int, y: int, width: int, height: int,
                        padding_x: int, padding_y: int,
                        max_width: int = None, max_height: int = None) -> Tuple[int, int, int, int]:
        """

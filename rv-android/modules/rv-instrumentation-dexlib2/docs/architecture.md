...
| Wrappers | `wrappersGenerated`, `wrappersSubstituted`, `wrappersAliasedToSubtype`, `constructorInlineApplied`, `constructorInlineSkippedAliasing` | Wrapper emission and call-site substitution. `wrappersAliasedToSubtype` counts subtype owners routed to a supertype's wrapper: APK-defined subclasses and framework subtypes such as `PublicKey` under a `Key+` wrapper. |

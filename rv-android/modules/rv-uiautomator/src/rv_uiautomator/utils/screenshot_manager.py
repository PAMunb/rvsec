...
from ..constants import SCREENSHOT_QUALITY, SCREENSHOT_DIR, SCREENSHOT_FORMAT
...
class ScreenshotManager:
    ...
    def __init__(self, screenshot_dir: str = SCREENSHOT_DIR):
        ...
        self.logger.debug(f"ScreenshotManager initialized with directory: {screenshot_dir}")
    
    @ErrorHandler.handle_errors(
        component="ScreenshotManager",
        phase="path_generation"
    )
    def generate_screenshot_path(self, prefix: str = "screenshot", extension: str = "png") -> str:
        """
    ...
    @ErrorHandler.handle_errors(
        component="ScreenshotManager",
        phase="screenshot_optimization"
    )
    def optimize_screenshot(self, screenshot_path: str, quality: int = SCREENSHOT_QUALITY) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(f"Screenshot optimization failed for {screenshot_path}: {e}")
            return False
    ...
    def validate_screenshot(self, screenshot_path: str) -> bool:
        ...
        try:
            ...
            if file_size < 1024:  # Less than 1KB indicates likely corruption
                self.logger.warning(f"Screenshot file too small ({file_size} bytes): {screenshot_path}")
                return False
            ...
            with Image.open(screenshot_path) as img:
                ...
                if width < 100 or height < 100:
                    self.logger.warning(f"Screenshot dimensions too small ({width}x{height}): {screenshot_path}")
                    return False
            ...
            self.logger.error(f"Screenshot validation failed for {screenshot_path}: {e}")
            return False
    
    @ErrorHandler.handle_errors(
        component="ScreenshotManager",
        phase="screenshot_cleanup"
    )
            ...
            for screenshot_file in self.screenshot_dir.glob("screenshot_*"):
                try:
                    ...
                    if file_age > max_age_seconds:
                        ...
                        cleanup_count += 1
                        self.logger.debug(f"Cleaned up old screenshot: {screenshot_file}")

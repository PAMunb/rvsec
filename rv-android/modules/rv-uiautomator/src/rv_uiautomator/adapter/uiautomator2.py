...
class UIAutomator2Adapter(UIAdapter):
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="connection")
    def connect(self, device_id: str) -> bool:
        ...
        try:
            ...
            if info:
                ...
                self.logger.info(
                    f"Connected to device {device_id}: {info.get('productName', 'Unknown')}"
                )
                return True
            else:
                self.connected = False
    ...
    @ErrorHandler.handle_errors(
        component="UIAutomator2Adapter", phase="state_capture", default_return={}
    )
    def get_ui_state(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if not self.connected or not self.device:
            self.logger.warning(
                "⚠️ DEBUG_STATE: Device not connected, cannot capture UI state"
            )
            self.logger.warning(
                f"   Connected: {self.connected}, Device: {self.device is not None}"
            )
            return {}
        ...
        try:
            ...
            self.logger.warning("🔍 DEBUG_STATE: UI state captured")
            self.logger.warning(f"   Package: {app_current.get('package', 'unknown')}")
            self.logger.warning(
                f"   Activity: {app_current.get('activity', 'unknown')}"
            )
            self.logger.warning(
                f"   XML length: {len(xml_hierarchy) if xml_hierarchy else 0}"
            )
            ...
            self.logger.debug(
                f"Captured UI state for {app_current.get('package', 'unknown')}"
            )
            return ui_state
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="click_action")
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="text_input")
    ...
    @ErrorHandler.handle_errors(
        component="UIAutomator2Adapter", phase="long_click_action"
    )
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="swipe_action")
    def swipe(
        self,
        x1: int,
        y1: int,
        ...
        try:
            ...
            time.sleep(ACTION_EXECUTION_DELAY)
            self.logger.debug(
                f"Swiped from ({x1}, {y1}) to ({x2}, {y2}) in {duration}s"
            )
            return True
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="back_press")
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="home_press")
    ...
    @ErrorHandler.handle_errors(
        component="UIAutomator2Adapter", phase="screenshot_capture"
    )
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="app_launch")
    def launch_app(self, package_name: str) -> bool:
        ...
        try:
            ...
            if current_app.get("package") == package_name:
                ...
                self.logger.warning(
                    f"App launch verification failed for {package_name}"
                )
                return False
    ...
    @ErrorHandler.handle_errors(component="UIAutomator2Adapter", phase="app_stop")

#!/usr/bin/env python3
"""Censo do substrato MOP do corpus da gh104, calculado como o `aperv-tool` calcula.

CÓPIA de `../../experimento-comp162/scripts/censo_substrato.py`. Mudou só de onde vem o
diretório: em vez de um literal, ele é resolvido por `corpus.py` (ou passado em `--corpus`),
porque o corpus desta campanha só existe depois do estágio 1.

**O censo desta campanha tem de ser idêntico ao da `comp162`, e isso é uma verificação, não
uma coincidência.** Os `.apk.json` são copiados tal e qual do corpus da `comp162` (decisão
D-c: denominador de cobertura idêntico), então o substrato é literalmente o mesmo artefato.
Se `censo_substrato.csv` divergir do da referência, alguma coisa foi regerada quando não
devia — e o diff entre os dois CSVs é o teste mais barato disso.
...
Uso:
    ...
    uv run python scripts/censo_substrato.py --corpus <dir>
"""
...
def census_row(dataset: Path, apk: str) -> dict:
...
def main() -> int:
    ...
    ap.add_argument("--corpus", default=None, help="diretório do corpus instrumentado")
    args = ap.parse_args()
    ...
    rows = [census_row(dataset, a) for a in apks]

#!/usr/bin/env python3
"""Os sete portões de EXECUÇÃO do smoke da gh104. É o critério de aceitação, não o `up -d`.

    uv run python scripts/smoke_gates.py

CÓPIA de `../../experimento-comp162/scripts/smoke_gates.py`. Mudaram os nomes dos
containers (`gh104smoke_NN`) e o tamanho do lote crítico, que agora é lido de
`filters/batch_*.txt` em vez de constante — a partição desta campanha é podada, então uma
constante desalinharia em silêncio com a projeção de wall-clock. A lógica dos portões é a
mesma.

**Estes portões não cobrem a promessa da gh104.** Eles perguntam se a configuração resolve:
braços corretos, cobertura não nula, corpus certo, sem `VerifyError`. Se as mensagens de
violação saíram legíveis é outra pergunta, e quem a responde é `gh104_gates.py` rodando
sobre os mesmos `results_smoke/`. Os dois têm de passar antes de a campanha subir.
...
O portão 7 confere que a guarda INV-APV-60 está *dentro* da imagem. Nesta campanha a
imagem é reconstruída por outro motivo — o código Java, Python e as specs da gh104 —, então
a guarda poderia se perder num rebuild sem que nada acusasse. Ela é o que faz exploração
truncada virar `ERROR` no ato em vez de `COMPLETED` silencioso.
"""
...
CONTAINERS = ["gh104smoke_00", "gh104smoke_01"]
TIMEOUT = 120
...
# O lote mais cheio da campanha, para a projeção de wall-clock. Lido dos filtros e não
# fixado: a partição desta campanha é herdada da comp162 e PODADA, então o lote crítico
# depende de quantas aplicações sobreviveram ao estágio 1.
_batches = sorted((EXP / "filters").glob("batch_*.txt"))
_biggest = max(
    (len([ln for ln in b.read_text().splitlines() if ln.strip()]) for b in _batches),
    default=21,
)
if not _batches:
    print("!! filters/batch_*.txt ausentes — a projeção de wall-clock usa o lote de 21 da "
          "comp162, não o desta campanha (rode make_campaign_filters.py)")
BIGGEST_BATCH_RUNS = _biggest * 3 * 3

fails: list[int] = []


def gate(n, title, ok, evidence, medido=None):
    """Imprime o veredito de um portão. `medido` é o tamanho da população que ele leu.

    Um portão cuja população é vazia **reprova**, nunca passa. Todo portão daqui é um
    laço sobre identidades ou arquivos com o acumulador começando em `True`: sem o
    contador, `tasks.json` ausente faz o laço não rodar e o portão anunciar PASS sobre
    nada — verde do que não foi medido, que é a forma de falha mais cara que este
    ferramental já produziu.
    """
    if medido is not None and medido == 0:
        ok = False
        evidence = ["NADA MEDIDO (população vazia) — o portão não passa por omissão",
                    *evidence]
...
gate(2, "cov_method > 0 e cov_act > 0 em todos os braços", ok, ev, medido=len(ev))
...
gate(3, "RUN_START: preset resolvido e deltas do braço corretos (jar stage-4)", ok3, ev3,
     medido=len(ev3))
gate(4, "ape.mopDataPath presente nos dois braços aperv", ok4, ev4, medido=len(ev4))
...
gate(5, "braço ape: cobertura > 0 e trace não vazio", ok5, ev5, medido=len(ev5))
...
gate(6, "zero VerifyError de carga (instrumentação nova)", not ve,
     ve or [f"nenhum VerifyError em {lidos} logcat(s)"], medido=lidos)

# --- Portão 7: a guarda INV-APV-60 está DENTRO da imagem --------------------
# Sem ela, exploração truncada é gravada COMPLETED com error_message vazio e o resume não a
# alcança — na cmp163 isso custou um script de reparo offline e um APK que truncou duas
# vezes sem ser detectado na hora. A imagem desta campanha é NOVA, construída por causa da
# gh104, então a presença da guarda volta a ser uma pergunta em aberto.
...
if els:
    ...
    print(f"     (lote crítico: {_biggest} APKs x 3 braços x 3 reps)")
    print(f"     (referência da comp162 no mesmo host: 370 s/run -> "
...
if fails:
    ...
    sys.exit(1)
print("SMOKE APROVADO — todos os sete portões de execução passaram")
print("FALTA o portão das mensagens; a campanha NÃO sobe só com isto:")
print("  uv run python scripts/gh104_gates.py \\")
print("      --results-glob 'results_smoke/gh104smoke_*/gh104smoke_*' --label smoke-gh104")

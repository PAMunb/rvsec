...
CMP162_MANIFEST = RV_ANDROID / "modules" / "aperv-tool" / "tests" / "fixtures" / "cmp162_manifest.json"

ARTICLE_ERRORS_CSV = WORKSPACE / "ase-journal" / "dataset" / "results" / "errors.csv"
ARTICLE_TIER_REPORT = WORKSPACE / "ase-journal" / "docs" / "20260806_owasp_cwe_mapping_report.md"
...
COMP162_HEADER: tuple[str, ...] = (
    "apk", "rep", "timeout", "tool", "time",
    "spec", "class", "method", "source", "message", "unique_msg",
)  # 11 columns - carries `source`

ARTICLE_HEADER: tuple[str, ...] = (
    "apk", "rep", "timeout", "tool", "time",
    "spec", "class", "method", "message", "unique_msg",
...
def make_observed_value(freeze: FreezeRegistry) -> Callable[[Mapping[str, str]], str | None]:
...
def load_comp162() -> Corpus:
    ...
    if not files:
        raise FileNotFoundError(f"no comp162 shards under {COMP162_RESULTS}/{COMP162_GLOB}")
    rows: list[dict[str, str]] = []
...
def measure_article(rows: list[dict[str, str]], freeze: FreezeRegistry) -> dict[str, Any]:
    ...
    env["rows"] = Envelope(
        "rows in the article dataset", total, total, "rows", (), ART_IN,
        excluded="nothing; the file is read whole", expected=97018,
    )
    env["distinct_messages"] = Envelope(
        "distinct `message` strings", len(messages), total, "distinct strings / rows", (), ART_IN,
        expected=19,
        note="19 strings for 97,018 events: the report vocabulary of the whole published corpus.",
    )
    env["mute"] = Envelope(
        "mute rows (`unknown`)", len(mute_rows), total, "rows", ("mute",), ART_IN,
        expected=70760,
    )
    env["mute_sites"] = Envelope(
        "distinct sites emitting a mute row", len({site(r) for r in mute_rows}), None,
        "sites", ("mute", "site_article"), ART_IN,
    )
    env["misuses"] = Envelope(
        "unique misuses (R1)", len({misuse(r) for r in rows}), total, "misuses / rows",
        ("misuse_article",), ART_IN, expected=454,
    )
    env["apps"] = Envelope(
        "applications with at least one violation", len({r["apk"] for r in rows}), None,
        "apks", (), ART_IN, expected=113,
    )

    # `but found .` - the legible-looking rows that say nothing either.
    bf_dot = [r for r in rows if observed(r) == ""]
    env["but_found_dot"] = Envelope(
        "rows whose observed value is empty (`but found .`)", len(bf_dot), total, "rows",
        ("observed_value",), ART_IN, expected=8843,
    )
    bf_by_spec = collections.Counter(r["spec"] for r in bf_dot)
    but_found_dot_by_spec = {
        spec: Envelope(
            f"`but found .` rows in {spec}", n, len(bf_dot), "rows",
            ("observed_value",), ART_IN,
        )
    ...
    for label, k in THIRD_PARTY_CUTS:
        ...
        third_party[label] = Envelope(
            f"rows in third-party packages ({label})", n, total, "rows",
            ("third_party_prefixes",), ART_IN,
    ...
    unsafe_algorithm = {
        "weak_of_all_unsafe_algorithm": Envelope(
            "weak-digest rows among all `UnsafeAlgorithm` rows", len(weak), len(ua_rows), "rows",
            ("error_type", "observed_value"), ART_IN, expected=5892,
    ...
    weak_breakdown = {
        value: Envelope(
            ...
            weak_by_value.get(value, 0), len(weak), "rows",
            ("observed_value",), ART_IN,
        )
    ...
    footprint = {}
    for spec, labelled in (("SSLContextSpec", "UnsafeProtocol"), ("TrustManagerFactorySpec", "UnsafeAlgorithm")):
        ...
        footprint[spec] = {
            "sequence": Envelope(
                ...
                sum(by_type.values()), "rows", ("error_type",), ART_IN,
                expected={"SSLContextSpec": 17510, "TrustManagerFactorySpec": 9015}[spec],
            ),
            "labelled": Envelope(
                f"{spec} `{labelled}` rows", by_type.get(labelled, 0), sum(by_type.values()),
                "rows", ("error_type",), ART_IN,
                expected={"SSLContextSpec": 8802, "TrustManagerFactorySpec": 9014}[spec],
            ),
            "ratio": round(
                by_type.get("InvalidSequenceOfMethodCalls", 0) / by_type[labelled], 4
            ) if by_type.get(labelled) else None,
        }

    return {
        "scalars": {k: v.to_dict() for k, v in env.items()},
        "but_found_dot_by_spec": {k: v.to_dict() for k, v in but_found_dot_by_spec.items()},
        ...
        "error_types": dict(sorted(collections.Counter(error_type(r) for r in rows).items())),
...
def measure_comp162(
    ...
    env: dict[str, Envelope] = {}
    env["rows"] = Envelope(
        "rows in the comp162 fixture", total, total, "rows", ("shards",), inputs,
        excluded="nothing; the eight shards are concatenated, never averaged", expected=19664,
    )
    env["mute"] = Envelope(
        "mute rows (`unknown`)", len(mute_rows), total, "rows", ("mute",), inputs, expected=15714,
    )
    env["mute_sites"] = Envelope(
        "distinct sites emitting a mute row", len(mute_sites), None, "sites",
        ("mute", "site_comp162"), inputs, expected=296,
    )
    env["legible_rows"] = Envelope(
        "legible rows (message is not `unknown`)", len(legible_rows), total, "rows",
        ("mute",), inputs, expected=3950,
    )
    env["legible_sites"] = Envelope(
        "distinct sites emitting a legible row", len(legible_sites), None, "sites",
        ("mute", "site_comp162"), inputs, expected=101,
    )
    ...
    env["mute_legible_twin_rows"] = Envelope(
        ...
        sum(mute_sites[s] for s in twin_sites), len(mute_rows), "rows",
        ("mute", "site_comp162"), inputs, expected=3950,
    )
    env["mute_legible_twin_sites"] = Envelope(
        "sites emitting both a mute and a legible row", len(twin_sites), len(mute_sites), "sites",
        ("mute", "site_comp162"), inputs, expected=101,
    )
    env["legible_only_sites"] = Envelope(
        "sites emitting only legible rows", len([s for s in legible_sites if s not in mute_sites]),
        len(legible_sites), "sites", ("mute", "site_comp162"), inputs, expected=0,
    ...
    per_site_types: dict[tuple[str, ...], collections.Counter] = collections.defaultdict(collections.Counter)
    for row in mute_rows:
        per_site_types[site(row)][error_type(row)] += 1
    mute_mute = {
        s: c for s, c in per_site_types.items()
    ...
    env["mute_mute_twin_rows"] = Envelope(
        ...
        sum(sum(c.values()) for c in mute_mute.values()), len(mute_rows), "rows",
        ("mute", "site_comp162", "error_type"), inputs, expected=838,
    )
    env["mute_mute_twin_sites"] = Envelope(
        "sites emitting two ErrorTypes in equal numbers, all mute", len(mute_mute), len(mute_sites),
        "sites", ("mute", "site_comp162", "error_type"), inputs, expected=12,
    )
    ...
    for etype, n in sorted(mute_mute_by_type.items()):
        env[f"mute_mute_twin_rows_{etype}"] = Envelope(
            f"of those, rows labelled `{etype}`", n,
            sum(mute_mute_by_type.values()), "rows",
            ("mute", "site_comp162", "error_type"), inputs, expected=419,
    ...
    remainder_sites = [s for s in mute_sites if s not in legible_sites and s not in mute_mute]
    env["mute_only_rows"] = Envelope(
        "mute rows at sites with no legible twin and no equal-count second type",
        sum(mute_sites[s] for s in remainder_sites), len(mute_rows), "rows",
        ("mute", "site_comp162", "error_type"), inputs, expected=10926,
    )
    env["mute_only_sites"] = Envelope(
        "sites in that remainder", len(remainder_sites), len(mute_sites), "sites",
        ("mute", "site_comp162", "error_type"), inputs, expected=183,
    )
    env["but_found_dot"] = Envelope(
        "rows whose observed value is empty (`but found .`)",
        sum(1 for r in rows if observed(r) == ""), total, "rows",
        ("observed_value",), inputs, expected=98,
    )
    ...
    env["identities_8tuple"] = Envelope(
        "distinct identities under the 8-tuple", len(ident), total, "identities / rows",
        ("identity8",), inputs, expected=6344,
    )
    env["repeated_rows"] = Envelope(
        "rows that are a repeat of an identity already seen", total - len(ident), total, "rows",
        ("identity8",), inputs, expected=total - 6344,
        note="67.74 % of the corpus is repetition of an identity under the 8-tuple.",
    )
    env["max_repetition"] = Envelope(
        "largest number of rows carried by one 8-tuple identity", max(ident.values()), total,
        "rows", ("identity8",), inputs, expected=49,
    )
    summary5 = {(r["spec"], error_type(r), r["class"], r["method"], r["source"]) for r in rows}
    env["identities_errorsummary_5tuple"] = Envelope(
        ...
        len(summary5), total, "identities / rows", ("identity8", "error_type"), inputs, expected=409,
    ...
    env["identities_spanning_shards"] = Envelope(
        ...
        sum(1 for s in shards_of_identity.values() if len(s) > 1), len(ident), "identities",
        ("identity8", "shards"), inputs, expected=0,
        note="zero, so 0 % of the repetition comes from replication: it is all within-shard.",
    )
    env["shard_apk_overlap"] = Envelope(
        "APKs appearing in more than one shard", overlap, len(seen), "apks", ("shards",), inputs,
        expected=0,
    )
    env["distinct_apks"] = Envelope(
        "distinct APKs across the eight shards", len(seen), None, "apks", ("shards",), inputs,
        expected=112,
    )

    per_spec_mute = {
        spec: Envelope(
            f"mute rows in {spec}", n, len(mute_rows), "rows", ("mute",), inputs,
        )
        for spec, n in collections.Counter(r["spec"] for r in mute_rows).most_common()
    }

    footprint = {}
    for spec, labelled in (("TrustManagerFactorySpec", "UnsafeAlgorithm"), ("SecureRandomSpec", "UnsafeAlgorithm")):
        by_type = collections.Counter(error_type(r) for r in rows if r["spec"] == spec)
        seq = by_type.get("InvalidSequenceOfMethodCalls", 0)
        lab = by_type.get(labelled, 0)
        footprint[spec] = {
            "sequence": Envelope(
                f"{spec} `InvalidSequenceOfMethodCalls` rows", seq, sum(by_type.values()), "rows",
                ("error_type",), inputs,
                expected={"TrustManagerFactorySpec": 2855, "SecureRandomSpec": 2882}[spec],
            ).to_dict(),
            "labelled": Envelope(
                f"{spec} `{labelled}` rows", lab, sum(by_type.values()), "rows",
                ("error_type",), inputs,
...
def measure_android_tier(rows: list[dict[str, str]], freeze: FreezeRegistry) -> dict[str, Any]:
    error_type = make_error_type(freeze)
    observed = make_observed_value(freeze)
    misuse = make_key(freeze.require("misuse_article"))
    ...
    for spec, etype, value, exp_ev, exp_apps, exp_um in ANDROID_TIER:
        sel = [r for r in rows if r["spec"] == spec and error_type(r) == etype and observed(r) == value]
        ...
        out.append({
            ...
            "events": Envelope(
                f"{spec} / {etype} / `{value}` events", len(sel), total_rows, "rows",
                ("error_type", "observed_value"), ART_IN, expected=exp_ev,
            ).to_dict(),
            "apps": Envelope(
                f"{spec} / {etype} / `{value}` applications", apps, 113, "apks",
                ("error_type", "observed_value"), ART_IN, expected=exp_apps,
            ).to_dict(),
            "misuses": Envelope(
                f"{spec} / {etype} / `{value}` unique misuses", misuses, all_misuses, "misuses",
                ("error_type", "observed_value", "misuse_article"), ART_IN, expected=exp_um,
            ).to_dict(),
        })
    ...
    return {
        ...
        "total_events": Envelope(
            "events in the publishable Android tier", ev_sum, total_rows, "rows",
            ("error_type", "observed_value"), ART_IN, expected=11409,
        ).to_dict(),
        "total_misuses": Envelope(
            "unique misuses in the publishable Android tier", um_sum, all_misuses, "misuses",
            ("error_type", "observed_value", "misuse_article"), ART_IN, expected=84,
        ).to_dict(),
        "source": {
...
def measure_spec_sets() -> dict[str, Any]:
    ...
    for name in SPEC_SET_DIRS:
        ...
        with_add_error = sum(1 for f in files if "addError" in f.read_text(encoding="utf-8", errors="replace"))
        with_log_v = sum(1 for f in files if "Log.v" in f.read_text(encoding="utf-8", errors="replace"))
        sets[name] = {
            "path": rel(directory),
            "files": len(files),
    ...
    return {
        ...
        "error_description": {
            "total": Envelope(
                "`new ErrorDescription(` sites in the frozen `jca` set", total, total, "sites",
                (), ("jca_spec_set",), expected=51,
            ).to_dict(),
            "three_arg": Envelope(
                "three-argument sites (no message: these emit `unknown`)", arity.get(3, 0), total,
                "sites", (), ("jca_spec_set",), expected=25,
            ).to_dict(),
            "four_arg": Envelope(
                "four-argument sites (a message is supplied)", arity.get(4, 0), total, "sites",
                (), ("jca_spec_set",), expected=26,
            ).to_dict(),
            "other_arity": {str(k): v for k, v in sorted(arity.items()) if k not in (3, 4)},
...
def collect_envelopes(node: Any, path: str = "") -> Iterable[tuple[str, dict[str, Any]]]:
...
def render_markdown(b: Mapping[str, Any]) -> str:
    ...
    add("")
    if dis:
        add("The following measurements do **not** reproduce the expected value. The data wins.")
        add("")
        add(_TABLE_HEAD)
        for path, env in dis:
            add(_env_row(env))
    ...
    add("### Weak digests under `UnsafeAlgorithm` - three denominators, three questions")
    add("")
    add(_TABLE_HEAD)
    ...
    for row in b["android_tier"]["rows"]:
        verdicts = {row["events"]["agrees"], row["apps"]["agrees"], row["misuses"]["agrees"]}
...
def render_definitions(b: Mapping[str, Any]) -> str:
    ...
    for key, item in b["definitions"].items():
        value = item["value"]
        rendered = (
            ", ".join(f"`{v}`" for v in value) if isinstance(value, list) else f"`{value}`"
        )
        add(f"### `{key}`")
        add("")
        add(item["definition"])
...
def write_baseline(out_dir: Path = OUT_DIR, freeze: FreezeRegistry = DEFAULT_FREEZE) -> dict[str, Any]:
    ...
    (out_dir / "definitions.md").write_text(render_definitions(baseline), encoding="utf-8")
    return baseline
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"envelopes: {len(envs)}; with an expected value: {len(checked)}; disagreements: {len(bad)}")
    for path, env in bad:
        print(f"  DISAGREES {path}: measured {env['numerator']} expected {env['expected']}")

...
def verify_phase_b(
    results_dir: Path, n_batches: int = 6, expected_rows: int = 945
) -> VerificationResult:
    ...
    result.checks.append(
        Check(
            ...
            detail=(
                ...
                if missing_batches
                else f"All {n_batches} batch summaries found"
            ),
        )
    )
        ...
        if "tool" in df.columns and "errors" in df.columns:
            ...
            result.checks.append(
                Check(
                    ...
                    passed=is_valid,
                    detail=(
                        f"BASELINE_MAX_ERRORS = {max_errors:.2f}"
                        if is_valid
                        else f"Invalid value: {max_errors}"
                    ),
                )
            )
        else:
...
def verify_phase_d(
    results_dir: Path, macro_dir: Path | None = None, expected_trials: int = 100
) -> VerificationResult:
            ...
            if macro_score > 0:
                ...
                result.checks.append(
                    Check(
                        name="score_improvement",
                        passed=False,
                        detail="Macro score is 0 — cannot compute improvement",
                    )
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--results-dir", required=True, type=Path, help="Path to phase results"
    )
    parser.add_argument(
        "--baseline-dir", type=Path, help="Path to baseline results (Phase E)"
    )
    parser.add_argument(
        "--macro-dir", type=Path, help="Path to macro results (Phase D)"
    )

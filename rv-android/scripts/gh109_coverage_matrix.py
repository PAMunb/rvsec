...
ADJUDICATED_STATES = {
    "HMACParameterSpec": (
        ...
        "is kept as documentation of untranslatability, not as coverage; retiring it to "
        "backup/ stays available and would move only the enumeration constants. On a JVM, "
        "where the class does load, the composition its ENSURES feeds is out of reach too: "
        "no algorithm of Mac.crysl:44 accepts the parameter spec, which is what rows 38 and "
        "80 of predicate_ledger.csv record from the requiring and the producing side",
    ),
    "Cookie": (
        "na-platform",

...
ROOT = Path(__file__).resolve().parents[1]  # experimento-20260721/ (self-contained: results/ + configs/)
...
def main():
    ...
    meta = json.loads((ROOT / "configs" / f"{name}_compare_meta.json").read_text())
    ...
    out = ROOT / "results" / f"{name}_consolidado"
    out.mkdir(parents=True, exist_ok=True)

    rows, seen = [], set()
    for i in range(containers):
        nn = f"{i:02d}"
        base = ROOT / "results" / f"{name}_{nn}" / f"{name}_{nn}"
        tj = base / "tasks.json"
        if not tj.exists():
            continue

...
ADB_PORT=5555
PARTITION=${PARTITION:-8192} # cndaqiang add to set storage. defaulr 8192 8G
...
TEST_AVD=$(avdmanager list avd | grep -c "${EMULATOR_NAME}.avd" || true)
...
echo "GPU           - $GPU_MODE"
echo "PARTITION     - $PARTITION" # cndaqiang add to set storage

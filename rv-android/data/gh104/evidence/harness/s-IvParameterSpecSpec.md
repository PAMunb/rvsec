...
| `IvParameterSpecSpec-zero-length.txt` | introduced | — | c2:IVPARAMETERSPEC-CONSTR-02 |
...
- `IvParameterSpecSpec-zero-length.txt` (B) `spec=IvParameterSpecSpec,ev=c2,type=UnsatisfiedConstraint,msg=v=1 code=IVPARAMETERSPEC-CONSTR-02 ev=c2 obj=IvParameterSpec val='0' exp='a length greater than zero' msg='the iv length given to IvParameterSpec(byte[], int, int) is not greater than zero'`

#!/usr/bin/env python3
"""ANÁLISE comp162 — admissibilidade primeiro, depois agregação.
...
somar qualquer coisa, e a regra mora em `admissibility.py` porque o `compare_cmp163.py`
precisa aplicar exatamente a mesma aos dois lados.
...
**O que R=3 acrescenta e a cmp163 não podia ter.** A §2 mede o desvio-padrão entre réplicas,
que é a lacuna declarada na §7 daquele relatório: com R=1 dava para dizer que uma diferença
era consistente entre aplicações, mas não que era maior que o ruído de repetição.
...
cru, que o `compare_cmp163.py` lê.
"""
...
TASKS_GLOB = str(EXP / "results/comp162_0*/comp162_0*/tasks.json")
...
def main() -> None:
    ...
    # Sem arredondar. Este CSV é a ENTRADA da estatística do `compare_cmp163.py`, não uma
    ...
    print("=" * 82)
    print("É a lacuna declarada na §7 do relatório da cmp163. A gh97 mediu 1,75-2,12 pp em")
    ...
    # As mesmas quatro famílias da cmp163, para que as duas campanhas respondam à mesma

#!/usr/bin/env python3
"""REPAIR — rede de segurança para tasks da comp162 que terminaram antes do orçamento.

Nesta campanha o script é uma **rede de segurança, não o mecanismo**, e a diferença é o
motivo de a imagem ter sido reconstruída. A imagem `0.9.3-comp162` carrega a guarda
...
grava `ERROR`, e o resume comum recupera a identidade sem operador nenhum. Na cmp163 nada
disso existia — truncamento chegava ao consolidador indistinguível de run íntegro, e este
script era o único caminho de recuperação.
...
Uso (o `tasks.json` é escrito como root; daí o docker):
    python3 scripts/repair.py comp162_00 comp162_01          # dry run
    docker run --rm -u 0 -v "$PWD:/w" -w /w --entrypoint python3 \
        phtcosta/rvandroid:0.9.3-comp162 scripts/repair.py --apply comp162_00
"""
...
RESULTS = EXP / "results"
BACKUP_DIR = EXP.parent / "backup" / "comp162-truncated"

#!/usr/bin/env python3
"""Consolidação offline da comp162: tasks.json + logcats -> CSVs em consolidado/.

    uv run python scripts/consolidate.py

Cópia adaptada de `.claude/skills/rv-experiment-compare/scripts/consolidate_compare.py`.
As mudanças são de caminho e de saída, nunca de métrica — as colunas e a agregação têm de
ser idênticas às da cmp163, senão o pareamento entre campanhas compara coisas diferentes
com o mesmo nome:

1. Os resultados moram em `experimento-comp162/results/`, não em `data/results/<name>_NN`,
   e não há `_compare_meta.json` — os braços vêm do `manifest.json`.
2. `per_rep.csv` é novo. Com R=3 existe dispersão dentro da célula, e ela é justamente o
   que a cmp163 não pôde medir (§7 do relatório dela). O CSV por réplica é o insumo do
   `analise.py` para o desvio-padrão entre réplicas.
...
def main() -> int:
    ...
    # teste pareado e a mesma agregação da cmp163, para que as duas campanhas possam ser

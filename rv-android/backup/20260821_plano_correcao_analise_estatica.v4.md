...
executadas. Nenhum defeito mudou de veredito e a aritmética das §0/§2 fecha (24 alegações − 15
mortas = 4 defeitos + 5 itens de faxina). Corrigiram-se: a contagem de edições locais no `gator`
(eram três, não duas); a linha da spec que sustenta a refutação do D3; a linha do `CONTEXTO.md` que
declara o preço da gh104; o bloco de contagem da §7, que não rodava e agora está verificado
(120/119 assinaturas medidos duas vezes); a pendência dos logs das sondas, que não existem mais; um
quarto sítio de `--sdkpath`; e um defeito documental novo em `static-analysis.md:85`. A trilha
segue decidida: **uma change só**, Quick Path. Versão anterior em
`backup/20260821_plano_correcao_analise_estatica.v3.md`.
...
dois deles somam quatro linhas, e o de maior alcance é o D2.
...
gator in rv-android libs"*) — é código do GATOR upstream, importado como veio. As **três** edições
locais no arquivo são o nome do jar mais `-outputFile` (`cf649214`, 2024-10-05), `--jvm-memory`
...
`docs/20260730_verificacao_consistencia_gh91.md:617` e
`docs/20260821_relatorio_analise_estatica_defeitos.md:92`.
...
**Não fracionar.** Os quatro compartilham a mesma superfície (o acoplamento experimento ↔
análise estática), tocam seis arquivos do lado Python mais dois da árvore Java, e dois deles somam
quatro linhas (P3 uma, D1-A três); o J1 é um parâmetro, uma guarda e dois chamadores. Quatro changes
para isso seria o anti-padrão que a própria §3 do WORKFLOW nomeia — *"a sledgehammer to crack a
nut"*.
...
| Docs | `docs/architecture/static-analysis.md` §3, §4 (o argv literal de `:85`), §7 (`:143`), a afirmação de validação de `:85` que o `validate_on_init=False` desmente, as quatro correções de `--sdkpath`, e o registro do acoplamento `specification_set → mop_dir` |
...
O `Count.java` e o `javac` que o compila estão em `docs/20260821_handoff_gh69_coringas.md` §8.1 —
copie o bloco de lá inteiro, porque só a linha do `java` não roda sem a classe compilada. Depois de
gerá-la em `$S`:
...
# medido 2026-08-21 e reexecutado na revisão 4 com o mesmo resultado
```
...
| Item | Onde |
|---|---|
| `--sdkpath` → `--sdk` (quatro sítios) | `openspec/changes/archive/2026-07-31-gh91-.../design.md:143`, `.../tasks.md:69`, `docs/20260730_verificacao_consistencia_gh91.md:617` e `docs/20260821_relatorio_analise_estatica_defeitos.md:92`, que reproduz a nota da gh91 — corrigir a nota e deixar a citação errada seria incoerente |
...
| logs das duas sondas **perdidos**, não apenas não commitados | `probe_run.log` e `probe_run_b.log` já não existem em `/tmp` (conferido na revisão 4). Não há o que copiar: registrar a perda onde a evidência os cita. O que sobrevive em `data/gh105/evidence/reach-probe/` são `probe-a.logcat`, `probe-b.logcat`, `coverage.csv`, `errors.csv`, `probe-a-errors.csv`, `summary.csv` e `experiment_config.json` — suficiente para o veredito das sondas, insuficiente para reconstruir o console do run |

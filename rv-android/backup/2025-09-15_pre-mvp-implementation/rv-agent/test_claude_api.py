#!/usr/bin/env python3
"""
Claude API Test Script for RVAgent Phase 0 Validation
Tests 3 apps × 5 screenshots using optimal parameters discovered from Qwen testing
"""
...
def create_enhanced_prompt(elements: List[Dict]) -> str:
    """Create the coordinate validation enhanced prompt that achieved 72% success."""
...
def test_claude_vision(image_path: str, uiautomator_path: str, temperature: float = 0.05) -> Dict[str, Any]:
    """Test Claude's vision capabilities with coordinate validation."""
    ...
    try:
        # Create client with basic configuration only
        client = anthropic.Anthropic(api_key=api_key)
    except TypeError as e:
        # If there's a TypeError with proxies, try without any HTTP client configuration
        try:
            ...
            return {'error': f'Client initialization failed even after clearing proxies: {e2}'}
...
def run_claude_test():
    """Run Claude API test with 3 apps × 5 screenshots using optimal parameters."""
    
    # Optimal parameters discovered from Qwen testing
    TEMPERATURE = 0.05  # Best performing parameter from extensive testing
    ...
    csv_file = f"claude_test_results_{timestamp}.csv"
    ...
    if success_rate >= 70:
        recommendation = "🎉 EXCELLENT - Claude performs as well as Qwen!"
...
if __name__ == "__main__":
    print("🔬 Claude API Test for RVAgent Phase 0 Validation")
    print("Using coordinate validation enhanced strategy")
    ...
    if results:
        ...
        print("Use analyze_results_simple.py for detailed analysis")
    else:

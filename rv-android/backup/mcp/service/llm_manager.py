"""
Resource-aware LLM manager with MCP integration.
...
strategies to maintain system stability while using the MCP framework.
"""
...
from rvandroid.mcp.mcp_data_structures import MCPConfiguration, MCPMessage, MCPRole, MCPTextContent
...
class ResourceAwareLLMManager:
    """
    Resource-aware manager for LLM operations using MCP that integrates with the
    ...
    - Uses MCP framework for standardized LLM communication
    ...
    - Provides a standardized MCP interface for LLM interactions
    """
    ...
    def __init__(self, static_data: Optional[StaticAnalysisData] = None,
        ...
        # Lazy initialization for the LLM service
        ...
        self.logger.info(f"Initialized resource-aware LLM manager with {model_type}/{model_name} using MCP")

    @property
    def llm_service(self):
        """Lazy initialization of LLM service using MCP."""
        if self._llm_service is None:
            from rvandroid.rvdroid.llm.llm_service import LLMService
            self.logger.info("Initializing LLM service with MCP")
    ...
    def get_strategic_guidance(self, query_type: str, state_data: Dict[str, Any],
        ...
        """
        Get strategic guidance with resource awareness using MCP.
    ...
    def get_action_feedback(self, action_data: Dict[str, Any], result: Dict[str, Any],
        ...
        """
        Get action feedback with resource awareness using MCP.
    ...
    def get_exploration_strategy(self, state_data: Dict[str, Any],
        ...
        """
        Get exploration strategy with resource awareness using MCP.
        ...
        try:
            ...
            # Measure performance
            with self.performance_monitor.measure_time("exploration_strategy_generation"):
    ...
    def interpret_monitored_operations(self, state_data: Dict[str, Any],
        ...
        """
        Interpret monitored operations with resource awareness using MCP.
        ...
        if self._should_throttle_llm_operation():
            self.operation_metrics["throttled_queries"] += 1
            self.logger.info("Throttling monitored operations interpretation due to resource constraints")
        ...
        try:
            # Optimize monitored operations based on resource status
            optimized_operations = monitored_operations
            if len(monitored_operations) > 10 and self.resource_manager.resource_status["throttling_level"] >= 1:
                # Prioritize directly reaching operations if there are many
                optimized_operations = [
                    op for op in monitored_operations 
            ...
            # Measure performance
            with self.performance_monitor.measure_time("monitored_operations_interpretation"):
                # Get interpretation from LLM service
                result = self.llm_service.interpret_monitored_operations(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["total_queries"] > 0:
            ...
            metrics["throttling_percentage"] = 0.0
    ...
    def _should_throttle_llm_operation(self) -> bool:
        ...
        if hasattr(self.resource_manager, 'should_throttle_operation'):
    ...
    def _generate_fallback_guidance(self, query_type: str) -> Dict[str, Any]:
        ...
        if query_type == "exploration":
            ...
            return {
                ...
                "guidance": "Focus on input fields and API usage patterns.",
                "directives": [
    ...
    def cleanup(self) -> None:
        ...
        if self._llm_service and hasattr(self._llm_service, 'cleanup'):

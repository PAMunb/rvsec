...
@click.option('--applications-dir', '-a',
              type=click.Path(exists=True),
              help='Directory containing APK files to test')
@click.option('--applications-path',
              type=click.Path(exists=True),
              help='Specific APK file path to test')
@click.option('--applications-patterns',
              help='Comma-separated patterns for APK discovery (default: *.apk)')
@pass_context
def run_single(ctx: CLIContext, tool: str, timeout: int, repetitions: int,
               ...
               applications_dir: Optional[str], applications_path: Optional[str],
               applications_patterns: Optional[str]):
    """
    ...
    with ctx.logger.with_context(
        ...
        try:
            ...
            config.execution.timeouts = [timeout]
            config.execution.repetitions = repetitions
            config.execution.no_window = no_window
            config.processing.generate_monitors = not skip_monitors
            config.processing.instrument = not skip_instrument
            config.processing.static_analysis = not skip_static
            
            if output_dir:
                config.output_dir = output_dir
            
            # Configure applications based on CLI parameters
            if applications_path:
                # Single APK file specified
                config.applications.path = applications_path
            elif applications_dir:
                # Directory with optional patterns specified
                config.applications.directory = applications_dir
                if applications_patterns:
                    config.applications.patterns = [p.strip() for p in applications_patterns.split(',')]
                else:
                    config.applications.patterns = ["*.apk"]
            else:
                # Use default applications directory if no specific application is configured
                default_apks_dir = "./apks_examples"
                if os.path.exists(default_apks_dir):
                    config.applications.directory = default_apks_dir
                    config.applications.patterns = ["*.apk"]
                    ctx.logger.info(f"Using default applications directory: {default_apks_dir}")
                else:
                    # Try common APK directories
                    common_dirs = ["./apks", "./applications", "./test_apps"]
                    for common_dir in common_dirs:
                        if os.path.exists(common_dir):
                            config.applications.directory = common_dir
                            config.applications.patterns = ["*.apk"]
                            ctx.logger.info(f"Found applications directory: {common_dir}")
                            break
                    else:
                        raise ValueError(
                            "No applications configured. Specify --applications-dir, --applications-path, "
                            "or ensure ./apks_examples directory exists with APK files."
                        )
...
def run_comparative(ctx: CLIContext, tools: str, timeouts: str, repetitions: int,
    ...
    with ctx.logger.with_context(
        ...
        try:
            ...
            config.execution.timeouts = timeout_list
            config.execution.repetitions = repetitions
            config.execution.no_window = no_window
            config.execution.parallel_execution = parallel
...
def run_local(ctx: CLIContext, tools: str, timeout: int, repetitions: int):
    ...
    with ctx.logger.with_context(
        ...
        try:
            ...
            config.execution.timeouts = [timeout]
            config.execution.repetitions = repetitions
            config.execution.no_window = True  # Always headless for local testing
            config.processing.generate_monitors = True
            config.processing.instrument = True
            config.processing.static_analysis = True
            config.output_dir = "./local_experiment_results"
            
            # Configure default applications for local testing
            default_apks_dir = "./apks_examples"
            if os.path.exists(default_apks_dir):
                config.applications.directory = default_apks_dir
                config.applications.patterns = ["*.apk"]
            else:
                # Try common APK directories for local development
                common_dirs = ["./apks", "./applications", "./test_apps"]
                for common_dir in common_dirs:
                    if os.path.exists(common_dir):
                        config.applications.directory = common_dir
                        config.applications.patterns = ["*.apk"]
                        ctx.logger.info(f"Found applications directory for local testing: {common_dir}")
                        break
                else:
                    raise ValueError(
                        "No applications found for local testing. Create ./apks_examples directory "
                        "with APK files or ensure one of: ./apks, ./applications, ./test_apps exists."
                    )

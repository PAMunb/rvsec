#!/usr/bin/env python3
"""
Teste Comparativo: V10 vs V12 vs V13 com Auxiliary Counters
=============================================================

Executa teste de 300 segundos (5 min) por app em cada versão de prompt.
Coleta métricas abrangentes incluindo auxiliary counters detalhados.
...
2. Decisor (LLM vs Algoritmo) + Auxiliary Counters (llm_fallback, forced_back, recovery)
3. UI Coverage detalhado
4. Exploração (estados, transições)
5. LLM (tokens, tempo, eficiência)
6. UI Tracker (discovery, interactions)
...
def extract_comprehensive_metrics(
    ...
    metrics = {
        ...
        "decisor": {
            "llm_count": results.get("llm_executed", results.get("llm_decisions", 0)),
            "algorithm_count": results.get("algorithm_chosen", results.get("algorithm_decisions", 0)),
            "llm_percentage": 0.0,
            "algorithm_percentage": 0.0,
            "llm_to_algorithm_ratio": 0.0
        },

        # === 3. AUXILIARY COUNTERS (V13+) ===
        "auxiliary_counters": {
            ...
            "has_data": "llm_fallback" in results
        },
        ...
        "llm": {
            ...
            "calls_total": results.get("llm_executed", results.get("llm_decisions", 0))
        },
        ...
        # === 11. SHORT TERM MEMORY (Screen-scoped) ===
        "short_term_memory": {
        ...
        # === 12. LONG TERM MEMORY (Cross-screen patterns) ===
        "long_term_memory": {
...
def main():
    ...
    print("\n" + "="*80)
    print("PROMPT COMPARISON TEST: V10 vs V12 vs V13")
    print("="*80)
    print()
    ...
    # Filter to only test specific apps
    apks = [apk for apk in apks if apk['package'] in TEST_APPS]

    if not apks:
        logger.error(f"None of the specified apps found in dataset. Looking for: {TEST_APPS}")
        return 1

    # Report missing apps
    found_packages = {apk['package'] for apk in apks}
    missing_packages = set(TEST_APPS) - found_packages
    if missing_packages:
        logger.warning(f"Missing APKs for: {missing_packages}")

    logger.info(f"Discovered {len(apks)}/{len(TEST_APPS)} APKs")

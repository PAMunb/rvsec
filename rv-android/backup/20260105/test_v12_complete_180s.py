#!/usr/bin/env python3
"""
V12 Complete Dataset Test - 180s timeout per app
...
def run_test(package: str, apk_path: Path, timeout: int = 180) -> dict:
    """Run single test with V12 and specified timeout."""

    logger.info(f"{'='*80}")
    logger.info(f"Testing {package} with V12 (timeout={timeout}s)")
    logger.info(f"{'='*80}")
    ...
    config = RVAgentConfig(
        ...
        timeout=timeout,
        max_iterations=200,
        screenshot_dir=f"/tmp/dataset_test_v12_180s/{package}",
...
def main():
    ...
    logger.info("="*80)
    logger.info("V12 COMPLETE DATASET TEST - 180s TIMEOUT")
    logger.info("="*80)
    logger.info("")
    ...
    for i, apk_info in enumerate(apks, 1):
        ...
        if not install_apk(apk_info['apk_path'], apk_info['package']):
            ...
            with open('/tmp/v12_complete_180s_progress.json', 'w') as f:
    ...
    final_path = f"/tmp/v12_complete_180s_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

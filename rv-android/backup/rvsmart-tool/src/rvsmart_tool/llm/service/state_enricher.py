...
class StateEnricher:
    ...
    def __init__(
            ...
            config: Optional[Union[PromptConfig, 'RvSmartToolConfig']] = None
    ):
        ...
        self.logger.warning("🔧 STATEENRICHER INITIALIZATION")
        self.logger.info(f"   Config type: {type(config).__name__ if config else 'None'}")
        self.logger.info(f"   Static data: {'Available' if static_data else 'None'}")
        if config and hasattr(config, 'llm_config'):
            llm_config = config.llm_config
            self.logger.info(f"   LLM config type: {type(llm_config).__name__}")
            if hasattr(llm_config, 'vision'):
                self.logger.info(f"   Vision capability: {llm_config.vision}")
            if hasattr(llm_config, 'model'):
                self.logger.info(f"   Model: {llm_config.model}")
        ...
        try:
            ...
            # RVSMART_FIX: Handle both PromptConfig.visitor_type and RvSmartToolConfig.get_visitor_type()
            visitor_type = None
            if config is not None:
                if hasattr(config, 'visitor_type'):
                    visitor_type = config.visitor_type
                elif hasattr(config, 'get_visitor_type'):
                    visitor_type = config.get_visitor_type()
        ...
        try:
            from rvsmart_tool.analysis.screenshot.screenshot_action_complementor import ScreenshotActionComplementor
    ...
    def enrich_state(self, state: Dict[str, Any]):
        ...
        """
        self.logger.warning("🔍 STATEENRICHER: Starting state enrichment")
        self.logger.info(f"   State keys: {list(state.keys()) if state else 'None'}")
        self.logger.info(f"   Screenshot path: {state.get('screenshot_path', 'Not available')}")
        self.logger.info(f"   Activity: {state.get('activity', 'Unknown')}")

        try:
            # Phase 1: Add screen description if not present
            self.logger.warning("📱 PHASE 1: Adding screen description")
            self._add_screen_description(state)

            # Phase 2: Process screenshot if available
            self.logger.warning("📸 PHASE 2: Processing screenshot")
            self._process_screenshot(state)

            # Phase 3: Update SCREEN_DESCRIPTION after screenshot processing
            self.logger.info("📝 PHASE 3: Updating screen description text")
            self._update_screen_description_text(state)

            # Phase 4: Add monitored operations information
            self.logger.warning("🎯 PHASE 4: Adding monitored operations")
            self._add_monitored_operations(state)
            
            self.logger.warning("✅ STATEENRICHER: State enrichment completed successfully")

        except Exception as e:
            self.logger.error(f"❌ STATEENRICHER: Error enriching state: {e}", exc_info=True)
    ...
    def _process_screenshot(self, state: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"📸 Screenshot processing - Path: {screenshot_path if screenshot_path else 'Not provided'}")
        
        if not screenshot_path:
            self.logger.warning("⚠️ No screenshot path provided in state - multimodal processing unavailable")
            return
    ...
    def create_processing_context(self) -> Dict[str, Any]:
        """
        Create processing context for coordinate enhancement.
        
        Configures vision settings based on tool configuration to enable
        coordinate enhancement in UIElementsFragment.
        
        Returns:
            Processing context dictionary with vision configuration
        """
        processing_context = {}
        
        self.logger.warning("🧠 STATEENRICHER: Creating processing context")
        self.logger.warning("DEBUG_COORD_ENH: Starting processing context creation")
        
        # Configure vision context for coordinate enhancement
        self._configure_vision_context(processing_context)
        
        self.logger.warning(f"DEBUG_COORD_ENH: Processing context created - keys: {list(processing_context.keys())}")
        self.logger.warning(f"DEBUG_COORD_ENH: Final processing context: {processing_context}")
        
        return processing_context
    
    def _configure_vision_context(self, processing_context: Dict[str, Any]) -> None:
        """
        Configure vision-related context for coordinate enhancement.
        
        Detects if vision is enabled in the tool configuration and sets
        the vision_enabled flag for downstream coordinate enhancement.
        
        Args:
            processing_context: Context dictionary to update
        """
        vision_enabled = False
        
        self.logger.warning("🔍 DEBUG_COORD_ENH: Starting vision context configuration")
        self.logger.warning(f"DEBUG_COORD_ENH: Config object: {type(self.config).__name__ if self.config else 'None'}")
        
        # Check if we have a config with vision capabilities
        # COORDINATE_ENH_FIX: Handle both RvSmartToolConfig and PromptConfig
        if self.config and hasattr(self.config, 'llm_config'):
            self.logger.warning("DEBUG_COORD_ENH: Found config.llm_config (RvSmartToolConfig)")
            llm_config = self.config.llm_config
            self.logger.warning(f"DEBUG_COORD_ENH: LLM config type: {type(llm_config).__name__}")
            self.logger.warning(f"DEBUG_COORD_ENH: LLM config attributes: {[attr for attr in dir(llm_config) if not attr.startswith('_')]}")
            
            if hasattr(llm_config, 'vision'):
                vision_value = llm_config.vision
                self.logger.warning(f"DEBUG_COORD_ENH: Found llm_config.vision = {vision_value} (type: {type(vision_value)})")
                if vision_value:
                    vision_enabled = True
                    self.logger.warning(f"✅ DEBUG_COORD_ENH: Vision detected from llm_config.vision = {vision_value}")
            else:
                self.logger.warning("DEBUG_COORD_ENH: llm_config has no 'vision' attribute")
        else:
            self.logger.warning("DEBUG_COORD_ENH: No config.llm_config found (likely PromptConfig)")
        
        # Check alternative config paths
        if not vision_enabled and self.config:
            self.logger.warning("DEBUG_COORD_ENH: Checking alternative config paths")
            self.logger.warning(f"DEBUG_COORD_ENH: Config attributes: {[attr for attr in dir(self.config) if not attr.startswith('_')]}")
            
            # Alternative path through tool_config
            if hasattr(self.config, 'vision'):
                vision_value = self.config.vision
                self.logger.warning(f"DEBUG_COORD_ENH: Found config.vision = {vision_value} (type: {type(vision_value)})")
                if vision_value:
                    vision_enabled = True
                    self.logger.warning(f"DEBUG_COORD_ENH: Vision detected from config.vision = {vision_value}")
            else:
                self.logger.warning("DEBUG_COORD_ENH: Config has no direct 'vision' attribute")
        
        processing_context['vision_enabled'] = vision_enabled
        
        self.logger.warning(f"DEBUG_COORD_ENH: Vision context configured - vision_enabled: {vision_enabled}")
        if vision_enabled:
            self.logger.warning("🚀 DEBUG_COORD_ENH: Coordinate enhancement will be ENABLED")
        else:
            self.logger.warning("⚠️ DEBUG_COORD_ENH: Coordinate enhancement will be DISABLED")

...
from rv_llm.llm.constants import PromptStrategyType, LLMType
...
class ComponentConfigurator:
    ...
    def _initialize_registries(self):
        ...
        if not self._registries['llm'].get_names():
            ...
            self._registries['llm'].register_lazy('huggingface', 'rvandroid.llm.huggingface_llm',
                                                  'HuggingFaceLLM')
        ...
        if not self._registries['strategy'].get_names():
            self._registries['strategy'].register_lazy(
                PromptStrategyType.STANDARD, 'rvandroid.llm.prompt.strategy.strategies.standard_strategy',
                'StandardStrategy')
            self._registries['strategy'].register_lazy(
                PromptStrategyType.BATCH_ACTION, 'rvandroid.llm.prompt.strategy.strategies.batch_action_strategy',
                'BatchActionStrategy')

        # Register parser types
        if not self._registries['parser'].get_names():
            self._registries['parser'].register_lazy(
                'droidbot', 'rv_screen_parser.parser.screen.droidbot.droidbot_parser', 'DroidBotParser')
            self._registries['parser'].register_lazy(
                'uiautomator', 'rv_screen_parser.parser.screen.uiautomator.uiautomator_parser', 'UIAutomator2Parser')

        # Register visitor types
        if not self._registries['visitor'].get_names():
            self._registries['visitor'].register_lazy(
                'basic', 'rv_screen_parser.parser.screen.visitor.basic_visitor', 'BasicTextVisitor')
            self._registries['visitor'].register_lazy(
                'default', 'rv_screen_parser.parser.screen.visitor.default_visitor', 'DefaultTextVisitor')
            self._registries['visitor'].register_lazy(
                'enhanced', 'rv_screen_parser.parser.screen.visitor.enhanced_visitor', 'EnhancedTextVisitor')
    ...
    def create_parser(self) -> BaseScreenParser:
        """

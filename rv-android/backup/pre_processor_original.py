...
class PreProcessor:
    ...
    def process(self, generate_monitors: bool, instrument: bool, static_analysis: bool):
        """
        Execute the pre-processing phase with artifact reuse detection.
        ...
        with self.logger.with_context(phase="pre_processing"):
            ...
            # Generate monitors if requested and not skippable
            ...
            # Instrument APKs if requested and not skippable
            if instrument:
            ...
            # Perform static analysis if requested and not skippable
            if static_analysis:

...
**WTG transitions sweep (2026-06-11)**: full-WTG static-analysis sweep on the 169 APKs of `experimento-20260604` (to populate `transitions[]`, missing in the skip-wtg dataset). Result in `out/sweep_20260604_wtg_spark/`: 169/169 complete (reach+windows+components), 72 with `transitions>0`. Plan + findings: `docs/20260609_sweep_wtg_completo_169.md`.
  - **Gator SPARK fix (committed `e584894a`, repo `rvsec`)**: `FlowgraphRebuilder.java` arity guard fixes a deterministic `ArrayIndexOutOfBoundsException` crash in `WTGBuilder.build()` under SPARK CG delegation (`cgDelegation=true`). Source now matches the compiled `lib/gator/*.jar` (build 2026-06-09 19:02) — rebuilding gator no longer loses the fix. See plan doc §5b.

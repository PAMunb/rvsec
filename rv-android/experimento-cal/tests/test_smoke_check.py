...
def _trace(ack: dict, *, on_new_state: str) -> str:
    """Render a [APE-LLM-CONFIG] + [APE-LLM-CONFIG-ACK] trace, overriding on_new_state so a
    caller can inject a deliberate field mismatch."""
    return (
        "[APE-RV] boot\n"
        ...
        f"[APE-LLM-CONFIG-ACK] server_model={SERVER_MODEL}\n"
    )
...
def _build_tree(tmp_path: Path, *, a3_on_new_state: str) -> Path:
    ...
    the manifest, "true" injects the deliberate mismatch.
    """
    ...
    tasks = [
        _write_task(
            task_dir, "app.alpha_1", "cal_a1", _trace(_CAL_A1_ACK, on_new_state="true")
        ),
        _write_task(
            task_dir,
            "app.bravo_2",
            "cal_a3",
            _trace(_CAL_A3_ACK, on_new_state=a3_on_new_state),
        ),
    ]
...
def test_smoke_all_match_passes(tmp_path):
    """When every trace matches its manifest arm, the gate passes (exit 0, no failures)."""
    iter_dir = _build_tree(tmp_path, a3_on_new_state="false")
    failures = smoke_check.run_smoke_check(iter_dir)
    assert failures == [], failures
    assert smoke_check.main(["--iter-dir", str(iter_dir)]) == 0


def test_smoke_config_ack_field_by_field(tmp_path, capsys):
    ...
    # cal_a3's on_new_state is named; cal_a1 is untouched.

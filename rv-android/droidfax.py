...
class DroidFax:
    ...
    @staticmethod
    def instrument_all(input_path, out_dir):
        ...
        try:
            ...
            error_msg = 'Error while creating folder {0}'.format(out_dir)
        ...
        if len(apks) < 1:
            logging.error('There is not apks enough .......')
            sys.exit(1)
    ...
    @staticmethod
    def instrument(app: App, out_dir):
        logging.info('Instrumenting: {0}'.format(app.name))
        ...
        # Verify if apk is already instrumented.
        if os.path.exists(os.path.join(out_dir, app.name)):
            logging.info('Using instrumented {0}'.format(app.path))
            return

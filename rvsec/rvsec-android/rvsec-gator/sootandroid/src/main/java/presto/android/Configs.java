...
public class Configs {
  ...
  public static String sootAndroidDir;
  ...
  // hailong: enable StringBuilder.append(string) analysis
  public static boolean enableStringAppendAnalysis = false;

  // hailong: enable string property analysis on views
  public static boolean enableStringPropertyAnalysis = false;
...
//    Options.v().set_force_android_jar(Configs.android);
    ...
    for (Field f : cls.getFields()) {
      if (f.getType().equals(String.class)) {
        try {
          ...
          if (res == null) {
            Logger.err("Configs", "You need to set `Configs." + f.getName() + "'");
            System.exit(-1);
          }
        } catch (Exception e) {
          e.printStackTrace();
  ...
  public enum AsyncOpStrategy {
    Default_EventHandler_Async, // handle Activity.runOnUiThread and View.post as part of event
    // handler

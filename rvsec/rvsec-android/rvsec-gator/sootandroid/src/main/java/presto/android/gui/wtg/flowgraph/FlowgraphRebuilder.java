...
public class FlowgraphRebuilder {
        ...
        while (unitItr.hasNext()) {
          ...
          if (s.containsInvokeExpr()) {
            ...
            try {
              ...
              Logger.trace("rebuildFlow",
                      "Cannot resolve target method for InvokeExpr "
                              + s.getInvokeExpr().toString());
              continue;
            }
            //Some classes or methods are missing in some api level.
            if (callee == null) {
              Logger.warn("rebuildFlow", "Cannot resolve target method for "
                      + s.getInvokeExpr().toString());
              continue;
            }
  ...
  // hailong: move to flow graph
  private boolean createPropagation(Stmt s) {
    if (!(s instanceof DefinitionStmt) || !s.containsInvokeExpr()) {
      return false;
    }
    return wtgUtil.isIntentPropagationCall(s) || wtgUtil.isValuePropagationCall(s);
//    if (!(s instanceof DefinitionStmt) || !s.containsInvokeExpr()) {
//      return false;
//    }
//    InvokeExpr ie = s.getInvokeExpr();
...
//    return true;
  ...
  private NNode simpleNode(Value x) {
    ...
    if (x instanceof ClassConstant) {
      ...
      if (clsName.charAt(clsName.length() - 1) == ';' && clsName.charAt(0) == 'L') {

 ...
 *
 * <p>{@link #RVSEC_PINNED} is {@code c005c2e5}: the six {@code .mop}-side routes were all re-run
 * there on 2026-09-08, because {@code a599be6b} had removed
 * {@code jca_android/RandomStringPassword.mop} from the corpus on 2026-09-02 and five of the six
 * answers moved with it. They were re-taken and not edited to match the component, which is the
 * only reading of INV-CONF-14 that leaves the gate able to fail: {@code probes/Census.java} and
 * {@code probes/Binding.java} were recompiled and re-run, the target-8 generation pass was re-run
 * over the 47 specifications, and target 6 was re-derived from the committed map. Every one of the
 * six agrees with the component at that commit. {@link #ORACLE_PINNED} did not move and neither
 * did its two targets: the oracle repository has not changed since May, and stamping it with a
 * September {@code rvsec} commit would date a measurement that was not re-taken. {@link #ORACLE_PINNED} is {@code f2f4d3b}, and it is the current HEAD
...
public final class CalibrationTargets {
    ...
    public static final String RVSEC_PINNED = "c005c2e5";
    ...
    /** The day the oracle-side routes were run, for the stamps that carry an instant. */
    public static final Instant TAKEN_AT = Instant.parse("2026-08-24T00:00:00Z");

    /** The day the {@code .mop}-side routes were re-run, all six of them, at {@link #RVSEC_PINNED}. */
    public static final Instant RVSEC_TAKEN_AT = Instant.parse("2026-09-08T00:00:00Z");
    ...
    private static SourceStamp rvsec() {
        return new SourceStamp("rvsec", RVSEC_PINNED, RVSEC_TAKEN_AT);
    }
    ...
    public static CalibrationTarget mopLift() {
        return new CalibrationTarget("T1-mop-lift",
                ...
                "238 files, 238 ok, 0 fail",
                List.of("jca 23/23", "jca_android 47/47", "jca_android_bug_predicate 23/23",
                ...
                "this is the one target whose corpus moved after the set was pinned: jca_android "
                        + "went from 48 files to 47 when a599be6b removed RandomStringPassword.mop, "
                        + "a specification that cannot accuse under any trace. The route was re-run "
                        + "over the five corpora at that later commit rather than the pinned value "
                        + "being edited to match the component, and it answers what the component "
                        + "answers; the other seven targets keep the original stamp because their "
                        + "corpora did not move");
    }
    ...
    public static CalibrationTarget androidMultiParameter() {
        return new CalibrationTarget("T3-android-multiparameter",
                ...
                "0 of 47", List.of(),
                "spec.getParameters().size() > 1 on the parsed AST, over the 47 files of "
                        + "jca_android. The histogram is {1:47}: every specification of the set "
                        ...
                        + "something. It is 47 and not 48 since a599be6b removed "
                        + "RandomStringPassword.mop, which declared one parameter like every other "
                        + "file of the set: the denominator moves, the answer does not",
                RouteClass.INDEPENDENT_PROBE, HARNESS + "/probes/Census.java",
    ...
    public static CalibrationTarget pairing() {
        return new CalibrationTarget("T6-pairing",
                ...
                "44 of 47",
                List.of("IvChainJunction", "OAEPParameterSpecSpec", "SSLEngineSpec"),
                "the 47 jca_android specifications minus the one that "
                        + "data/jca_android/order_alphabet_map.csv declares a G-ORDER skip, minus "
                        ...
                        + "therefore states no ordering to compare. The numerator is 44 before and "
                        + "after a599be6b: RandomStringPassword was one of the two declared skips "
                        + "and left the set with the file, so the denominator lost one and the "
                        + "subtractions lost the same one. The items are the skip and the two rules "
                        + "that do not load, named by .mop file name without the extension",
    ...
    public static CalibrationTarget partialBinding() {
        return new CalibrationTarget("T7-partial-binding",
                ...
                "1 of 47",
                List.of("HMACParameterSpecSpec"),
                "over the 47 jca_android specifications that declare at least one parameter, those "
                        ...
                        + "task 4.2. RandomStringPassword was the second member of the numerator "
                        + "and left it with the set at a599be6b: its two events bind neither of "
                        + "the parameters it declared, which is the same statement as its refusal "
                        + "under M0, and a file outside the corpus is neither numerator nor "
                        + "denominator. Items are .mop file names without the extension",
    ...
    public static CalibrationTarget withoutMapOfMonitor() {
        return new CalibrationTarget("T8-without-map-of-monitor",
                ...
                "1 of 47",
                List.of("HMACParameterSpecSpec"),
                "the 47 jca_android specifications are regenerated with rv-monitor-generator into "
                        ...
                        + "DECLARED name, so IvChainJunctionSpec is mapped back to its file "
                        + "IvChainJunction.mop before the items are compared",
                RouteClass.REGENERATED_ARTIFACT,
                "rv-monitor-generator over the 47 jca_android specs, into scratch; the generated "
                ...
                "the component answers with an AST proxy — at least one declared parameter and at "
                        ...
                        + "is what it costs a generation pass for (D-18). The pass was re-run on "
                        + "2026-09-08 over all 47 specifications: the generated "
                        + "MultiSpec_1RuntimeMonitor.java declares 46 MapOfMonitor fields and the "
                        + "one without is the one named above, so the monitor and the proxy agree "
                        + "here and the route is a route again. It read three at 38 specifications "
                        + "under gh109 task 2.R: CipherInputStreamSpec, CipherOutputStreamSpec and "
                        + "KeyStoreSpec had left under gh105, which gave each a parameter to index "
                        + "on, KeySpec joined at gh109 task 2.14 and left again at task 7.1, and "
                        + "RandomStringPassword left the set itself at a599be6b");
    }
